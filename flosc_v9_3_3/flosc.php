...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7: [link]. It has the complete IPA guide and video walkthrough. Ready to dive in?\"",
    ...
    public function get_quiz_questions($request) {
        ...
        // Get quiz from options or use sample
        $saved_quiz = get_option('flosc_quiz_' . $quiz_id);
        
        if ($saved_quiz && !empty($saved_quiz['questions'])) {
            return new WP_REST_Response([
                'success' => true,
                'id' => $quiz_id,
                'title' => $saved_quiz['title'] ?? 'Quick Assessment',
                'questions' => $saved_quiz['questions'],
            ]);
        }
        
        // Return sample quiz for testing/fallback
        $sample_questions = [
            [
                'id' => 'q1',
