...
Note on user-specific AI guidance: an administrator who can edit users may save and enable "Sticky for User" on a WordPress user profile. FLOSC places that private, administrator-authored section immediately after the attached personality only in that authenticated user's AI prompts. The text remains saved but is not sent while its checkbox is disabled. Supported variables can expand to that user's name, email address, user ID, access/member level, quiz score, weakest phonemes, and the current flow/site name. Therefore, when this optional field is enabled, its text and expanded personal data are sent to the AI provider configured for that flow. The field is never sent for another user, never displayed verbatim to the user, and is not used by scripted IVR.
