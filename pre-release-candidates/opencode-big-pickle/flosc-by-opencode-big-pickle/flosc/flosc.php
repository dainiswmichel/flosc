	...
	private function init_hooks() {
		...
		if ( ! get_option( 'flosc_menus_v800' ) ) {
			foreach ( array( 'flosc_guest_menu_items', 'flosc_member_menu_items' ) as $_menu_key ) {
				...
				if ( ! in_array( 'view_profile', $_actions )) { array_unshift( $_menu, array(
				'label' => 'My Profile',
				'action' => 'view_profile',
				) );
                }
				if ( ! in_array( 'logout', $_actions )) {       $_menu[] = array(
				'label' => 'Log Out',
				'action' => 'logout',
				);
                }
...
The Team',
	...
	public function handle_kb_upload() {
		...
		if ( function_exists( 'flosc_knowledge_base_put' ) && ! flosc_knowledge_base_get( $kb_id ) ) {
			flosc_knowledge_base_put( array(
			'id' => $kb_id,
			'label' => $kb_id,
			'access' => array(),
			) );
...
\"As a member you have full access. For that topic, start with the matching lesson — want me to open it?\"",
	...
	private function process_prelogin_data_for_user( $user_id ) {
		...
		if ( ! $score_data || ! isset( $score_data['score'] ) ) {
			...
			if ( $raw && isset( $raw['score'] ) ) {
				...
				setcookie( 'flosc_quiz_result', '', array(
				'expires' => time() - 3600,
				'path' => '/',
				'samesite' => 'Lax',
				) );
	...
	* The browser already scored each phrase against the flow-configured pronunciation API during the quiz.
	...
	public function ajax_serve_user_audio() {
		...
		// phpcs:disable WordPress.Security.NonceVerification.Recommended -- the reads in this block are the signed-media URL payload; origin proof is the HMAC in ?sig=/?exp= checked with hash_equals() by is_valid_audio_access_signature() below, and a nonce cannot travel in an <audio src>.
	...
	public function render_buddyboss_quiz_tab() {
		...
		if ( ! $profile_completed && $is_guest_user ) {
			...
			$days_note    = ( $days_remaining !== null )
				? 'This page and all associated data will be removed from our servers in <strong>' . esc_html((string) $days_remaining) . '</strong> day' . ( $days_remaining !== 1 ? 's' : '' ) . ' if you don\'t upgrade.' . $upgrade_link
				: '';
			...
			$days_note = ( $days_remaining !== null )
				? ' You have <strong>' . esc_html((string) $days_remaining) . '</strong> day' . ( $days_remaining !== 1 ? 's' : '' ) . ' of guest access remaining.'
				: '';
		...
		if ( $phrase_results && empty( $quiz_attempts ) ) {
			...
			foreach ( $phrase_results as $i => $pr ) {
				...
				echo '<span class="flosc-quiz-summary-left">';
				echo '<span class="flosc-bb-chevron flosc-quiz-chevron">&#9654;</span>';
				echo '<span class="flosc-quiz-summary-text"><strong>Phrase ' . esc_html((string) ( $i + 1 )) . ':</strong> ' . esc_html( $phrase_text ) . '</span>';
				echo '</span>';
