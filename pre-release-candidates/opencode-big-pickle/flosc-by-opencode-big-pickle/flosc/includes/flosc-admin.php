...
trait FLOSC_Admin_Trait {
	...
	public function add_admin_menu() {
		...
		add_submenu_page(
			...
			'manage_options',
			'flosc-identity',
			array( $this, 'redirect_to_identity_tab' )
		);
		...
		add_submenu_page(
			...
			'manage_options',
			'flosc-da1',
			array( $this, 'redirect_to_da1_tab' )
		);
	}
	...
	public function register_settings() {
		...
		$this->register_settings_group(
			array(
				...
				'flosc_content_item_category'        => 'text',
				'flosc_oto_offer_id'                 => 'text',
				'flosc_free_content_item_mode'       => 'text',
				'flosc_free_content_item_count'      => 'text',
				'flosc_free_content_item_proportion' => 'text',
		...
		<section class="flosc-contact-form-wrap" aria-label="Contact Form">
			<div class="flosc-contact-form-card">
				...
				<form class="flosc-contact-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" novalidate>
					...
					<input type="hidden" name="flosc_contact_rendered_at" value="<?php echo esc_attr( (string) $rendered_at ); ?>">
	...
	private function get_contact_form_settings( $flow_id = '' ) {
		...
		if ( $flow_id !== '' ) {
			...
			if ( ! empty( $flow['ivr_file'] ) ) {
				...
				$flow_settings = get_option( 'flosc_flow_' . $resolved_flow, array() );
				if ( is_array( $flow_settings ) ) {
					$settings = $flow_settings;
