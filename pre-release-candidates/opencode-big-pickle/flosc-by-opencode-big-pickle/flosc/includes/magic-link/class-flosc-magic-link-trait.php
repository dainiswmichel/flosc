...
trait FLOSC_Magic_Link_Trait {
	...
	public function handle_login_token() {
		...
		foreach ( array(
			 ...
			 */
			// phpcs:disable WordPress.Security.NonceVerification.Recommended -- CSRF control is the token itself, verified below before any session; see comment above.
	...
	private function build_guest_request_admin_redirect( $notice_key ) {
		...
		if ( $ivr === '' ) {
			...
			// phpcs:disable WordPress.Security.NonceVerification.Missing -- every call site (handle_guest_request_approve / _approve_send / _deny_block / _delete) verifies check_admin_referer( 'flosc_guest_request_action' ) and current_user_can( 'manage_options' ) before this helper is reached.
