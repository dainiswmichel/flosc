...
function flosc_nav_param( $key, array $allowed = array(), $default = '', $sanitizer = 'sanitize_key' ) {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- display-selection GET reads; constrained by $allowed or a named sanitizer, never used to authorize an action or write.
	if ( ! isset( $_GET[ $key ] ) || ! is_scalar( $_GET[ $key ] ) ) {
		return $default;
	}

	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- see the read above; same display-selection boundary.
...
function flosc_nav_param_present( $key ) {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- presence-switch on a display GET parameter; the branch it feeds reads nothing and writes nothing.
