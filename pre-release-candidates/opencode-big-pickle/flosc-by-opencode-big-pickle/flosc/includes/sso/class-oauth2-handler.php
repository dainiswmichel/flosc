...
class OAuth2_Handler {
	...
	public function handle_callback( $request ) {
		 ...
		 * a request this site did not compose.
		 *
		...
		foreach ( array( 'code', 'state', 'error', 'error_description' ) as $flosc_k ) {
			// phpcs:disable WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended -- provider callback; CSRF control is the OAuth `state`, verified by verify_state() before any value here is used.
	...
	private function generate_state( $provider_id, $redirect_to = '', $flow_id = '' ) {
		...
		if ( ! $verify ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( '[FLOSC SSO] WARNING: State transient not readable after save — options fallback active' );
			}
		}
	...
	private function verify_state( $state ) {
		...
		$transient_key = self::STATE_PREFIX . $state;
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( '[FLOSC SSO] Verifying state for key: ' . $transient_key );
		}
