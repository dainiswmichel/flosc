...
$flosc_protection_fields = array(
	'anonymous_chat_limit'     => array( 'Anonymous chat', 'Chat requests an hour from one visitor who is not logged in. A conversation costs more requests than reading does, so chat has its own budget.' ),
	'authenticated_chat_limit' => array( 'Signed-in chat', 'Chat requests an hour from one logged-in person.' ),
	'anonymous_ivr_limit'      => array( 'Anonymous content reads', 'Requests an hour to the other public endpoints — IVR content, offers, page context.' ),
	'metered_compute_limit'    => array( 'Metered compute', 'Requests an hour to endpoints that spend tokens.' ),
...
if ( $flosc_can_assign_editors ) {
	...
	$flosc_assigned_editor_ids = array_map(
	...
	<table class="widefat striped flosc-admin-status-table">
		...
		<tbody>
			...
			<tr>
				...
				<td><?php echo esc_html( '1' === $flosc_protection['enabled'] ? 'On' : 'Off' ); ?></td>
			</tr>
			<tr>
