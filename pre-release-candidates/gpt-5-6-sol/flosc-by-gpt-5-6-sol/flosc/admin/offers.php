...
function flosc_parse_offer_coupons_from_post( array $flosc_post ) {
	...
	$types   = is_array( $flosc_post['offer_coupon_type'] ?? null ) ? $flosc_post['offer_coupon_type'] : array();
	$values  = is_array( $flosc_post['offer_coupon_value'] ?? null ) ? $flosc_post['offer_coupon_value'] : array();
	$froms   = is_array( $flosc_post['offer_coupon_valid_from'] ?? null ) ? $flosc_post['offer_coupon_valid_from'] : array();
	$untils  = is_array( $flosc_post['offer_coupon_valid_until'] ?? null ) ? $flosc_post['offer_coupon_valid_until'] : array();
	$actives = is_array( $flosc_post['offer_coupon_active'] ?? null ) ? $flosc_post['offer_coupon_active'] : array();
	...
	for ( $i = 0; $i < $n; $i++ ) {
		...
		if ( ! in_array( $flosc_type, array( 'fixed_price', 'percent' ), true ) ) {
...
function flosc_handle_offer_save() {
	...
	$all_formats     = array( 'card', 'pill', 'compact', 'banner', 'featured', 'text', 'inline-checkout' );
	$display_formats = array();
	foreach ( $all_formats as $fmt ) {
		$key                     = str_replace( '-', '_', $fmt );
		$display_formats[ $fmt ] = array(
		...
		if ( in_array( $fmt, array( 'card', 'featured', 'banner' ), true ) ) {
	...
	$flosc_fs       = get_option( $fk, array() );
	$all_offers     = $flosc_fs['offers'] ?? array();
	...
	if ( array_key_exists( 'flosc_offer_token_source', $flosc_post ) ) {
		...
		if ( ! in_array( $token_source, array( 'flow', 'custom', 'none' ), true ) ) {
			$token_source = 'flow';
		}
		$token_mode = sanitize_key( (string) ( $flosc_post['flosc_offer_token_mode'] ?? 'onetime' ) );
		if ( ! in_array( $token_mode, array( 'onetime', 'recurring', 'recurring_yearly' ), true ) ) {
			$token_mode = 'onetime';
		}
		$token_cap_mode = sanitize_key( (string) ( $flosc_post['flosc_offer_token_cap_mode'] ?? 'flow' ) );
		if ( ! in_array( $token_cap_mode, array( 'flow', 'none', 'custom' ), true ) ) {
			$token_cap_mode = 'flow';
		}

		$tokens             = is_array( $existing_offer['tokens'] ?? null ) ? $existing_offer['tokens'] : array();
...
$flosc_all_format_meta = array(
	'card'            => array(
		'icon'  => '🃏',
		'label' => 'Card',
		'desc'  => 'Rich card with headline, features, pricing, CTA button',
	),
	'pill'            => array(
		'icon'  => '💊',
		'label' => 'Pill',
		'desc'  => 'Compact clickable pill in PromptPanel (AutoPrompt-style)',
	),
	'compact'         => array(
		'icon'  => '📋',
		'label' => 'Compact',
		'desc'  => 'Minimal one-line offer with price and CTA',
	),
	'banner'          => array(
		'icon'  => '🏗️',
		'label' => 'Banner',
		'desc'  => 'Wide banner with gradient background and countdown',
	),
	'featured'        => array(
		'icon'  => '⭐',
		'label' => 'Featured',
		'desc'  => 'Premium card with badge, savings callout, guarantee',
	),
	'text'            => array(
		'icon'  => '📝',
		'label' => 'Text',
		'desc'  => 'Plain text offer mention within conversation',
	),
	'inline-checkout' => array(
		'icon'  => '🛒',
		'label' => 'Inline Checkout',
		'desc'  => 'Full checkout form embedded in chat',
	),
);
?>
...
foreach ( $flosc_offers as $flosc_offer ) :
	...
	foreach ( $flosc_all_format_meta as $flosc_fid => $flosc_fm ) {
		if ( ! empty( $flosc_df[ $flosc_fid ]['enabled'] ) ) {
	...
	if ( empty( $flosc_enabled_fmts ) && ! empty( $flosc_offer['display_format'] ) ) {
		...
		if ( isset( $flosc_all_format_meta[ $flosc_bf ] ) ) {
			$flosc_enabled_fmts[] = $flosc_all_format_meta[ $flosc_bf ]['icon'] . ' ' . $flosc_all_format_meta[ $flosc_bf ]['label'];
		}
	}
	if ( empty( $flosc_enabled_fmts ) ) {
		$flosc_enabled_fmts[] = '🃏 Card';
	}
	?>
...
$flosc_demo_offers = array(
	array(
		...
		'offer_description' => "You've identified exactly which American English sounds trip you up. The full course gives you targeted lessons for every gap — so you stop guessing and start sounding natural.",
	...
	array(
		...
		'offer_name'        => 'Pronunciation Mastery',
		'offer_headline'    => 'Master American English Pronunciation — Complete System',
		'offer_description' => 'The complete pronunciation program. Every lesson, every sound, every accent pattern — with an AI coach that knows exactly where you are and what you need next.',
	...
	array(
		...
		'offer_badge'       => 'Free',
		'offer_savings'     => '',
		'offer_grants'      => 'guest_pronunciation_learner',
		'fmt_card'          => true,
		'fmt_pill'          => true,
...
function flosc_render_offer_editor_v2( $flosc_offer, $flosc_flow_key, $flosc_current_ivr, $flosc_all_format_meta ) {
	...
	if ( empty( $flosc_df ) && $is_new ) {
		$flosc_df['card'] = array( 'enabled' => true );
	}
	?>
	<form method="post">
		<?php wp_nonce_field( 'flosc_save_offer', 'flosc_save_offer_nonce' ); ?>
		...
		<table class="form-table flosc-offer-form-table">
							...
							<tr>
								...
								<td>
									<select name="offer_currency" class="flosc-offer-width-100px">
										<?php foreach ( array( 'USD', 'EUR', 'GBP', 'CAD', 'AUD' ) as $cur ) : ?>
			...
			<tr>
				...
				<td>
					<?php
					$flosc_offer_acs = $flosc_offer['access_codes'] ?? array();
					if ( ! is_array( $flosc_offer_acs ) ) {
						$flosc_offer_acs = array();
					}
			...
			<tr>
				...
				<td>
					...
					<?php
					$flosc_coupons = $flosc_offer['coupons'] ?? array();
					...
					<table class="widefat striped flosc-table-max-960">
						...
						<tbody>
						<?php
						foreach ( $flosc_coupons as $ci => $fc ) :
							if ( ! is_array( $fc ) ) {
								continue;
							}
							?>
		...
		<table class="form-table flosc-offer-form-table">
			...
			<th class="flosc-offer-col-150"><label>Show When</label></th>
				<td>
					<select name="offer_trigger">
						<?php
						foreach ( array(
							'manual'          => 'Manual (pill/phrase only)',
							'quiz_complete'   => 'Quiz Completed',
							'lesson_complete' => 'First Lesson Completed',
							'login_phase'     => 'User Enters Login Phase',
							'inactivity'      => 'After 7 Days Inactivity',
						) as $v => $l ) :
							?>
			...
			<tr id="offer-after-offer-row-<?php echo esc_attr( $flosc_safe_id ); ?>" class="<?php echo ( 'after_offer' === $flosc_reveal_event ) ? '' : 'flosc-hidden'; ?>">
				...
				<td>
					...
					$flosc_all_for_parent = $flosc_fs_for_editor['offers'] ?? array();
					if ( ! is_array( $flosc_all_for_parent ) ) {
						$flosc_all_for_parent = array();
					}
					?>
					<select name="offer_after_offer_id" id="offer_after_offer_id_<?php echo esc_attr( $flosc_safe_id ); ?>" class="regular-text">
						...
						<?php
						foreach ( $flosc_all_for_parent as $flosc_pid => $flosc_poff ) :
			...
			<tr>
				...
				<td>
					...
					$ml_registry   = $flosc_fs_for_editor['member_levels'] ?? array();
					...
					<p class="description">Select a level from the <a href="
					<?php
					echo esc_url(
						add_query_arg(
							array(
		...
		<?php
		foreach ( $flosc_all_format_meta as $fmt_id => $flosc_fmt_meta ) :
			$flosc_fmt_key = str_replace( '-', '_', $fmt_id );
			$fmt_data      = $flosc_df[ $fmt_id ] ?? array();
