...
/**
 * Coordinate FLOSC Companion Mode behavior and the WordPress services used by its methods.
 */
 ...
 * @return mixed Result produced by the enqueue companion operation.
	...
	public function enqueue_companion() {
		...
		$context_params = $pass_page_context ? $this->build_companion_context_params( $context_scope ) : array();
		...
		if ( ! is_array( $companion_config['contextParams'] ?? null ) ) {
			$companion_config['contextParams'] = array();
		}
		if ( ! is_array( $companion_config['triggerSuppressPathPatterns'] ?? null ) ) {
			$companion_config['triggerSuppressPathPatterns'] = array();
		}
	...
	private function get_host_auth_config( $flow_id ) {
		...
		if ( class_exists( '\FLOSC\SSO\SSO_Manager' ) && '' !== $flow_id ) {
			...
			$host   = isset( $_SERVER['HTTP_HOST'] )
				...
				: '';
			foreach ( array( 'google', 'facebook', 'apple', 'microsoft', 'linkedin' ) as $pid ) {
 ...
 * @return mixed Result produced by the companion flow context operation.
	...
	private function resolve_companion_flow_context( $handoff_request = false ) {
		...
		if ( '' !== $hint ) {
			...
			$hint_ok = is_array( $hint_flow )
				...
				&& in_array( (string) ( $hint_flow['companion_content_display_mode'] ?? 'in_chat' ), array( 'companion', 'both' ), true );
	...
	private function find_companion_flows_for_request( $req_path, array $category_slugs ) {
		...
		foreach ( $ivr_files as $filename ) {
			...
			if ( ! $enabled || ! in_array( $mode, array( 'companion', 'both' ), true ) ) {
				continue;
			}
 ...
 * @return mixed Result produced by the companion chat app url operation.
	...
	private function get_companion_chat_app_url() {
		...
		if ( ! empty( $app ) ) {
			$candidates[] = esc_url_raw( (string) $app, array( 'http', 'https' ) );
		}
 ...
 * @return mixed Result produced by the companion allowed modes operation.
	...
	private function get_companion_allowed_modes() {
		...
		return is_array( $modes ) ? array_values( array_unique( array_map( 'strval', $modes ) ) ) : array( 'in_chat', 'companion', 'both' );
	}
 ...
 * @return mixed Result produced by the companion widget modes operation.
	...
	private function get_companion_widget_modes() {
		...
		return is_array( $modes ) ? array_values( array_unique( array_map( 'strval', $modes ) ) ) : array( 'companion', 'both' );
	}
 ...
 * @return mixed Result produced by the companion allowed positions operation.
	...
	private function get_companion_allowed_positions() {
		...
		return is_array( $positions ) ? array_values( array_unique( array_map( 'strval', $positions ) ) ) : array( 'bottom-right', 'bottom-left' );
	}
 ...
 * @return mixed Result produced by the companion mobile behaviors operation.
	...
	private function get_companion_mobile_behaviors() {
		...
		return is_array( $behaviors ) ? array_values( array_unique( array_map( 'strval', $behaviors ) ) ) : array( 'fullscreen', 'panel' );
	}
 ...
 * @return mixed Result produced by the companion context scopes operation.
	...
	private function get_companion_context_scopes() {
		...
		return is_array( $scopes ) ? array_values( array_unique( array_map( 'strval', $scopes ) ) ) : array( 'basic', 'extended' );
	}
 ...
 * @return mixed Result produced by the companion motion modes operation.
	...
	private function get_companion_motion_modes() {
		...
		return is_array( $modes ) ? array_values( array_unique( array_map( 'strval', $modes ) ) ) : array( 'system', 'reduce', 'full' );
	}
 ...
 * @return mixed Result produced by the companion state storages operation.
	...
	private function get_companion_state_storages() {
		...
		return is_array( $storages ) ? array_values( array_unique( array_map( 'strval', $storages ) ) ) : array( 'session', 'local' );
	}
 ...
 * @return mixed Result produced by the companion target types operation.
	...
	private function get_companion_target_types() {
		...
		return is_array( $types ) ? array_values( array_unique( array_map( 'strval', $types ) ) ) : array( 'path', 'page', 'post', 'category', 'tag' );
	}
