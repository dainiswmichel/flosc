...
class FLOSC_Bridge_Data_Manager {
	...
	public function handle_purchase_completed( $user_id ) {
		...
		$this->clear_bridge_state( $user_id );

		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "FLOSC Bridge: User {$user_id} purchased - exited bridge state" );
		}
	}
	...
	public function handle_quiz_completion( $quiz_result, $user_id ) {
		...
		$correct   = $quiz_result['correct'] ?? array();
		$incorrect = $quiz_result['incorrect'] ?? array();
	...
	public function handle_external_quiz( $user_id, $quiz_id, $score_data ) {
		...
		$scoring_results = array(
			...
			'total_questions' => count( $score_data['correct_items'] ?? array() ) + count( $score_data['incorrect_items'] ?? array() ),
			'correct_items'   => $score_data['correct_items'] ?? array(),
			'incorrect_items' => $score_data['incorrect_items'] ?? array(),
			'item_results'    => $score_data['item_results'] ?? array(),
			'source'          => 'external',
	 ...
	 *
	 * @param mixed $item Value consumed by this operation.
	 * @return String.
	 */
	...
	public function flosc_create_bridge_data( $user_id, $quiz_id, $scoring_results ) {
		...
		$bridge_data = array(
			...
			'correct_items'   => $scoring_results['correct_items'] ?? array(),
			'incorrect_items' => $scoring_results['incorrect_items'] ?? array(),
			'item_results'    => $scoring_results['item_results'] ?? array(),
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "FLOSC Bridge: Created bridge data for user {$user_id}, quiz {$quiz_id}, score {$bridge_data['score']}%" );
		}
	...
	public function get_flosc_user_summary( $user_id ) {
		...
		return array(
			...
			'correct_count'    => count( $latest['correct_items'] ?? array() ),
			'incorrect_count'  => count( $latest['incorrect_items'] ?? array() ),
		);
	}
