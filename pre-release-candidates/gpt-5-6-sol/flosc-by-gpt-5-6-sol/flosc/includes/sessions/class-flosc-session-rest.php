...
/**
 * Coordinate FLOSC Session Rest behavior and the WordPress services used by its methods.
 */
		...
		if ( 'guest' === $user_state ) {
			...
			if ( $max_chats > 0 && $count >= $max_chats ) {
				...
				$limit_msg = str_replace(
					...
					array( (string) $max_chats, (string) $count, $flow_name, $name, $name ),
					(string) $limit_msg
				);
				return new WP_REST_Response(
					array(
	...
	public function delete_session_from_do( $session_id ) {
		...
		if ( is_wp_error( $response ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC: delete_session_from_do — {$session_id}: " . $response->get_error_message() );
			}
		}
	}
