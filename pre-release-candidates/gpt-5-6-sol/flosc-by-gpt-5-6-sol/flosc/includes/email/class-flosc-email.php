...
/**
 * Coordinate FLOSC Email behavior and the WordPress services used by its methods.
 */
class FLOSC_Email {
 ...
 * @return mixed Result produced by the send score email operation.
	...
	public function send_score_email( $user, $score_data ) {
		...
		$flow_settings = is_array( $context['settings'] ?? null ) ? $context['settings'] : array();
		...
		$body = str_replace(
			array( '{name}', '{score}', '{correct}', '{incorrect}', '{oto_section}', '{app_link}', '{product_name}' ),
			array( $user->display_name, $score, $correct_list, $incorrect_list, $oto_section, $oto_link, $product_name ),
			$body_template
		);
 ...
 * @return mixed Result produced by the send due guest followups for user operation.
	...
	public function send_due_guest_followups_for_user( $user_id ) {
		...
		if ( get_user_meta( $user->ID, '_flosc_purchased', true ) ) {
			return;
		}
...
public function dispatch_member_welcome_email( $user_id, $purchase_data = array() ) {
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "FLOSC member welcome: user {$user_id} flow {$flow_stem} level {$level} sent=" . ( $ok ? '1' : '0' ) );
		}
	}
 ...
 * @return mixed Result produced by the dispatch newsletter welcome email operation.
	...
	public function dispatch_newsletter_welcome_email( $user_id, $flow_id = '' ) {
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "FLOSC newsletter welcome: user {$user_id} flow {$flow_stem} sent=" . ( $ok ? '1' : '0' ) );
		}
	}
 ...
 * @return mixed Result produced by the send due series followups operation.
	...
	public function send_due_series_followups( $user, $prefix, $anchor_ts, $sent_meta_key, $flow_id ) {
		...
		$settings  = is_array( $context['settings'] ?? null ) ? $context['settings'] : array();
		$followups = $settings[ $prefix . '_followups' ] ?? array();
		if ( ! is_array( $followups ) || empty( $followups ) ) {
			return;
		}
		...
		$done    = is_array( $sent[ $bucket ] ?? null ) ? $sent[ $bucket ] : array();
		$updated = false;
		...
		foreach ( $followups as $i => $fu ) {
			...
			if ( $days_elapsed >= $day && ! in_array( $i, $done, true ) ) {
				...
				$updated = true;
				if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
					flosc_log( "FLOSC followup: user {$user->ID} {$bucket} idx {$i} day {$day}" );
				}
			}
		}
		if ( $updated ) {
 ...
 * @return mixed Result produced by the run sso email sequence for user operation.
	...
	public function maybe_run_sso_email_sequence_for_user( $user_id ) {
		...
		$flow_id     = sanitize_key( (string) get_user_meta( $user_id, '_flosc_registration_flow', true ) );
		$this->send_sso_welcome_email( $user_id, $provider_id, array( 'flow_id' => $flow_id ) );
 ...
 * @return mixed Result produced by the sso flow email sequence operation.
	...
	public function maybe_process_sso_flow_email_sequence( $user_id, $provider_id, $user_data ) {
		...
		$this->send_sso_welcome_email( $user_id, $provider_id, is_array( $user_data ) ? $user_data : array() );
		$this->send_due_guest_followups_for_user( $user_id );
	}
}
