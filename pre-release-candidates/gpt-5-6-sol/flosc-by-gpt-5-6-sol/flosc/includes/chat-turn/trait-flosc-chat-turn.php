...
/**
 * Coordinate FLOSC Chat Turn Trait behavior and the WordPress services used by its methods.
 */
...
/**
 * Coordinate the chat turn behavior implemented by this code path.
 *
 * @param mixed $request Request object carrying the input consumed by this handler.
 * @return Array Structured chat turn data.
 */
private function handle_chat_turn( $request ) {
		...
		if ( is_array( $context ) ) {
			...
			if ( isset( $context['browsing_surf_trail'] ) ) {
				$trail      = is_array( $context['browsing_surf_trail'] ) ? $context['browsing_surf_trail'] : array();
				$safe_trail = array();
		...
		if ( '' !== $flow_id || '' !== $ivr_file ) {
			...
			if ( ! empty( $resolved['messages'] ) ) {
				$ivr_config = array(
					...
					'phases'   => $resolved['phases'] ?? array(),
					'styles'   => $resolved['styles'] ?? array(),
				);
			}
		}
		...
		if ( class_exists( 'FLOSC_Trajectory' ) ) {
			$trajectory_flow_settings = array();
			if ( ! empty( $flow_key ) && isset( $fs ) && is_array( $fs ) ) {
				...
				$trajectory_flow_settings = get_option( $trajectory_key, array() );
			}
		...
		if ( class_exists( 'FLOSC_Concierge' ) ) {
			...
			if ( is_array( $concierge_response ) ) {
				...
				if ( in_array( $concierge_state, array( 'awaiting_password', 'retry' ), true ) ) {
			...
			$concierge_guidance = ( 0 !== strncmp( $message, '[SYSTEM:', 8 ) )
		...
		try {
			...
			try {
				...
				flosc_log(
					'[FLOSC] Page context metadata failed: ' . $flosc_meta_error->getMessage()
			...
			flosc_log(
				'[FLOSC] Page context init failed: ' . $flosc_page_context_error->getMessage()
		...
		if ( empty( $chatpack_conv_history ) ) {
			...
			if ( ! empty( $visitor_history ) && is_array( $visitor_history ) ) {
				...
				if ( ! empty( $chatpack_conv_history ) ) {
					...
					if ( is_array( $flosc_tail ) && 'user' === ( $flosc_tail['role'] ?? '' ) ) {
						array_pop( $chatpack_conv_history );
					}
					while ( ! empty( $chatpack_conv_history ) && ( 'user' !== ( $chatpack_conv_history[0]['role'] ?? '' ) ) ) {
						$flosc_lead = array_shift( $chatpack_conv_history );
						if ( is_array( $flosc_lead ) && 'assistant' === ( $flosc_lead['role'] ?? '' ) ) {
		...
		$is_system_generated = ( 0 === strncmp( (string) $message, '[SYSTEM:', 8 ) );
		...
		try {
			...
			if ( ! $response_message ) {
				...
				if ( $ai_available ) {
					...
					if ( $flosc_use_rag ) {
						...
						if ( $flosc_rag_response && ! is_wp_error( $flosc_rag_response ) ) {
							$response_message      = array(
								...
								'user_autoprompts' => $flosc_rag_response['user_autoprompts'] ?? array(),
								'phase_change'     => null,
							);
					...
					if ( ! $flosc_use_rag && ! $response_message ) {
						...
						$flosc_response_source = ( 'ai' === $dispatch_source && '' !== $ai_response ) ? 'ai' : 'fallback';
					}
				} else {
			...
			if ( ! empty( $visitor_reservation['id'] ) ) {
				$this->flosc_settle_visitor_reservation( $visitor_reservation['id'], $token_provider, array() );
				$visitor_reservation = null;
			}
		...
		$billing_usage = is_array( $billing_meta['usage'] ?? null ) ? $billing_meta['usage'] : array();
...
/**
 * Coordinate the chat with rag turn behavior implemented by this code path.
 *
 * @param mixed $request Request object carrying the input consumed by this handler.
 * @return Mixed Result produced by the chat with rag turn operation.
 */
private function handle_chat_with_rag_turn( $request ) {
		...
		if ( class_exists( 'FLOSC_Concierge' ) ) {
			...
			if ( '' !== $flow_id || '' !== $ivr_file ) {
				...
				$flow_messages = $resolved['messages'] ?? array();
				if ( ! empty( $flow_messages ) ) {
					$concierge_session  = $this->flosc_concierge_session_key( $session_id );
					$concierge_response = FLOSC_Concierge::handle( $message, $concierge_session, array( 'messages' => $flow_messages ) );
					if ( is_array( $concierge_response ) ) {
						$concierge_state = (string) ( $concierge_response['flosc_concierge'] ?? '' );
						if ( in_array( $concierge_state, array( 'awaiting_password', 'retry' ), true ) ) {
							$concierge_message = '';
						...
						$response_message      = array(
							'content'          => (string) ( $concierge_response['content'] ?? '' ),
							'user_autoprompts' => $concierge_response['user_autoprompts'] ?? array(),
		...
		if ( ! $validation_result['valid'] ) {
			...
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				...
				if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
					flosc_log( 'FLOSC SECURITY: Original response: ' . substr( $ai_response, 0, 200 ) );
				}
				if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
					flosc_log( 'FLOSC SECURITY: Violations: ' . wp_json_encode( $validation_result['violations'] ) );
				}
			}
		}
