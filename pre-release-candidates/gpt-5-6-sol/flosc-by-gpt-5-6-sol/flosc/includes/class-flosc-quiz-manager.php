...
class FLOSC_Quiz_Manager {
	...
	public function register_rest_routes() {
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'handle_external_quiz_rest' ),
				'permission_callback' => array( $this, 'check_external_quiz_permission' ),
			)
		);
	}
	...
	public static function submit_score( $user_id, $quiz_id, $score_data ) {
		if ( ! $user_id || ! $quiz_id || ! is_array( $score_data ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC Quiz Manager: Invalid parameters - user_id: {$user_id}, quiz_id: {$quiz_id}" );
			}
			return false;
		}
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "FLOSC Quiz Manager: Submitted score for user {$user_id}, quiz {$quiz_id}, score {$score_data['score']}%" );
		}
