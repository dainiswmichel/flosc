...
/**
 * Coordinate FLOSC Lesson Manager behavior and the WordPress services used by its methods.
 */
...
public static function instance() {
	...
	public function get_all_lessons() {
		...
		if ( empty( $categories ) ) {
			...
			if ( empty( $single ) ) {
				if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
					flosc_log( '[FLOSC Lessons] No lesson categories configured. Set content_item_groups or content_item_category.' );
				}
				return array();
			}
		...
		if ( empty( $cat_ids ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( '[FLOSC Lessons] None of the configured categories exist: ' . implode( ', ', $categories ) );
			}
			return array();
		}
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( '[FLOSC Lessons] v3.0.8 get_all_lessons: categories=' . implode( ',', $categories ) . ' found=' . $query->found_posts );
		}
