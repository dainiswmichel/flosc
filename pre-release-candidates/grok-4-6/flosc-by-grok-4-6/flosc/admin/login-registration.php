...
$flosc_guest_request_messages = array(
	'approved'            => array(
		'type' => 'success',
		'text' => 'Guest account request approved.',
	),
	'approve_sent'        => array(
		'type' => 'success',
		'text' => 'Guest account request approved and MagicLink sent.',
	),
	'approve_send_failed' => array(
		'type' => 'error',
		'text' => 'Approved, but MagicLink email send failed.',
	),
	'denied_blocked'      => array(
		'type' => 'success',
		'text' => 'Guest account request denied and email blocked.',
	),
	'deleted'             => array(
		'type' => 'success',
		'text' => 'Guest account request deleted.',
	),
	'invalid_email'       => array(
		'type' => 'error',
		'text' => 'Invalid email for guest account request action.',
	),
);
...
<table class="form-table">
	<tr>
		...
		<td>
			<textarea id="flow_user_status_visitor" name="flow_user_status_visitor" class="large-text" rows="2">
			<?php
	...
	<tr>
		...
		<td>
			<textarea id="flow_user_status_guest" name="flow_user_status_guest" class="large-text" rows="2">
			<?php
	...
	<tr>
		...
		<td>
			<textarea id="flow_user_status_member" name="flow_user_status_member" class="large-text" rows="2">
			<?php
	...
	<tr>
		...
		<td>
			<textarea id="flow_user_status_member_level" name="flow_user_status_member_level" class="large-text" rows="2">
			<?php
	...
	<tr>
		...
		<td>
			<textarea id="flow_user_status_admin" name="flow_user_status_admin" class="large-text" rows="2">
			<?php
...
<?php
$flosc_request_queue    = get_option( 'flosc_guest_account_request_queue', array() );
$flosc_request_queue    = is_array( $flosc_request_queue ) ? $flosc_request_queue : array();
$flosc_request_denylist = get_option( 'flosc_guest_account_request_denylist', array() );
$flosc_request_denylist = is_array( $flosc_request_denylist ) ? $flosc_request_denylist : array();
...
if ( empty( $flosc_queue_rows ) ) {
	...
	foreach ( $flosc_queue_rows as $flosc_request_row ) {
		...
		if ( ! in_array( $flosc_status, array( 'pending', 'approved', 'denied' ), true ) ) {
...
<?php
$flosc_guest_log = get_option( 'flosc_guest_link_log', array() );
