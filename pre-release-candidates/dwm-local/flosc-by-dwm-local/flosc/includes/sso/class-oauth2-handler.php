...
class OAuth2_Handler {
    ...
    public function handle_callback($request) {
        ...
        $get  = array();
        if (isset($payload['code'])) $get['code'] = $payload['code'];
        if (isset($payload['state'])) $get['state'] = $payload['state'];
        if (isset($payload['error'])) $get['error'] = $payload['error'];
        if (isset($payload['error_description'])) $get['error_description'] = $payload['error_description'];
        $post = $get;
        $server = array();
