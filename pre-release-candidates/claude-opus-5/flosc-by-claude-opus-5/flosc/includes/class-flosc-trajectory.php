...
class FLOSC_Trajectory {
	 ...
	 * @param mixed $flow_settings Flow settings.
	 */
	...
	public static function is_trajectory_post( $post ) {
		...
		foreach ( $terms as $term ) {
			...
			if ( intval( $term->parent ) > 0 && in_array( $slug, array( 'trajectory', 'trajectories' ), true ) ) {
	...
	public static function sync_post( $post ) {
		...
		if ( ! isset( $fs['trajectory_rules'] ) || ! is_array( $fs['trajectory_rules'] ) ) {
			$fs['trajectory_rules'] = array();
		}

		$id                            = self::post_rule_id( $post );
		$fs['trajectory_rules'][ $id ] = array(
			'id'                 => $id,
	...
	public static function unsync_post( $post ) {
		...
		$all_flows = function_exists( 'flosc_config_glob' ) ? flosc_config_glob( '*_ivr.md' ) : array();
