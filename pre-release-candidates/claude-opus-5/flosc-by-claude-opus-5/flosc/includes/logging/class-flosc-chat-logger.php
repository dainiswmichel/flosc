...
class FLOSC_Chat_Logger {
	...
	public function flosc_set_session_archived( $by, $value, $flow_id = '', $archived = true ) {
		...
		if ( $archived ) {
			...
			$keys = array_values(
				array_filter(
					$keys,
					static function ( $existing_key ) use ( $key ) {
	...
	private function flosc_write_journey_marks( $data ) {
		...
		$marks = array_values(
			array_filter(
				$marks,
				static function ( $m ) {
					return in_array( $m, array( '+G', '+M' ), true );
				}
			)
		);
	...
	public function flosc_log_chat( $data ) {
		...
		if ( ! in_array( $user_tier, array( 'visitor', 'guest', 'member' ), true ) ) {
			$user_tier = '';
		}
	...
	public function flosc_get_sessions( $flow_id = '', $max_rows = 800, $archive_status = 'active' ) {
		...
		$archive_status  = in_array( $archive_status, array( 'active', 'archived', 'all' ), true ) ? $archive_status : 'active';
