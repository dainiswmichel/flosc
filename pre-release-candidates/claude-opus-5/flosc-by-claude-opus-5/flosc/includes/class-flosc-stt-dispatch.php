...
class FLOSC_STT_Dispatch {
	...
	private function assemblyai_transcribe( $audio_path ) {
		...
		if ( is_wp_error( $upload_response ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( 'FLOSC AssemblyAI Upload Error: ' . $upload_response->get_error_message() );
			}
	...
	private function openai_whisper_transcribe( $audio_path ) {
		...
		$flosc_whisper_endpoint = 'https://api.openai.com/v1/audio/transcriptions';
		$response               = wp_remote_post(
			$flosc_whisper_endpoint,
			array(
				'headers' => array(
		...
		if ( is_wp_error( $response ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( 'FLOSC Whisper Error: ' . $response->get_error_message() );
			}
			return $response;
		}
