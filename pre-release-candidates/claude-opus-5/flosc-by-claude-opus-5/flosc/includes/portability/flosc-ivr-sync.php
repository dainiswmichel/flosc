	...
	foreach ( $lines as $raw_line ) {
		...
		if ( ! preg_match( '/^(\s*)([^:]+):(.*)$/', $line, $m ) ) {
			return array(
				'success' => false,
				'data'    => array(),
				'error'   => 'Invalid YAML line: ' . trim( $line ),
			);
		}
		...
		if ( 0 !== ( $indent % 2 ) ) {
			return array(
				'success' => false,
				'data'    => array(),
				'error'   => 'Indent must be multiples of 2 spaces',
			);
		}
		...
		if ( empty( $stack ) ) {
			return array(
				'success' => false,
				'data'    => array(),
				'error'   => 'Invalid YAML indentation hierarchy',
			);
		}
...
function flosc_portable_settings_bootstrap_allowlist() {
	return array(
		...
		// Policy page definitions travel with their content: a kit that
		// carried the body of a terms page but not its address or heading
		// would import as a page at a different URL from the one it left.
...
function flosc_import_ivr_to_database( $preview_only = false, $custom_ivr_file = null, $flow_key = null, $mode = 'merge' ) {
	...
	if ( ! file_exists( $ivr_file ) ) {
		return array(
			'success' => false,
			'message' => 'flosc_default_technical_ivr.md file not found',
		);
	}
	...
	if ( function_exists( 'flosc_is_allowed_ivr_source_path' ) && ! flosc_is_allowed_ivr_source_path( $ivr_file ) ) {
		return array(
			'success' => false,
			'message' => 'IVR file path is not an allowed FLOSC configuration location',
		);
	}
	...
	if ( empty( $config ) ) {
		return array(
			'success' => false,
			'message' => 'Failed to parse flosc_default_technical_ivr.md',
		);
	}
	...
	$incoming_messages = $config['messages'] ?? array();
	...
	if ( $preview_only ) {
		...
		foreach ( $to_update as $msg_id ) {
			$db_msg   = $normalize_for_compare( $current_messages[ $msg_id ] ?? array() );
			$file_msg = $normalize_for_compare( $incoming_messages[ $msg_id ] ?? array() );
			$diffs    = array();
	...
	if ( 'merge' === $mode ) {
		...
		$final_phases = array();
		foreach ( ( $config['phases'] ?? array() ) as $phase_name => $message_ids ) {
	...
	if ( is_array( $phases ) ) {
		foreach ( $phases as $phase_name => $message_ids ) {
			if ( ! isset( $normalized_phases[ $phase_name ] ) ) {
				$normalized_phases[ $phase_name ] = array();
			}
...
function flosc_sync_flow_offers_with_ivr_messages( $flow_key, $messages ) {
	if ( empty( $flow_key ) || ! is_array( $messages ) ) {
		return array(
			'success' => false,
			'error'   => 'Invalid flow key or messages payload',
		);
	}

	$fs              = get_option( $flow_key, array() );
	$existing_offers = is_array( $fs['offers'] ?? null ) ? $fs['offers'] : array();
	...
	foreach ( $referenced_offers as $offer_id => $seed_offer ) {
		$existing_offer = ( isset( $existing_offers[ $offer_id ] ) && is_array( $existing_offers[ $offer_id ] ) ) ? $existing_offers[ $offer_id ] : array();
		...
		$existing_meta        = ( isset( $existing_offer['meta'] ) && is_array( $existing_offer['meta'] ) ) ? $existing_offer['meta'] : array();
	...
	if ( ! empty( $removed_offer_ids ) ) {
		...
		if ( ! empty( $removed_snapshot ) ) {
			if ( ! isset( $fs['offers_removed_by_sync'] ) || ! is_array( $fs['offers_removed_by_sync'] ) ) {
				$fs['offers_removed_by_sync'] = array();
			}
