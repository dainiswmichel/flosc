...
class FLOSC_Sample_Text_Based_Quiz extends FLOSC_Abstract_Quiz_Type {
	...
	public function analyze( $input, $expected_content, $context = array() ) {
		...
		foreach ( $missed as $answer ) {
			$refs        = $content_map[ $answer ] ?? array();
			$incorrect[] = array(
				'answer'          => $answer,
				'correct_content' => $refs['correct_content'] ?? array(),
				'related_content' => $refs['related_content'] ?? array(),
				'topics'          => array(),
			);
		}
	...
	public function format_results( $analysis, $lessons, $response_templates ) {
		...
		$message = str_replace(
			array( '{score}', '{total_correct}', '{total_possible}', '{lesson_recommendations}' ),
			array( $score, $details['total_correct'], $details['total_possible'], $lesson_text ),
			$template
		);
