...
class FLOSC_Chatpack {
		...
		switch ( $phase ) {
			case 'freeline':
				return $quiz_in_progress
					? array( 'Finish the in-progress quiz' )
					: array( 'Quiz entry', 'Inquiry clarification' );
			case 'login':
				return array( 'Quiz result clarity', 'Free lesson delivery' );
			case 'offer':
				return array( 'Offer readiness', 'Objection handling' );
			case 'sale':
				return array( 'Confident purchase confirmation', 'Member onboarding' );
			case 'content':
				return array( 'Learning momentum', 'Progress reinforcement' );
			default:
				return array();
		}
	}
	...
	private static function load_knowledge_files( $eval_context ) {
		...
		if ( ! in_array( $user_level, array( 'visitor', 'guest', 'member' ), true ) ) {
			$user_level = 'visitor';
		}
