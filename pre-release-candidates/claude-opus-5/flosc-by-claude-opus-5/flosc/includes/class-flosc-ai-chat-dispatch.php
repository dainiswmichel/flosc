...
class FLOSC_AI_Chat_Dispatch {
	...
	private function flosc_context_user_is_member( $context ) {
		...
		if ( array_key_exists( 'purchased', $context ) ) {
			...
			if ( is_string( $purchased ) ) {
				...
				if ( in_array( $norm, array( 'yes', 'true', '1', 'member' ), true ) ) {
					return true;
				}
			}
		}
	...
	private function build_context_string( $context ) {
		...
		$skip_keys = array( 'ivr_guidance' );

		// Put identity fields first so the model sees them before long KB/quiz blocks.
		$priority_keys = array( 'access_level', 'is_member', 'purchased', 'member_entitlement', 'user_name', 'phase', 'logged_in' );
	...
	private function resolve_model_price_per_1m( $provider, $model ) {
		...
		if ( false !== strpos( $m, 'haiku' ) ) {
			...
			$seed = array(
				'input'  => 300000,
				'output' => 1500000,
			);   // ~$3 / $15 per 1M (override in settings)
		}
	 ...
	 * @param mixed $context       Context.
	 * @param mixed $test_mode     Test mode.
	 */
	private function call_provider( $provider, $message, $system_prompt, $context, $test_mode ) {
		switch ( $provider ) {
			case 'openai':
				return $this->openai_request( $message, $system_prompt, $context, $test_mode );
			case 'anthropic':
				return $this->anthropic_request( $message, $system_prompt, $context, $test_mode );
			case 'xai':
				return $this->xai_request( $message, $system_prompt, $context, $test_mode );
			case 'gemini':
	...
	private function wp_ai_chat_request( $provider, $message, $system_prompt, $context = array(), $test_mode = false ) {
		...
		$result = FLOSC_WP_AI_Client::generate(
			array(
				'provider'      => $provider,
				'message'       => $message,
	...
	private function xai_request( $message, $system_prompt, $context = array(), $test_mode = false ) {
		...
		/*
		 * The endpoint is named on its own line so the suppression below stays
		 * attached to it. It used to sit inline in the wp_remote_post() call, and
		 * a WordPress Coding Standards pass split that call across lines --
		 * leaving the phpcs:ignore above `wp_remote_post(` and the host it
		 * annotates one line further down, where the suppression does not reach.
		 */
		...
		$flosc_xai_endpoint = 'https://api.x.ai/v1/chat/completions';
			...
			array(
				...
				'body'    => wp_json_encode(
					array_merge(
						...
						function_exists( 'flosc_get_model_parameters' ) ? flosc_get_model_parameters( 'xai' ) : array()
					)
				),
				'timeout' => 30,
			)
		...
		if ( is_wp_error( $response ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( 'FLOSC xAI Error: ' . $response->get_error_message() );
			}
			if ( $test_mode ) {
				return new WP_Error(
		...
		if ( isset( $body['error'] ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( 'FLOSC xAI API Error: ' . wp_json_encode( $body['error'] ) );
			}
			if ( $test_mode ) {
