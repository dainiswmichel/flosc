...
class FLOSC_Framework {
	...
	/**
	 * The one instance, built on the first call to instance().
	 *
	 * @var FLOSC_Framework|null
	 */
	private static $instance = null;

	/**
	 * Filesystem access, with every path checked against the directories FLOSC owns.
	 *
	 * @var FLOSC_Filesystem
	 */
	private $filesystem;

	/**
	 * Client IP resolution, rate limiting and signed cookies.
	 *
	 * @var FLOSC_Request_Guard
	 */
	private $request_guard;

	/**
	 * DA1 catalog loading, parsing and catalog-query answering.
	 *
	...
	/**
	 * The companion widget: which pages it appears on, and what it is told.
	 *
	 * @var FLOSC_Companion_Mode
	 */
	private $companion_mode;

	/**
	 * The full-page chat app, and the flow's own legal pages.
	 *
	 * @var FLOSC_Full_Page_Mode
	 */
	private $full_page_mode;

	/**
	 * FLOSC's own login, registration and auth tokens.
	 *
	 * @var FLOSC_First_Party_Authentication
	 */
	private $first_party_auth;

	/**
	 * Every email FLOSC sends, and the follow-up series.
	 *
	 * @var FLOSC_Email
	 */
	...
	/**
	 * Offers, coupons, payment intents and purchase completion.
	 *
	 * @var FLOSC_Checkout_REST
	 */
	private $checkout_rest;

	/**
	 * Token wallets, for visitors, guests and members alike.
	 *
	 * @var FLOSC_Token_Ledger
	 */
	private $token_ledger;

	/**
	 * Chat sessions over REST.
	 *
	 * @var FLOSC_Session_REST
	 */
	 ...
	 *
	 * @var FLOSC_AI_Chat_Dispatch
	 */
	private $ai_chat_dispatch;

	/**
	 * The speech-to-text dispatcher.
	 *
	 * @var FLOSC_STT_Dispatch
	 */
	private $stt_dispatch;

	/**
	 * Discovers the quiz types under includes/quiz-types/.
	 *
	 * @var FLOSC_Quiz_Type_Factory
	 */
	 ...
	 *
	 * @var FLOSC_Session_Manager
	 */
	private $session_manager;

	/**
	 * Scores a recording against its target pronunciation.
	 *
	 * @var FLOSC_Pronunciation_Analyzer
	 */
	private $pronunciation_analyzer;

	/**
	 * Offers, providers and entitlements. Loaded separately from the rest.
	 *
	 * @var FLOSC_Sale_Manager
	 */
	...
	/**
	 * Decides which indexed content a given user may be shown.
	 *
	 * @var FLOSC_User_Access_Manager
	 */
	private $user_access_manager;

	/**
	 * Strips from retrieved content whatever the reader is not entitled to.
	 *
	 * @var FLOSC_Content_Filter
	 */
	private $content_filter;

	/**
	 * Retrieval over the site content index, for grounded answers.
	 *
	 * @var FLOSC_RAG_Manager
	 */
	private $rag_manager;

	/**
	 * Picks and records the one free lesson a guest is offered.
	 *
	 * @var FLOSC_Free_Content_Item_Manager
	 */
	private $free_lesson_manager;

	/**
	 * Member level checks. Content protection reads this.
	 *
	 * @var FLOSC_Member_Access
	 */
	private $member_access;

	/**
	 * The SSO providers and the OAuth2 round trip.
	 *
	 * @var FLOSC_SSO_Manager
	 */
	private $sso_manager;

	/**
	 * Lesson lookup and delivery.
	 *
	 * @var FLOSC_Lesson_Manager
	 */
	...
	/**
	 * Refuse a password login for an account whose email is not verified yet.
	 *
	...
	/**
	 * Check a FLOSC auth token and say whose it is.
	 *
	...
	/**
	 * Clear the FLOSC auth cookie and the entry-flow recall cookie.
	 *
	...
	/**
	 * The sender identity and product wording a guest email should use.
	 *
	...
	/**
	 * The Reply-To header for this flow's mail.
	 *
	 * Delegates to FLOSC_Email.
	 *
	 * @param string $flow_id Flow whose identity to read; '' for the current one.
	...
	/**
	 * Record a newsletter opt-in and send the welcome.
	 *
	 * Delegates to FLOSC_Email.
	 *
	 * @param int    $user_id The subscriber.
	...
	/**
	 * The email body a flow starts with before the operator edits it.
	 *
	 * Delegates to FLOSC_Email.
	 *
	...
	/**
	 * REST: the offers this flow has configured.
	 *
	 * Delegates to FLOSC_Checkout_REST.
	 *
	 * @param WP_REST_Request $request The request.
	 * @return WP_REST_Response|WP_Error The offer list.
	 */
	...
	/**
	 * An offer's list price, before any coupon.
	 *
	 * Delegates to FLOSC_Checkout_REST.
	 *
	...
	/**
	 * An offer's monthly and yearly subscription prices, before any coupon.
	 *
	 * Delegates to FLOSC_Checkout_REST.
	 *
	 * @param array $offer The offer row.
	...
	/**
	 * Whether checkout should treat this offer as a subscription.
	 *
	...
	public function paypal_prepare_subscription( $request ) {
		...
		if ( ! is_array( $plans ) || empty( $plans['monthly_plan_id'] ) || empty( $plans['yearly_plan_id'] ) ) {
			$plans = get_option( 'flosc_paypal_plans', array() );
		}
		if ( ! is_array( $plans ) ) {
			$plans = array();
		}
		...
		if ( ! function_exists( 'flosc_paypal_purchase_intent_create' ) ) {
			return new WP_Error( 'intent_unavailable', __( 'Purchase intent unavailable', 'flosc' ), array( 'status' => 500 ) );
		}
	...
	/**
	 * REST: what the current user is entitled to in this flow.
	 *
	 * Delegates to FLOSC_Checkout_REST.
	 *
	 * @param WP_REST_Request $request The request.
	 * @return WP_REST_Response|WP_Error The access description.
	 */
	...
	/**
	 * Credit the tokens a purchase buys.
	 *
	...
	/**
	 * Credit the tokens a subscription renewal buys.
	 *
	 * Delegates to FLOSC_Token_Ledger.
	 *
	 ...
	 * @return WP_REST_Response The balance, or null when there is no wallet.
	 */
	...
	/**
	 * REST: the current user's chat sessions.
	 *
	 * Delegates to FLOSC_Session_REST.
	 *
	 * @param WP_REST_Request $request The request.
	 * @return WP_REST_Response|WP_Error The session list.
	 */
	...
	/**
	 * REST: one chat session and its messages.
	 *
	 * Delegates to FLOSC_Session_REST.
	 *
	 * @param WP_REST_Request $request The request, naming the session.
	 * @return WP_REST_Response|WP_Error The session.
	 */
	...
	/**
	 * REST: start a new chat session.
	 *
	 * Delegates to FLOSC_Session_REST. Subject to the per-state session limits.
	 *
	 * @param WP_REST_Request $request The request.
	 * @return WP_REST_Response|WP_Error The new session.
	 */
	...
	private function dispatch_remote_playback_conversion( $session_id, $targets ) {
		...
		$response = flosc_safe_remote_request(
			'POST',
	...
	private function init_hooks() {
		...
		add_filter( 'determine_current_user', array( $this, 'authenticate_flosc_token' ), 20 );
		...
		add_filter( 'rest_authentication_errors', array( $this, 'allow_flosc_token_auth' ), 99 );
		...
		add_action( 'wp_logout', array( $this, 'clear_flosc_auth_token' ), 10, 1 );

		// A password reset ends every session that was opened under the old
		// one. FLOSC tokens are sessions, so they end with it.
		add_action( 'after_password_reset', array( $this, 'revoke_flosc_auth_tokens_for_user' ), 10, 1 );
		add_action( 'profile_update', array( $this, 'revoke_flosc_auth_tokens_on_password_change' ), 10, 2 );
		...
		add_action( 'init', array( $this, 'handle_login_token' ), 0 );

		// v1.1.9: Custom domain mapping - check early before WP routing.
		add_action( 'init', array( $this, 'handle_custom_domain' ), 1 );
		...
		add_filter( 'update_footer', array( $this, 'relabel_admin_footer' ), 20 );
		add_filter( 'admin_footer_text', array( $this, 'relabel_admin_footer_left' ) );

		// v8.0.5: Show user audio files on WP admin user profile page.
		add_action( 'edit_user_profile', array( $this, 'render_admin_user_audio_section' ) );
		add_action( 'show_user_profile', array( $this, 'render_admin_user_audio_section' ) );
		// Profile reminder for email-registered users who haven't set nickname/password yet.
		add_action( 'show_user_profile', array( $this, 'render_credential_setup_reminder' ) );
		add_filter( 'manage_users_columns', array( $this, 'flosc_add_users_columns' ) );
		add_filter( 'manage_users_custom_column', array( $this, 'flosc_render_users_custom_column' ), 10, 3 );
		add_action( 'wp_ajax_flosc_serve_user_audio', array( $this, 'ajax_serve_user_audio' ) );
		add_action( 'wp_ajax_nopriv_flosc_serve_user_audio', array( $this, 'ajax_serve_user_audio' ) );

		// Auto-flush permalinks when slug changes.
		add_action( 'update_option_flosc_app_slug', array( $this, 'handle_slug_change' ), 10, 2 );

		// v1.2.9: New flush permalinks handler with Michel timestamp.
		add_action( 'admin_post_flosc_flush_permalinks_v129', array( $this, 'handle_flush_permalinks_v129' ) );
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_companion' ) );

		// Register shortcodes (v9.2.0).
		add_shortcode( 'flosc_visitor_only', array( $this, 'shortcode_visitor_only' ) );
		add_shortcode( 'flosc_member_only', array( $this, 'shortcode_member_only' ) );
		add_shortcode( 'flosc_contact_form_01', array( $this, 'shortcode_contact_form_01' ) );
		add_shortcode( 'flosc-contact-form-01', array( $this, 'shortcode_contact_form_01' ) );
		...
		add_action( 'flosc_cleanup_visitor_audio', array( $this, 'cleanup_expired_visitor_audio' ) );
		...
		if ( ! get_option( 'flosc_da1_explicit_catalogs_v800' ) ) {
			$flosc_da1_assign = get_option( 'flosc_da1_flow_catalogs', array() );
			if ( ! is_array( $flosc_da1_assign ) ) {
				$flosc_da1_assign = array();
			}
				...
				if ( is_array( $flosc_da1_flow_files ) ) {
					foreach ( $flosc_da1_flow_files as $flosc_da1_flow_path ) {
						...
						if ( ! isset( $flosc_da1_assign[ $flosc_da1_flow_name ] ) ) {
							$flosc_da1_assign[ $flosc_da1_flow_name ] = array( 'default' );
						}
					}
		...
		if ( ! get_option( 'flosc_da1_assignments_pruned_v800' ) ) {
			$flosc_da1_stored = get_option( 'flosc_da1_flow_catalogs', array() );
		...
		if ( ! get_option( 'flosc_menus_upgrade_btn_v800' ) ) {
			...
			$_pb = get_option( 'flosc_profile_bar', array() );
			if ( ! is_array( $_pb ) ) {
				$_pb = array();
			}
		...
		add_action( 'flosc_member_access_granted', array( $this, 'dispatch_member_welcome_email' ), 10, 2 );
		// G→M additive token grant (remaining + member_token_grant), once per flow.
		add_action( 'flosc_member_access_granted', array( $this, 'apply_member_token_grant_on_access' ), 15, 2 );
		...
		add_action( 'user_register', array( $this, 'flosc_mark_journey_account_created' ), 20, 1 );
		add_action( 'flosc_member_access_granted', array( $this, 'flosc_mark_journey_member_granted' ), 20, 2 );
		// Newsletter opt-in profile checkbox (optional lead-gen).
		add_action( 'show_user_profile', array( $this, 'render_newsletter_profile_field' ) );
		add_action( 'edit_user_profile', array( $this, 'render_newsletter_profile_field' ) );
		add_action( 'personal_options_update', array( $this, 'save_newsletter_profile_field' ) );
		add_action( 'edit_user_profile_update', array( $this, 'save_newsletter_profile_field' ) );
		add_action( 'show_user_profile', array( $this, 'render_user_sticky_profile_field' ) );
		add_action( 'edit_user_profile', array( $this, 'render_user_sticky_profile_field' ) );
		add_action( 'personal_options_update', array( $this, 'save_user_sticky_profile_field' ) );
		add_action( 'edit_user_profile_update', array( $this, 'save_user_sticky_profile_field' ) );
		add_action( 'flosc_guest_followup_cron', array( $this, 'run_guest_followup_emails' ) );
		...
		add_action( 'admin_post_flosc_set_default_flow', array( $this, 'handle_set_default_flow' ) );
		...
		add_action( 'admin_init', array( $this, 'maybe_redirect_flosc_admin_shortcuts' ), 1 );

		// Fix 6: Lesson catalog auto-regeneration on post save + manual admin-post handler.
		add_action( 'save_post', array( $this, 'maybe_regenerate_lesson_catalog' ), 20, 2 );
		...
		add_action( 'wp_ajax_flosc_protect_category', array( $this, 'ajax_protect_category' ) );
		add_action( 'wp_ajax_flosc_unprotect_category', array( $this, 'ajax_unprotect_category' ) );

		// v1.5.0: SSO connection test AJAX (inline diagnostics — no popups).
		add_action( 'wp_ajax_flosc_test_sso_connection', array( $this, 'ajax_test_sso_connection' ) );
		...
		add_action( 'admin_post_flosc_guest_request_approve', array( $this, 'handle_guest_request_approve' ) );
		add_action( 'admin_post_flosc_guest_request_approve_send', array( $this, 'handle_guest_request_approve_send' ) );
		add_action( 'admin_post_flosc_guest_request_deny_block', array( $this, 'handle_guest_request_deny_block' ) );
		add_action( 'admin_post_flosc_guest_request_delete', array( $this, 'handle_guest_request_delete' ) );
		...
		add_action( 'wp_ajax_flosc_clear_chat_logs', array( $this, 'ajax_flosc_clear_chat_logs' ) );

		// v1.9.5: Rate a chat log entry (-10 to +10).
		add_action( 'wp_ajax_flosc_rate_log', array( $this, 'ajax_flosc_rate_log' ) );
		add_action( 'wp_ajax_flosc_delete_chat_session', array( $this, 'ajax_flosc_delete_chat_session' ) );
		add_action( 'wp_ajax_flosc_manage_chat_sessions', array( $this, 'ajax_flosc_manage_chat_sessions' ) );
		// v8.0.0: Admin joins a conversation (posts a human, pale-green "(admin)" message).
		add_action( 'wp_ajax_flosc_admin_join', array( $this, 'ajax_flosc_admin_join' ) );
		add_action( 'wp_ajax_flosc_admin_assign_tokens', array( $this, 'ajax_flosc_admin_assign_tokens' ) );
		add_action( 'admin_post_flosc_download_chat_tsv', array( $this, 'handle_download_chat_tsv' ) );

		// v8.0.0: PayPal connection test AJAX.
		add_action( 'wp_ajax_flosc_test_paypal', array( $this, 'ajax_test_paypal' ) );
	...
	/**
	 * The speech-to-text dispatcher, which routes a recording to AssemblyAI, Whisper or a custom endpoint.
	 *
	 * @return FLOSC_STT_Dispatch The collaborator.
	 */
	public function stt() {
	 ...
	 *
	 * @return FLOSC_Session_Manager The collaborator.
	 */
	public function sessions() {
		return $this->session_manager; }
	/**
	 * The pronunciation analyzer used by the audio quiz.
	 *
	 * @return FLOSC_Pronunciation_Analyzer The collaborator.
	 */
	public function analyzer() {
		return $this->pronunciation_analyzer; }
	/**
	 * The sale manager, which owns offers, providers and entitlements.
	 *
	 * @return FLOSC_Sale_Manager The collaborator.
	 */
	 ...
	 * The lesson manager.
	 *
	 * @return FLOSC_Lesson_Manager The collaborator.
	 */
	public function lessons() {
		return $this->lesson_manager; }
	/**
	 * The member access checker, which content protection reads.
	 *
	 * @return FLOSC_Member_Access The collaborator.
	 */
	...
	/**
	 * Whether this client is still within the rate limit for an endpoint.
	 *
	 * Delegates to FLOSC_Request_Guard. Counted per client per endpoint.
	 *
	...
	/**
	 * Sign a value so a cookie carrying it cannot be edited by the browser.
	 *
	...
	/**
	 * Sign a value and send it as a cookie.
	 *
	 * Delegates to FLOSC_Request_Guard.
	 *
...
The Team',
	 ...
	 *
	 * @param int   $user_id    The user.
	 * @param array $score_data The result, carrying at least score and quiz_id.
	 */
	...
	public function add_rewrite_rules() {
		...
		$files = flosc_config_glob( array( '*_ivr.md', 'ivr*.md' ) );
		if ( ! empty( $files ) ) {
			foreach ( $files as $file ) {
				...
				$flow_settings = get_option( $settings_key, array() );
	...
	public function backfill_flow_defaults() {
		...
		foreach ( $files as $file ) {
			...
			if ( isset( $settings['status'] ) && ! in_array( $settings['status'], array( 'active', 'draft' ), true ) ) {
	...
	public function generate_lesson_catalog() {
		...
		if ( empty( $write_paths ) ) {
			...
			$write_paths = array( $catalog_dir . 'lesson_catalog.md' );
		}
	...
	public function ajax_accuracy_test_message() {
		...
		if ( ! class_exists( 'FLOSC_Chatpack' ) || ! $this->ai_chat_dispatch ) {
			wp_send_json_error( array( 'message' => 'Chatpack or AI dispatch not available.' ) );
		}
	...
	public function ajax_protect_category() {
		...
		wp_send_json_success( array( 'message' => 'Category protected' ) );
	}
	...
	public function ajax_unprotect_category() {
		...
		wp_send_json_success( array( 'message' => 'Protection removed' ) );
	}
	...
	public function ajax_test_sso_connection() {
		...
		if ( ! in_array( $provider_id, array( 'google', 'facebook', 'apple', 'microsoft', 'linkedin' ), true ) ) {
		...
		$cb_result = flosc_safe_remote_request(
			'GET',
			$callback_url,
			array(
				'timeout'     => 10,
	...
	public function flosc_add_post_visibility_meta_box() {
		...
		add_meta_box(
			...
			array( $this, 'flosc_render_post_visibility_meta_box' ),
			'post',
			'side',
			'high'
		);
	...
	public function flosc_save_post_visibility_meta( $post_id ) {
		...
		$valid_modes = array( 'protected', 'title_excerpt', 'title_readmore', 'full' );
		...
		foreach ( array(
			'flosc_vgm'   => array( '_flosc_vgm', array( 'visitor', 'guest', 'member' ) ),
			'flosc_depth' => array( '_flosc_depth', array( 'title', 'excerpt', 'readmore', 'full' ) ),
	...
	private function pull_pending_session_from_do( $user_id ) {
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC: pull_pending_session_from_do — user={$user_id}, session={$session_id}" );
			}
		}
	...
	private function enqueue_flosc_quiz_ui_styles() {
		...
		wp_register_style( 'flosc-quiz-ui', false, array(), FLOSC_VERSION );
	 ...
	 *
	 * @param int   $user_id           Whose result this is.
	 * @param array $quiz_data         The quiz payload for one session.
	...
	private function render_session_result_card( $user_id, $quiz_data, $is_guest_user, $profile_completed, $can_play_audio = false ) {
		...
		$ranked_phonemes = $quiz_data['ranked_phonemes'] ?? array();
		$phrase_results  = $quiz_data['phrase_results'] ?? array();
	...
	/**
	 * Print one of the flow's legal pages and end the response.
	 *
	...
	public function get_floscflow_identity() {
		...
		if ( $flow ) {
			$id = is_array( $flow['identity'] ?? null ) ? $flow['identity'] : array();
	...
	public function build_ai_context( $frontend_context = array() ) {
		...
		if ( is_user_logged_in() ) {
			...
			if ( $bridge_data ) {
				...
				$context['bridge_correct_count']   = count( $bridge_data['correct_items'] ?? array() );
				$context['bridge_incorrect_count'] = count( $bridge_data['incorrect_items'] ?? array() );
	...
	/**
	 * The catalog rows a flow carries, filtered to what this access level may see.
	 *
	...
	/**
	 * Parse a DA1 catalog from its tab-separated form.
	 *
	 * Delegates to FLOSC_DA1_Catalogs.
	 *
	...
	/**
	 * Cut text to a length, without cutting a word in half.
	 *
	 * Delegates to FLOSC_DA1_Catalogs.
	 *
	...
	/**
	 * Build an answer to stand in for a provider reply that hedged.
	 *
...
- DO be warm and encouraging";
	...
	private function check_admin_introspection( $message, $current_ivr_file = '' ) {
		...
		$triggers = array(
			'files'     => array( 'what files', 'which files', 'ivr files', 'configuration files', 'config files', 'available files' ),
			'offers'    => array( 'what offers', 'which offers', 'available offers', 'configured offers', 'show offers', 'list offers' ),
			'system'    => array( 'about yourself', 'tell me about you', 'who are you', 'system info', 'system status', 'debug info', 'flosc status' ),
			'flows'     => array( 'what flows', 'which flows', 'available flows', 'configured flows', 'show flows', 'list flows' ),
			'providers' => array( 'payment providers', 'which providers', 'available providers', 'payment methods' ),
			'quizzes'   => array( 'what quizzes', 'which quizzes', 'available quizzes', 'configured quizzes', 'quiz types' ),
			'current'   => array( 'current config', 'current ivr', 'current flow', 'what am i using', 'which ivr' ),
			'status'    => array( 'user status', 'my status', 'my account', 'my role', 'am i admin', 'my access', 'my user', 'account status', 'my profile', 'what role', 'my permissions' ),
			'help'      => array( 'admin help', 'admin commands', 'what can i ask', 'introspection help' ),
		);
	...
	private function get_introspection_user_status() {
		...
		if ( $bridge_data ) {
			...
			$correct   = $bridge_data['correct_items'] ?? array();
			$incorrect = $bridge_data['incorrect_items'] ?? array();
...
\"As a member you have full access. For that topic, start with the matching lesson — want me to open it?\"",
	 ...
	 * @since 1.0.3
	 *
	 * @return WP_REST_Response The current bridge state.
	 */
	 ...
	 * @since 9.3.2
	 *
	 * @param WP_REST_Request $request Expects the quiz id and the score.
	...
	public function store_quiz_result( $request ) {
		...
		if ( is_user_logged_in() ) {
			...
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC v1.0.7: Quiz stored for user {$user_id}, score {$score}%, fired flosc_quiz_completed" );
			}
		}
	...
	private function find_ivr_response( $phase, $user_message, $context, $ivr_config ) {
		...
		$phase_message_names = $ivr_config['phases'][ $phase ] ?? array();

		// Build phase messages array from names.
		$phase_messages = array();
	 ...
	 * @since 1.0.8
	 *
	 * @param array  $messages     The messages to search.
	 * @param string $user_message What the visitor said.
	 * @param array  $context      Request context, for evaluating message conditions.
	...
	private function search_ivr_match( $messages, $user_message, $context, $only_always = false ) {
		...
		foreach ( $messages as $msg ) {
			...
			$matchable_types = array( 'suggested_user_autoprompt', 'offer' );
			...
			if ( ! empty( $msg['content'] ) ) {
				return array(
					...
					'user_autoprompts' => $msg['user_autoprompts'] ?? array(),
		...
		$stop_words       = array( 'i', 'me', 'my', 'the', 'a', 'an', 'is', 'are', 'was', 'do', 'does', 'did', 'can', 'to', 'for', 'of', 'in', 'on', 'it', 'and', 'or', 'but', 'not', 'this', 'that', 'with', 'have', 'has', 'what', 'how', 'please', 'want', 'would', 'like', 'just', 'about' );
		...
		if ( $best_match ) {
			return array(
				...
				'user_autoprompts' => $best_match['user_autoprompts'] ?? array(),
	...
	private function find_message_by_name( $message_name, $phase, $ivr_config ) {
		...
		$phase_messages = $ivr_config[ $phase ] ?? array();
	...
	private function build_quiz_fallback_response( $message, $eval_context ) {
		...
		if ( empty( $incorrect ) ) {
			$incorrect = $eval_context['incorrect_items'] ?? $eval_context['incorrectItems'] ?? array();
		}
	...
	public function handle_ai_query( $request ) {
		...
		if ( empty( $message ) ) {
			return new WP_Error( 'empty_message', __( 'Message is required', 'flosc' ), array( 'status' => 400 ) );
		}
	...
	public function handle_process_audio( $request ) {
		...
		if ( empty( $files['audio'] ) ) {
			return new WP_Error( 'no_audio', __( 'No audio file provided', 'flosc' ), array( 'status' => 400 ) );
		}
	...
	public function handle_process_quiz( $request ) {
		...
		if ( empty( $input ) ) {
			return new WP_Error( 'no_input', __( 'No input provided', 'flosc' ), array( 'status' => 400 ) );
		}

		$quiz_type = FLOSC_Quiz_Registry::get_quiz( $quiz_type_id );

		if ( ! $quiz_type ) {
			return new WP_Error( 'invalid_quiz_type', __( 'Quiz type not found', 'flosc' ), array( 'status' => 404 ) );
		}
		...
		if ( is_user_logged_in() ) {
			...
			$quiz_result = array(
				...
				'correct'        => $analysis['correct'] ?? array(),
				'incorrect'      => $analysis['incorrect'] ?? array(),
				'missed'         => $analysis['incorrect'] ?? array(),
			);
			...
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC v1.0.7: Quiz completed for user {$user_id} with score {$analysis['score']}%" );
			}
		}
	...
	public function flosc_flow_quiz_ids( $stem = '' ) {
		...
		$enabled = $settings['enabled_quizzes'] ?? $flow['enabled_quizzes'] ?? array();
		if ( is_array( $enabled ) ) {
		...
		foreach ( array( 'default_audio_quiz_id', 'default_text_quiz_id', 'quiz_type' ) as $key ) {
			$v = '';
	...
	public function handle_client_chat_log( $request ) {
		if ( ! class_exists( 'FLOSC_Chat_Logger' ) ) {
			return new WP_Error( 'no_logger', __( 'Chat logger unavailable', 'flosc' ), array( 'status' => 500 ) );
		}
	 ...
	 *
	 * @param WP_REST_Request $request Expects the code.
	 * @return WP_REST_Response|WP_Error What the code granted, or an error when it is not valid here.
	 */
	public function handle_redeem_access_code( $request ) {
		...
		if ( empty( $code ) ) {
			return new WP_Error( 'missing_code', __( 'No access code provided', 'flosc' ), array( 'status' => 400 ) );
		}

		$code_norm   = strtoupper( trim( (string) $code ) );
		$option_key  = 'flosc_flow_' . sanitize_key( $flow_id );
		$flow_option = get_option( $option_key, array() );
		...
		$user    = get_userdata( $user_id );
		if ( ! $user ) {
			return new WP_Error( 'no_user', __( 'User not found', 'flosc' ), array( 'status' => 401 ) );
		}
		if ( ! empty( $flow_id ) ) {
	 ...
	 * @since 8.0.7
	 *
	 * @param WP_REST_Request $request Expects the temp_id the browser held in localStorage.
	 * @return WP_REST_Response|WP_Error The full assessment.
	 */
	public function handle_score_pending_audio( $request ) {
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "[FLOSC v8.0.7] score-pending-audio called for user {$user_id}" );
		}
		...
		if ( $existing && ! empty( $existing['phrase_results'] ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "[FLOSC v8.0.7] User {$user_id} already has scored data, returning it" );
			}
			return new WP_REST_Response(
				array(
					'success'        => true,
		...
		if ( ! $temp_id || ! preg_match( '/^\d{4}-\d{2}m-\d{2}d-\d{2}h-\d{2}m-\d{2}s-[0-9a-f]{5}$/', $temp_id ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "[FLOSC v8.0.7] Invalid temp_id: {$temp_id}" );
			}
			return new WP_REST_Response(
				array(
					'success' => false,
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "[FLOSC v8.0.7] Scoring audio for user {$user_id}, temp_id={$temp_id}" );
		}
		...
		if ( $audio_score ) {
			...
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "[FLOSC v8.0.7] Scoring complete: {$audio_score['score']}%" );
			}
			return new WP_REST_Response(
				array(
					'success'    => true,
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( '[FLOSC v8.0.7] score_visitor_audio returned false' );
		}
		return new WP_REST_Response(
			array(
				'success' => false,
	 ...
	 * @return WP_REST_Response|WP_Error Whether the payload was stashed and whether a link was minted.
	 */
	 ...
	 *
	 * @param WP_REST_Request $request Expects a nickname, and optionally a password.
	 * @return WP_REST_Response|WP_Error The updated profile.
	 */
	...
	private function process_prelogin_data_for_user( $user_id ) {
		...
		if ( ! $score_data || ! isset( $score_data['score'] ) ) {
			...
			if ( $raw && isset( $raw['score'] ) ) {
				...
				if ( FLOSC_DEBUG ) {
					if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
						flosc_log( "FLOSC v3.0.7: Using flosc_quiz_result fallback cookie for user {$user_id}" );
					}
				}
		...
		if ( $score_data && isset( $score_data['score'] ) ) {
			...
			if ( $bridge_manager ) {
				...
				if ( ! $existing ) {
					$bridge_manager->update_flosc_bridge_data( $user_id, array( 'score' => $score ) );
				}
			}
			...
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
					flosc_log( "FLOSC Auth: Transferred pre-login score {$score}% (quiz: {$quiz_id}) for user {$user_id}" );
				}
			}
		}
	}
	 ...
	 *
	 * @param WP_REST_Request $request The request.
	 * @return WP_REST_Response|WP_Error The lesson, and a record that it was delivered.
	 */
	 ...
	 *
	 * @param WP_REST_Request $request The request.
	 * @return WP_REST_Response|WP_Error The monthly and yearly plan ids.
	 */
	public function paypal_get_plans( $request ) {
		...
		$paypal = $this->sale_manager->get_provider( 'paypal' );
		if ( ! $paypal || ! $paypal->is_configured() ) {
			return new WP_Error( 'paypal_not_configured', __( 'PayPal is not configured', 'flosc' ), array( 'status' => 500 ) );
		}
		...
		if ( '' !== $offer_id && '' !== $coupon_code ) {
			...
			if ( ! method_exists( $paypal, 'ensure_plans_for_prices' ) ) {
				return new WP_Error( 'paypal_plans', __( 'PayPal provider cannot create promo plans.', 'flosc' ), array( 'status' => 500 ) );
			}
	...
	private function resolve_paypal_subscription_plan_type( array $sub, $requested_plan_type = '' ) {
		...
		if ( empty( $subscription_plan_id ) ) {
			return new WP_Error( 'missing_plan_id', __( 'PayPal subscription is missing a plan_id.', 'flosc' ), array( 'status' => 400 ) );
		}
		...
		if ( empty( $resolved_plan_type ) ) {
			return new WP_Error( 'unknown_paypal_plan', __( 'PayPal subscription plan is not recognized.', 'flosc' ), array( 'status' => 400 ) );
		}

		if ( ! empty( $requested_plan_type ) && $requested_plan_type !== $resolved_plan_type ) {
			return new WP_Error( 'plan_mismatch', __( 'Requested plan does not match the verified PayPal subscription.', 'flosc' ), array( 'status' => 400 ) );
		}
	...
	public function paypal_activate_subscription( $request ) {
		...
		if ( ! is_array( $sub ) ) {
			return new WP_Error( 'paypal_error', __( 'Invalid PayPal subscription response', 'flosc' ), array( 'status' => 502 ) );
		}
		...
		if ( ! empty( $intent['expires_at'] ) && time() > (int) $intent['expires_at'] ) {
			return new WP_Error( 'intent_expired', __( 'Purchase intent expired. Restart checkout.', 'flosc' ), array( 'status' => 403 ) );
		}
		...
		if ( $user_id <= 0 ) {
			...
			if ( $existing_user ) {
				...
				do_action( 'flosc_user_registered', $user_id, 'paypal_purchase', array( 'flow_id' => $flow_id ) );
				$is_new_user = true;
			}
		}
	 ...
	 * @return WP_REST_Response|WP_Error The PayPal order.
	 */
	public function paypal_create_order( $request ) {
		...
		if ( ! $offer ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( '[FLOSC-PAYPAL] create_order FAIL: offer "' . $offer_id . '" not found' );
			}
			return new WP_Error( 'invalid_offer', 'Offer not found: ' . $offer_id, array( 'status' => 404 ) );
		}
		...
		if ( $amount <= 0 ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( '[FLOSC-PAYPAL] create_order FAIL: no price for offer "' . $offer_id . '"' );
			}
			return new WP_Error( 'no_price', 'No price configured for offer "' . $offer_id . '". Set the price in FLOSC Offers tab.', array( 'status' => 400 ) );
		}
	...
	public function paypal_capture_order( $request ) {
		...
		if ( empty( $order_id ) || empty( $offer_id ) ) {
			return new WP_Error( 'missing_params', __( 'Missing order_id or offer_id', 'flosc' ), array( 'status' => 400 ) );
		}
		...
		if ( $user_id > 0 && $captured_user_id && intval( $captured_user_id ) > 0 && intval( $captured_user_id ) !== $user_id ) {
			...
			return new WP_Error( 'user_mismatch', __( 'Payment does not belong to this user', 'flosc' ), array( 'status' => 403 ) );
		}
		...
		if ( ! $user_id ) {
			...
			if ( $existing_user ) {
				...
				if ( is_wp_error( $created_id ) ) {
					return new WP_Error( 'user_creation_failed', 'Could not create account: ' . $created_id->get_error_message(), array( 'status' => 500 ) );
				}
		...
		if ( $user_id <= 0 ) {
			return new WP_Error( 'no_buyer', __( 'Could not resolve buyer account for this payment.', 'flosc' ), array( 'status' => 500 ) );
		}
	...
	public function handle_session_bootstrap( $request ) {
		...
		$payload = $this->build_app_user_payload(
			$user_id,
			$flow_stem,
			array(
	...
	public function build_app_user_payload( $user_id, $flow_stem = '', $args = array() ) {
		...
		if ( $this->flosc_flow_should_surface_quiz_data( $flow_stem, $quiz_data, $quiz_id ) ) {
			...
			$completed                   = get_user_meta( $user_id, '_flosc_completed_quizzes', true );
			$payload['completedQuizzes'] = is_array( $completed ) ? $completed : array();
		} else {
	...
	/**
	 * REST: the current user's token balance and recent ledger.
	 *
	 * Ten ledger entries, which is what the token panel shows.
	 *
	 * @return WP_REST_Response The balance and the ledger.
	 */
	...
	public function declare_intent( $request ) {
		...
		if ( ! $affiliate || ! $affiliate->is_configured() ) {
			return new WP_Error( 'not_configured', __( 'Affiliate system not configured', 'flosc' ), array( 'status' => 500 ) );
		}
	...
	public function get_intent_offers( $request ) {
		...
		if ( ! isset( $intents[ $intent_id ] ) ) {
			return new WP_Error( 'not_found', __( 'Intent not found', 'flosc' ), array( 'status' => 404 ) );
		}
	 ...
	 * @since 1.1.0
	 *
	 * @param WP_REST_Request $request Optionally names flow_id and ivr_file, which REST requests must, having no URL context to resolve from.
	 * @return WP_REST_Response|WP_Error The applicable messages.
	 */
	public function get_ivr_messages( $request ) {
		...
		$all_messages = $config['messages'] ?? array();
		...
		$is_member = ! empty( $user_context['is_member'] )
			|| ( isset( $user_context['access_level'] ) && in_array( (string) $user_context['access_level'], array( 'member', 'full' ), true ) );
	 ...
	 * @since 3.0.8
	 *
	 * @param WP_REST_Request $request Expects a flow scope, and optionally quiz_only or search.
	 * @return WP_REST_Response|WP_Error The lessons, as full records or as locked stubs.
	 */
	 ...
	 *
	 * @param WP_REST_Request $request Expects the lesson id and a flow scope.
	 * @return WP_REST_Response|WP_Error The lesson with its content, or a locked stub.
	 */
	 ...
	 * @since 9.4.2
	 *
	 * @param WP_REST_Request $request Expects the score.
	 * @return WP_REST_Response|WP_Error Confirmation. The score goes into a signed cookie so it cannot be edited before login.
	 */
	public function store_prelogin_score( $request ) {
		$score_data = array(
			...
			'correct'              => $request->get_param( 'correct' ) ?? array(),
			'incorrect'            => $request->get_param( 'incorrect' ) ?? array(),
			'quiz_type'            => sanitize_text_field( $request->get_param( 'quiz_type' ) ?? '' ),
			'ranked_worst_lessons' => $request->get_param( 'ranked_worst_lessons' ) ?? array(),
			'timestamp'            => time(),
		);
		...
		if ( is_user_logged_in() ) {
			...
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC v3.0.2: /store-score fired flosc_quiz_completed for logged-in user {$user_id}, score {$score}%, quiz {$quiz_id}" );
			}
		}
	 ...
	 * @since 8.0.0
	 *
	 * @param WP_REST_Request $request Expects the phrase recordings and their phrase numbers.
	 * @return WP_REST_Response|WP_Error The temp id the browser holds until they register.
	 */
	public function store_visitor_audio( $request ) {
		...
		if ( $files['audio']['size'] > 2 * 1024 * 1024 ) {
			return new WP_Error( 'too_large', __( 'Audio file exceeds 2MB limit', 'flosc' ), array( 'status' => 400 ) );
		}

		$phrase_num = intval( $request->get_param( 'phrase_num' ) );
		if ( $phrase_num < 1 || $phrase_num > 10 ) {
			return new WP_Error( 'bad_phrase', __( 'Invalid phrase number', 'flosc' ), array( 'status' => 400 ) );
		}
		...
		if ( ! preg_match( '/^\d{4}-\d{2}m-\d{2}d-\d{2}h-\d{2}m-\d{2}s-[0-9a-f]{5}$/', $temp_id ) ) {
			return new WP_Error( 'bad_temp_id', __( 'Invalid session', 'flosc' ), array( 'status' => 400 ) );
		}
		...
		$ext      = in_array( $format, array( 'webm', 'mp4', 'ogg' ), true ) ? $format : 'webm';
		$filename = 'phrase-' . $phrase_num . '.' . $ext;
		$filepath = $temp_dir . '/' . $filename;

		$tmp_audio = $files['audio']['tmp_name'] ?? '';
		if ( empty( $tmp_audio ) || ! is_uploaded_file( $tmp_audio ) ) {
			return new WP_Error( 'write_failed', __( 'Invalid uploaded audio', 'flosc' ), array( 'status' => 400 ) );
		}
		...
		$meta                  = file_exists( $meta_path ) ? json_decode( flosc_fs_get_contents( $meta_path ), true ) : array(
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC v8.0.0: Stored visitor audio phrase-{$phrase_num} in {$temp_id}/ (" . filesize( $filepath ) . ' bytes)' );
			}
		}
	...
	private function store_browser_quiz_data( $user_id, $quiz_data, $temp_id = '' ) {
		...
		if ( ! empty( $quiz_data['phraseResults'] ) && is_array( $quiz_data['phraseResults'] ) ) {
			$score_data['phrase_results'] = array_map(
				function ( $pr ) {
					if ( ! is_array( $pr ) ) {
						return array(
							'phrase' => '',
		...
		if ( $phoneme_map && $score_data['ranked_phonemes'] ) {
			...
			$js_scores = array();
			foreach ( ( $score_data['ranked_worst_lessons'] ?? array() ) as $rwl ) {
		...
		if ( file_exists( $meta_path ) ) {
			...
			if ( is_array( $meta ) && ! empty( $meta['phrases'] ) && is_array( $meta['phrases'] ) ) {
				...
				foreach ( $meta['phrases'] as &$phrase ) {
					...
					$ready = null;
					foreach ( array( 'mp4', 'm4a', 'wav' ) as $ext ) {
						$name = 'phrase-' . $num . '.' . $ext;
	...
	private function pull_session_from_do( $user_id, $session_id ) {
		...
		if ( ! preg_match( '/^\d{4}-\d{2}m-\d{2}d-\d{2}h-\d{2}m-\d{2}s-[0-9a-f]{5}$/', $session_id ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC: pull_session_from_do — invalid session_id: {$session_id}" );
			}
			return false;
		}
		...
		if ( $existing_owner && intval( $existing_owner ) !== intval( $user_id ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
					flosc_log( "FLOSC: pull_session_from_do — blocked cross-user session reuse: session {$session_id}, requested user {$user_id}, existing owner {$existing_owner}" );
				}
			}
			return false;
		}
		...
		if ( is_wp_error( $response ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( 'FLOSC: pull_session_from_do — GET /session failed: ' . $response->get_error_message() );
			}
			return false;
		}
		...
		if ( ! $session_data || empty( $session_data['phrase_results'] ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( 'FLOSC: pull_session_from_do — empty or invalid session data' );
			}
			return false;
		}

		$score           = intval( $session_data['score'] ?? 0 );
		$ranked_phonemes = $session_data['ranked_phonemes'] ?? array();
		...
		foreach ( ( $session_data['phoneme_scores'] ?? array() ) as $ipa => $scores ) {
		...
		$session_phrases = array();
		for ( $n = 1; $n <= $phrase_count; $n++ ) {
			$audio_resp = flosc_safe_remote_request(
				'GET',
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "FLOSC v8.0.0: pull_session_from_do() — user {$user_id}, session {$session_id}, score: {$score}%" );
		}
		return true;
	}
	 ...
	 *
	 * @param WP_REST_Request $request Expects the quiz payload the browser read back out of localStorage.
	 * @return WP_REST_Response|WP_Error Confirmation.
	 */
	public function handle_store_quiz_data( $request ) {
		$user_id = get_current_user_id();
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "FLOSC store-quiz-data: called. user_id={$user_id}, session_id=" . ( $request->get_param( 'session_id' ) ?? 'null' ) );
		}
		if ( ! $user_id ) {
			return new WP_REST_Response(
				array(
		...
		if ( is_array( $existing ) && ! empty( $existing['phrase_results'] ) && ! $session_id ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC store-quiz-data: user {$user_id} has existing data, no new session_id — returning cached" );
			}
			return new WP_REST_Response(
				array(
					'success'        => true,
	...
	public function score_visitor_audio( $user_id, $temp_id ) {
		...
		if ( ! file_exists( $meta_path ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC v8.0.0: No metadata.json for temp_id {$temp_id}" );
			}
			return false;
		}
		...
		foreach ( $meta['phrases'] as $phrase_info ) {
			...
			if ( is_wp_error( $response ) ) {
				if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
					flosc_log( "FLOSC v8.0.0: API error for phrase {$phrase_info['num']}: " . $response->get_error_message() );
				}
				continue;
			}
		...
		if ( empty( $all_results ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC v8.0.0: No successful API responses for temp_id {$temp_id}" );
			}
			return false;
		}
		...
		foreach ( $all_results as $r ) {
			$words_data = $r['data']['words'] ?? array( array( 'phonemes' => $r['data']['phonemes'] ?? array() ) );
		...
		foreach ( $mapped_worst as $p ) {
			...
			$lessons = is_array( $val ) ? $val : array( $val );
			foreach ( $lessons as $l ) {
				$incorrect[] = (int) $l;
			}
		}
		...
		if ( ! $this->write_json_atomic( $user_meta_path, $user_meta ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC v8.0.0: failed to persist scored metadata for session {$temp_id}" );
			}
		}
	...
	public function cleanup_expired_visitor_audio() {
		...
		if ( $cleaned > 0 && FLOSC_DEBUG ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC v8.0.0: Cleaned up {$cleaned} expired visitor audio dirs" );
			}
		}
	}
	...
	public function mark_funnel_complete() {
		...
		if ( ! $user_id ) {
			return new WP_Error( 'not_logged_in', __( 'User must be logged in', 'flosc' ), array( 'status' => 401 ) );
		}
	...
	public function handle_test_ai() {
		...
		try {
			...
			$ai_context    = array( 'phase' => 'freeline' );
	...
	private function probe_provider_models( $provider, $api_key ) {
		...
		return array(
			'models_probed' => true,
			'models'        => isset( $catalog['models'] ) && is_array( $catalog['models'] ) ? $catalog['models'] : array(),
		);
	}
	...
	public function ajax_flosc_rate_log() {
		...
		if ( ! $log_id ) {
			wp_send_json_error( array( 'message' => 'Missing log_id' ) );
		}
	...
	public function ajax_flosc_manage_chat_sessions() {
		...
		if ( ! in_array( $operation, array( 'archive', 'restore', 'delete' ), true ) ) {
	...
	public function ajax_flosc_admin_assign_tokens() {
		...
		if ( $session_id <= 0 || $amount <= 0 ) {
			wp_send_json_error( array( 'message' => 'Session id and positive token amount are required.' ) );
		}

		$token_provider = $this->sale_manager->get_provider( 'tokens' );
		if ( ! $token_provider ) {
			wp_send_json_error( array( 'message' => 'Token provider unavailable.' ) );
		}
	 ...
	 * @since 8.0.0
	 *
	 * @param WP_REST_Request $request Expects the conversation token and a cursor.
	 * @return WP_REST_Response|WP_Error Admin messages newer than the cursor.
	 */
	public function handle_admin_messages_token( $request ) {
		...
		if ( $session_id <= 0 ) {
			return new WP_Error( 'invalid_session_id', __( 'Invalid session id.', 'flosc' ), array( 'status' => 400 ) );
		}

		if ( ! $this->current_request_owns_chat_session( $session_id ) ) {
			return new WP_Error( 'flosc_session_ownership_failed', __( 'Session ownership check failed.', 'flosc' ), array( 'status' => 403 ) );
		}
	 ...
	 * @since 8.0.0
	 *
	 * @param WP_REST_Request $request Expects the conversation and a cursor.
	 * @return WP_REST_Response|WP_Error Admin messages newer than the cursor.
	 */
	 ...
	 * @since 1.9.0
	 *
	 * @param WP_REST_Request $request Expects the flagged response and the admin's note.
	 * @return WP_REST_Response|WP_Error Confirmation. Stored in the flow settings under ai_feedback.
	 */
	public function handle_save_feedback( $request ) {
		...
		if ( empty( $user_message ) || empty( $bad_response ) || empty( $admin_note ) ) {
			return new WP_Error( 'missing_fields', 'user_message, bad_response, and admin_note are required', array( 'status' => 400 ) );
		}
		...
		if ( empty( $flow_id ) ) {
			return new WP_Error( 'no_flow', 'Could not determine flow', array( 'status' => 400 ) );
		}

		$settings_key   = 'flosc_flow_' . sanitize_key( pathinfo( basename( $flow_id ), PATHINFO_FILENAME ) );
		$flow_settings  = get_option( $settings_key, array() );
		$feedback_items = $flow_settings['ai_feedback'] ?? array();
	 ...
	 *
	 * @param WP_REST_Request $request The request.
	 * @return WP_REST_Response|WP_Error Every feedback entry recorded on this flow.
	 */
	 ...
	 * @since 1.9.0
	 *
	 * @param WP_REST_Request $request Expects the feedback id.
	 * @return WP_REST_Response|WP_Error Confirmation.
	 */
	public function handle_delete_feedback( $request ) {
		...
		$feedback_items = array_values(
			array_filter(
				$feedback_items,
				function ( $c ) use ( $feedback_id ) {
		...
		if ( count( $feedback_items ) === $original_count ) {
			return new WP_Error( 'not_found', 'Feedback not found', array( 'status' => 404 ) );
		}
	 ...
	 * @since 1.9.0
	 *
	 * @param WP_REST_Request $request Expects the response being reinforced and the admin's note.
	 * @return WP_REST_Response|WP_Error Confirmation.
	 */
	public function handle_save_praise( $request ) {
		...
		if ( empty( $user_message ) || empty( $good_response ) || empty( $admin_note ) ) {
			return new WP_Error( 'missing_fields', 'user_message, good_response, and admin_note are required', array( 'status' => 400 ) );
		}
		...
		$settings_key  = 'flosc_flow_' . sanitize_key( pathinfo( basename( $flow_id ), PATHINFO_FILENAME ) );
		$flow_settings = get_option( $settings_key, array() );
		$praises       = $flow_settings['ai_praises'] ?? array();
	 ...
	 * @since 1.9.0
	 *
	 * @param WP_REST_Request $request Expects the praise id.
	 * @return WP_REST_Response|WP_Error Confirmation.
	 */
	public function handle_delete_praise( $request ) {
		...
		$praises        = array_values(
			array_filter(
				$praises,
				function ( $p ) use ( $praise_id ) {
		...
		if ( count( $praises ) === $original_count ) {
			return new WP_Error( 'not_found', 'Praise not found', array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param WP_REST_Request $request Expects the phase and the request context.
	 * @return WP_REST_Response|WP_Error The messages that apply to this user right now.
	 */
	...
	public function render_admin_user_audio_section( $user ) {
		...
		$quiz_data = get_user_meta( $user_id, '_flosc_last_quiz_data', true );
		$sess_id   = $this->resolve_quiz_session_id( is_array( $quiz_data ) ? $quiz_data : array() );
		$meta_path = '';
		...
		if ( $quiz_data ) {
			...
			$ranked      = $quiz_data['ranked_phonemes'] ?? array();
	...
	public function ajax_serve_user_audio() {
		...
		if ( $is_download && $session_id ) {
			...
			$ext           = in_array( $ext, array( 'mp4', 'm4a', 'webm', 'ogg', 'wav' ), true ) ? $ext : 'bin';
	...
	public function buddyboss_quiz_tab_screen() {
		...
		add_action( 'bp_template_content', array( $this, 'render_buddyboss_quiz_tab' ) );
	...
	public function render_buddyboss_quiz_tab() {
		...
		if ( ! $profile_completed && $is_guest_user ) {
			...
			if ( $days_note ) {
				echo '<p class="flosc-guest-warning-copy flosc-guest-warning-copy--tight">' . wp_kses_post( $days_note ) . '</p>';
			}
			echo '</div>';
		...
		if ( ! empty( $quiz_attempts ) ) {
			...
			foreach ( $quiz_attempts as $idx => $attempt ) {
				...
				if ( empty( $attempt_quiz_data ) || ! is_array( $attempt_quiz_data ) ) {
					...
					$attempt_quiz_data = is_array( $quiz_data ) ? $quiz_data : array();
				}
		...
		if ( $phrase_results && empty( $quiz_attempts ) ) {
			$word_ipa   = $quiz_data['word_ipa'] ?? array();
	 ...
	 *
	 * @param int    $user_id        Whose recording this is.
	 * @param string $session_id     The session.
	 ...
	 *
	 * @param int    $user_id    Whose recording the URL points at.
	 * @param string $session_id The session.
	 ...
	 *
	 * @param int    $user_id    Whose recording is being requested.
	 * @param string $session_id The session.
	 * @param string $file       The file name.
	...
	public function enqueue_assets() {
		...
		if ( $paypal && $paypal->has_client_id() ) {
			...
			if ( $pp_client_id ) {
				...
				wp_enqueue_script( 'paypal-js', $pp_sdk, array(), defined( 'FLOSC_VERSION' ) ? FLOSC_VERSION : '1', true );
	...
	/**
	 * Which companion-enabled flows claim this path or these categories.
	 *
	...
	/**
	 * Category slugs on the current front-end request.
	 *
	...
	/**
	 * Look up an active flow by slug, when hardening a companion URL.
	 *
	 * Delegates to FLOSC_Companion_Mode.
	 *
	...
	/**
	 * Turn the path-pattern textarea into a list of normalised prefixes.
	 *
	...
	/**
	 * The current request path, resolved in a way that is safe on the front end.
	 *
	 * Delegates to FLOSC_Companion_Mode.
	 *
