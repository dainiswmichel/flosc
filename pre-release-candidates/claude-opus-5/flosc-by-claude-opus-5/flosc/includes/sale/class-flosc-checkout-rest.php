...
class FLOSC_Checkout_Rest {
	...
	public function get_offer_content( $request ) {
		...
		switch ( $source ) {
			case 'html':
				...
				if ( empty( $file ) ) {
					return new WP_REST_Response( array( 'error' => 'Missing file parameter' ), 400 );
				}
			...
			case 'woo':
				...
				if ( ! $product_id || ! function_exists( 'wc_get_product' ) ) {
					return new WP_REST_Response( array( 'error' => 'WooCommerce not available or invalid product' ), 400 );
				}
				...
				if ( method_exists( $product, 'get_catalog_visibility' )
					...
					return new WP_REST_Response( array( 'error' => 'Product not available' ), 403 );
				}
			...
			case 'post':
				...
				if ( ! FLOSC_Content_Protection::instance()->user_can_access( $post_id )
					...
					return new WP_REST_Response(
						array(
							'error'  => 'Not entitled to this content',
							'locked' => true,
						),
						403
					);
			...
			default:
				return new WP_REST_Response( array( 'error' => 'Invalid source type' ), 400 );
		}
	}
	...
	public function handle_purchase( $request ) {
		...
		if ( ! $user_id ) {
			return new WP_Error( 'not_logged_in', __( 'You must be logged in to make a purchase', 'flosc' ), array( 'status' => 401 ) );
		}
		...
		$payment_data = $request->get_param( 'payment_data' ) ?? array();
	...
	private function flosc_resolve_subscription_coupon_prices( array $offer, $coupon_code = '' ) {
		...
		if ( $monthly <= 0 && $yearly <= 0 ) {
			return new WP_Error( 'no_price', __( 'Coupon reduced subscription to zero. Use Access Code to unlock.', 'flosc' ), array( 'status' => 400 ) );
		}
		return array(
			'monthly'      => $monthly,
			'yearly'       => $yearly,
	...
	public function handle_apply_offer_coupon( $request ) {
		...
		$access_codes = $offer['access_codes'] ?? array();
	...
	public function handle_sandbox_purchase( $request ) {
		...
		$sandbox_offer = array(
			...
			'grants' => array(
				'level'    => $member_level,
				'features' => array( 'all_lessons', 'all_quizzes', 'ai_chat', 'premium_content' ),
			),
		);
	...
	public function create_payment_intent( $request ) {
		...
		if ( ! $stripe || ! $stripe->is_configured() ) {
			return new WP_Error( 'stripe_not_configured', __( 'Stripe is not configured. Add keys under Payments for this flow.', 'flosc' ), array( 'status' => 503 ) );
		}
		...
		$stripe = $this->flosc->sale()->get_provider( 'stripe' );
		if ( ! $stripe || ! $stripe->is_configured() ) {
			return new WP_Error( 'stripe_not_configured', __( 'Stripe is not configured', 'flosc' ), array( 'status' => 500 ) );
		}
		...
		if ( is_numeric( $price_or_amount ) && (int) $price_or_amount <= 0 ) {
			return new WP_Error( 'no_price', 'No price configured for offer "' . $offer_id . '". Set a Stripe Price ID or a price in FLOSC → Offers tab.', array( 'status' => 400 ) );
		}
	...
	public function complete_purchase( $request ) {
		...
		if ( empty( $payment_intent_id ) || empty( $offer_id ) ) {
			return new WP_Error( 'missing_params', __( 'Missing payment_intent_id or offer_id', 'flosc' ), array( 'status' => 400 ) );
		}

		$user_id = get_current_user_id();
		if ( ! $user_id ) {
			return new WP_Error( 'not_logged_in', __( 'You must be logged in to complete a purchase', 'flosc' ), array( 'status' => 401 ) );
		}
	...
	public function handle_webhook( $request ) {
		...
		if ( ! $provider ) {
			return new WP_Error( 'invalid_provider', __( 'Unknown provider', 'flosc' ), array( 'status' => 400 ) );
		}
