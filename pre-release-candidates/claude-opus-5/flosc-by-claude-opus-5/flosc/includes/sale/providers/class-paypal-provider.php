...
class FLOSC_PayPal_Provider extends FLOSC_Payment_Provider {
	...
	public function ensure_webhook_registered( $force_refresh = false ) {
		...
		$matched_id = '';
		$webhooks   = is_array( $list_body['webhooks'] ?? null ) ? $list_body['webhooks'] : array();
		...
		if ( '' === $matched_id ) {
			...
			if ( $create_code >= 200 && $create_code < 300 && ! empty( $create_body['id'] ) ) {
				...
				if ( '' === $matched_id ) {
					...
					return new WP_Error( 'paypal_webhook_create_failed', $msg, array( 'status' => 502 ) );
				}
			}
		}
	 ...
	 * @param mixed $amount_dollars Amount dollars.
	 * @param mixed $currency Currency.
	 * @param int $offer_id Offer ID.
	 * @param string $purchase_uuid Purchase uuid.
	 */
	...
	public function capture_order( $order_id ) {
		...
		$payer                 = is_array( $body['payer'] ?? null ) ? $body['payer'] : array();
		$payment_source_paypal = is_array( $body['payment_source']['paypal'] ?? null ) ? $body['payment_source']['paypal'] : array();
	 ...
	 * @param mixed $interval_unit Interval unit.
	...
	public function get_stored_plans() {
		...
		if ( empty( $plans['monthly_plan_id'] ) || empty( $plans['yearly_plan_id'] ) ) {
			$legacy = get_option( 'flosc_paypal_plans', array() );
			if ( is_array( $legacy )
		...
		if ( function_exists( 'flosc' ) ) {
			...
			if ( is_array( $flow ) ) {
				...
				foreach ( $offers as $offer ) {
					...
					$sub_plans = is_array( $offer['subscription']['plans'] ?? null ) ? $offer['subscription']['plans'] : array();
	...
	public function ensure_plans_for_prices( $monthly_price, $yearly_price, $product_name = '' ) {
		...
		if ( $monthly_price <= 0 && $yearly_price <= 0 ) {
			return new WP_Error( 'no_price', __( 'Subscription amounts must be greater than zero.', 'flosc' ), array( 'status' => 400 ) );
		}
		if ( $monthly_price <= 0 ) {
	...
	public function resolve_plan_type_for_id( $plan_id ) {
		...
		$index = get_option( $this->get_plans_option_key() . '_index', array() );
		if ( is_array( $index ) && ! empty( $index[ $plan_id ] ) ) {
			$t = sanitize_key( (string) $index[ $plan_id ] );
			return in_array( $t, array( 'monthly', 'yearly' ), true ) ? $t : '';
		}
		return '';
	}
	...
	public static function collect_transmission_headers_from_request( $request = null ) {
		...
		$server_aliases = array(
			'paypal-transmission-id'   => array( 'HTTP_PAYPAL_TRANSMISSION_ID', 'PAYPAL_TRANSMISSION_ID' ),
			'paypal-transmission-time' => array( 'HTTP_PAYPAL_TRANSMISSION_TIME', 'PAYPAL_TRANSMISSION_TIME' ),
			'paypal-transmission-sig'  => array( 'HTTP_PAYPAL_TRANSMISSION_SIG', 'PAYPAL_TRANSMISSION_SIG' ),
			'paypal-cert-url'          => array( 'HTTP_PAYPAL_CERT_URL', 'PAYPAL_CERT_URL' ),
			'paypal-auth-algo'         => array( 'HTTP_PAYPAL_AUTH_ALGO', 'PAYPAL_AUTH_ALGO' ),
		);
		...
		foreach ( $keys as $key ) {
			...
			if ( $request instanceof WP_REST_Request ) {
				...
				if ( ( null === $value || '' === $value ) && strpos( $key, 'paypal-' ) === 0 ) {
	...
	private function extract_paypal_transmission_headers( array $headers ) {
		...
		$aliases = array(
			'transmission_id'   => array( 'paypal-transmission-id', 'transmission-id', 'transmission_id' ),
			'transmission_time' => array( 'paypal-transmission-time', 'transmission-time', 'transmission_time' ),
			'transmission_sig'  => array( 'paypal-transmission-sig', 'transmission-sig', 'transmission_sig' ),
			'cert_url'          => array( 'paypal-cert-url', 'cert-url', 'cert_url' ),
			'auth_algo'         => array( 'paypal-auth-algo', 'auth-algo', 'auth_algo' ),
		);
	...
	private function begin_webhook_credential_context( array $pack ) {
		$this->runtime_credential_override = array(
