...
class FLOSC_ClickBank_Provider extends FLOSC_Payment_Provider {
	...
	public function handle_webhook( $payload, $headers = array() ) {
		...
		if ( null === $params ) {
			...
			if ( ! is_array( $raw_params ) || empty( $raw_params ) ) {
				...
				return new WP_Error( 'invalid_payload', __( 'Unrecognized ClickBank notification payload', 'flosc' ), array( 'status' => 400 ) );
			}
			$required = array( 'ctransaction', 'ctransreceipt', 'cvendor', 'ccustname', 'ccustemail' );
			foreach ( $required as $field ) {
				if ( empty( $raw_params[ $field ] ) ) {
					/* translators: %s: name of the missing ClickBank webhook field. */
					return new WP_Error( 'missing_field', sprintf( __( 'Missing required field: %s', 'flosc' ), $field ), array( 'status' => 400 ) );
				}
			}
			if ( empty( $raw_params['cverify'] ) ) {
				return new WP_Error( 'missing_signature', __( 'Missing IPN signature', 'flosc' ), array( 'status' => 401 ) );
			}
			if ( ! $this->verify_ipn_signature( $raw_params, $secret_key ) ) {
				return new WP_Error( 'invalid_signature', __( 'Invalid IPN signature', 'flosc' ), array( 'status' => 401 ) );
			}
			$params = $this->sanitize_ipn_params( $raw_params );
		}

		if ( ! is_array( $params ) || empty( $params['ctransreceipt'] ) ) {
			return new WP_Error( 'invalid_payload', __( 'ClickBank notification missing receipt', 'flosc' ), array( 'status' => 400 ) );
		}
		...
		if ( $msg_vendor !== $our_vendor ) {
			return new WP_Error( 'vendor_mismatch', __( 'Vendor does not match', 'flosc' ), array( 'status' => 400 ) );
		}
	...
	private function decrypt_ins_notification( $encrypted_b64, $iv_b64, $secret_key ) {
		...
		// phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode -- ClickBank INS transports the AES ciphertext as base64; strict decoding is required before decryption.
		...
		if ( ! is_array( $order ) ) {
			return new WP_Error( 'ins_json_invalid', __( 'Decrypted ClickBank INS payload is not valid JSON', 'flosc' ), array( 'status' => 400 ) );
		}
	...
	private function map_ins_json_to_params( array $order ) {
		...
		$shipping = is_array( $order['customer']['shipping'] ?? null ) ? $order['customer']['shipping'] : array();
		...
		$item_no    = '';
		$line_items = $order['lineItems'] ?? array();
