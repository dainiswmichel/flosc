...
class FLOSC_Access_Manager {
		...
		return array_values(
			array_filter(
				...
				static function ( $level ) {
					return '' !== $level && false === strpos( $level, 'guest' );
				}
			)
		);
	}
	...
	public function has_feature( $user_id, $feature ) {
		...
		if ( ! is_array( $member_content_features ) ) {
			$member_content_features = array( 'all_lessons', 'full_access' );
		}
	...
	public function revoke_feature( $user_id, $feature ) {
		...
		$access['features'] = array_filter(
			$access['features'],
			function ( $f ) use ( $feature ) {
				return $f !== $feature;
			}
		);
