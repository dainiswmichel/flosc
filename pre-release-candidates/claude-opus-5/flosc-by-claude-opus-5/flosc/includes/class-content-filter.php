...
class flosc_content_filter {
	...
	public function apply_content_filter( $content ) {
		...
		try {
			...
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
					flosc_log( 'FLOSC Content Filter error: ' . $e->getMessage() );
				}
			}
			return $content;
		}
	...
	public function filter_markdown_by_access( $content, $access_level ) {
		...
		switch ( $access_level ) {
			case 'visitor':
				$allowed_sections = array( 'visitor' );
				break;
			case 'guest':
				$allowed_sections = array( 'visitor', 'guest' );
				break;
			case 'member':
				$allowed_sections = array( 'visitor', 'guest', 'member' );
				break;
		}
