...
class FLOSC_RAG_Manager {
	...
	public function execute_tool( $tool_name, $input, $access_level, $category_id = 0 ) {
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC RAG: Executing tool '{$tool_name}' for access level '{$access_level}' in category '{$category_id}'" );
			}
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( 'FLOSC RAG: Input - ' . wp_json_encode( $input ) );
			}
		}
		...
		try {
			...
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( 'FLOSC RAG Error: ' . $e->getMessage() );
			}
