...
class FLOSC_Member_Access {
	...
	public function revoke_member_access( $user_id, $reason = '' ) {
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "FLOSC: Revoked member access for user {$user_id}. Reason: {$reason}" );
		}
	...
	public function get_level_aliases( $level ) {
		 ...
		 */
		$groups = apply_filters( 'flosc_member_level_alias_groups', array() );
		if ( is_array( $groups ) ) {
	...
	public function grant_level( $user_id, $level ) {
		...
		if ( get_role( $sanitized_level ) ) {
			...
			if ( $user ) {
				...
				$guest_aliases = apply_filters( 'flosc_guest_level_slugs_to_clear_on_member_grant', array(), $sanitized_level );
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "FLOSC: Granted {$level} membership to user {$user_id}" );
		}
	...
	public function revoke_level( $user_id, $level, $reason = '' ) {
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "FLOSC: Revoked {$level} membership from user {$user_id}. Reason: {$reason}" );
		}
	...
	public function grant_guest_access( $user_id, $post_id ) {
		...
		if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
			flosc_log( "FLOSC: Granted guest access to post {$post_id} for user {$user_id}" );
		}
