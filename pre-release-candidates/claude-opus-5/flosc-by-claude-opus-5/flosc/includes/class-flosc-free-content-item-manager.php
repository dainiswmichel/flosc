...
class FLOSC_Free_Content_Item_Manager {
	...
	public function handle_quiz_completion( $quiz_result, $user_id ) {
		...
		if ( $score >= 100 ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC: User {$user_id} scored {$score}% — no free lesson needed" );
			}
			return;
		}
		...
		if ( empty( $missed ) ) {
			if ( defined( 'FLOSC_DEBUG' ) && FLOSC_DEBUG ) {
				flosc_log( "FLOSC: No missed lessons found for user {$user_id}" );
			}
			return;
		}
		...
		$ranked_worst = $quiz_result['ranked_worst_lessons'] ?? array();
	...
	/**
	 * Lesson number is free eligible.
	 *
	 * @param int    $lesson_num Lesson num.
	 * @param string $quiz_id Quiz ID.
	 * @return bool
	...
	private function collect_eligible_from_tier( array $tier_entries, $quiz_id, array $exclude = array() ) {
		...
		foreach ( $tier_entries as $entry ) {
			$lesson_nums = array_map( 'intval', (array) ( $entry['lessons'] ?? array() ) );
	...
	/**
	 * Pick eligible lesson from tiers.
	 *
	...
	private function get_missed_lessons( $quiz_result ) {
		...
		if ( ! empty( $incorrect ) && is_array( $incorrect ) ) {
			...
			foreach ( $incorrect as $item ) {
				...
				foreach ( array( 'lesson', 'lesson_number', 'question_index', 'index', 'id' ) as $key ) {
		...
		if ( empty( $configured_cat ) ) {
			...
			if ( ! empty( $files ) ) {
				foreach ( array_unique( array_map( 'basename', $files ) ) as $fn ) {
					$s = get_option( 'flosc_flow_' . sanitize_key( pathinfo( $fn, PATHINFO_FILENAME ) ), array() );
					...
					if ( ! empty( $s['content_item_category'] ) ) {
						$configured_cat = $s['content_item_category'];
						break; }
				}
			}
		}
