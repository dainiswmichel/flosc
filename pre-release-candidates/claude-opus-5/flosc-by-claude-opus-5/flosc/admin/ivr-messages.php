...
if ( ! in_array( $flosc_ivr_management_view, array( 'single', 'all' ), true ) ) {
...
function flosc_run_ivr_diagnostics() {
	...
	if ( $read_back === $test_value ) {
		$diagnostics['db_connection'] = array(
			'status'  => 'green',
			'message' => 'Connected',
			'details' => array( 'Write/read/delete cycle successful' ),
		);
	} else {
	...
	if ( file_exists( $ivr_file ) ) {
		...
		$file_message_count = count( $config['messages'] ?? array() );

		if ( $file_message_count > 0 ) {
			$diagnostics['ivr_file'] = array(
				'status'  => 'green',
			...
			$diagnostics['ivr_file'] = array(
				'status'  => 'yellow',
				'message' => 'Parse issue',
				'details' => array( 'File exists but parser returned 0 messages' ),
			);
		}
	} else {
	...
	$flosc_fs    = $GLOBALS['flosc_current_settings'] ?? array();
	...
	if ( isset( $config ) && ! empty( $db_messages ) ) {
		$file_ids = array_keys( $config['messages'] ?? array() );
		...
		if ( empty( $in_file_not_db ) && empty( $in_db_not_file ) ) {
			...
			foreach ( $file_ids as $flosc_id ) {
				$file_msg = $normalize_for_compare( $config['messages'][ $flosc_id ] ?? array() );
				$db_msg   = $normalize_for_compare( $db_messages[ $flosc_id ] ?? array() );
			...
			if ( empty( $mismatches ) ) {
				$diagnostics['sync_status'] = array(
					'status'  => 'green',
					'message' => 'Match ✓',
					'details' => array( 'Active IVR Messages MD file and FLOSC DB match (' . count( $file_ids ) . ' messages, all fields checked)' ),
				);
			} else {
				$diagnostics['sync_status'] = array(
					'status'  => 'yellow',
	...
	$db_offers = $flosc_fs['offers'] ?? array();
	if ( isset( $config ) ) {
		$ivr_offers = array();
		foreach ( ( $config['messages'] ?? array() ) as $message_id => $flosc_msg ) {
			$flosc_msg_type = strtolower( trim( (string) ( $flosc_msg['type'] ?? '' ) ) );
			if ( 'offer' !== $flosc_msg_type ) {
				continue;
			}
		...
		if ( is_array( $db_offers ) ) {
			foreach ( $db_offers as $db_offer_id => $offer ) {
				...
				$registry_offers[ $offer_id ] = array(
		...
		if ( ! empty( $in_ivr_not_registry ) ) {
			...
			$diagnostics['offer_sync'] = array(
				'status'  => 'yellow',
				'message' => 'Out of sync',
				'details' => $flosc_details,
			);
		} else {
			$flosc_offer_fields = array( 'name', 'description', 'display_format', 'condition', 'reveal_phrase', 'icon' );
			...
			if ( empty( $field_mismatches ) ) {
				if ( ! empty( $in_registry_not_ivr ) ) {
					$flosc_details = array( 'In FLOSC DB only: ' . implode( ', ', array_slice( $in_registry_not_ivr, 0, 3 ) ) );
					...
					$diagnostics['offer_sync'] = array(
						'status'  => 'green',
						'message' => 'Match ✓',
						'details' => array( 'IVR offers and FLOSC DB offers match (' . count( $ivr_offer_ids ) . ' offers, key fields checked)' ),
					);
				}
			} else {
				$diagnostics['offer_sync'] = array(
	...
	if ( is_wp_error( $api_response ) ) {
		...
		if ( $fk ) {
			...
			$tmp['api_last_check'] = $michel_timestamp;
			update_option( $fk, $tmp ); }
	} else {
...
if ( isset( $flosc_post['flosc_clear_ivr_db'] ) ) {
	...
	if ( $flosc_flow_key ) {
		...
		flosc_flow_set_runtime( $flosc_fs, array(), $flosc_empty_phases, array() );
...
if ( isset( $flosc_post['flosc_confirm_import'] ) ) {
	...
	$flosc_import_mode = ( isset( $flosc_post['flosc_import_mode'] ) && 'replace' === $flosc_post['flosc_import_mode'] ) ? 'replace' : 'merge';
	$flosc_result      = flosc_import_ivr_to_database( false, $flosc_ivr_file_path, $flosc_flow_key, $flosc_import_mode );

	if ( $flosc_result['success'] ) {
		$flosc_did_merge = ( 'replace' !== $flosc_import_mode );
...
if ( isset( $flosc_post['save_ivr_message'] ) ) {
	...
	if ( '' === $flosc_msg_id ) {
		...
		if ( is_wp_error( $flosc_clean_content ) ) {
			...
			if ( 'offer' === $flosc_message_data['type'] ) {
				...
				foreach ( $flosc_offer_fields as $flosc_k => $flosc_v ) {
					if ( 'timer' === $flosc_k || 'post_id' === $flosc_k ) {
						$flosc_message_data[ $flosc_k ] = $flosc_v;
					} elseif ( '' !== $flosc_v && null !== $flosc_v ) {
						$flosc_message_data[ $flosc_k ] = $flosc_v;
					}
				}
			}
			...
			if ( 'concierge' === $flosc_message_data['type'] ) {
...
if ( isset( $flosc_get['delete_message'] ) && isset( $flosc_get['phase'] ) ) {
	...
	if ( '' === $flosc_msg_id ) {
		...
		if ( $flosc_flow_key ) {
			...
			if ( isset( $flosc_phases[ $flosc_phase ] ) ) {
				$flosc_phases[ $flosc_phase ] = array_diff( $flosc_phases[ $flosc_phase ], array( $flosc_msg_id ) );
			}
			flosc_flow_set_runtime( $flosc_fs, $flosc_messages, $flosc_phases, $flosc_styles );
			update_option( $flosc_flow_key, $flosc_fs );
			$GLOBALS['flosc_current_settings'] = $flosc_fs;
...
<div class="flosc-diagnostics-panel flosc-ivr-panel">
	...
	<p class="description flosc-ivr-panel__intro">
		<?php if ( 'single' === $flosc_ivr_management_view ) : ?>
	...
	<div class="flosc-ivr-actions-row">
		<a href="<?php echo esc_url( admin_url( 'admin.php?page=flosc-settings&tab=ivr-messages&ivr=' . rawurlencode( $flosc_active_ivr_file ) . '&view=single' ) ); ?>" class="button <?php echo esc_attr( 'single' === $flosc_ivr_management_view ? 'button-primary' : '' ); ?>">
			Single Flow: Message Editing
		</a>
		<a href="<?php echo esc_url( admin_url( 'admin.php?page=flosc-settings&tab=ivr-messages&ivr=' . rawurlencode( $flosc_active_ivr_file ) . '&view=all' ) ); ?>" class="button <?php echo esc_attr( 'all' === $flosc_ivr_management_view ? 'button-primary' : '' ); ?>">
...
<?php if ( 'all' === $flosc_ivr_management_view ) : ?>
<div class="flosc-info-box flosc-ivr-info-box">
	...
	if ( file_exists( $flosc_ivr_file_path ) ) {
		...
		if ( false === $flosc_active_ivr_full_text ) {
			$flosc_active_ivr_full_text = '';
		}
	}
	?>
...
if ( null !== $flosc_import_preview ) :
	...
	<div class="flosc-import-preview flosc-ivr-compare-preview">
						...
						<div class="flosc-ivr-entry-grid">
							<?php foreach ( $flosc_import_preview['added'] as $flosc_id ) : ?>
								<div class="flosc-ivr-entry-card flosc-ivr-entry-card--file">
									...
									<pre class="flosc-ivr-entry-card__json"><?php echo esc_html( wp_json_encode( $flosc_file_messages_for_compare[ $flosc_id ] ?? array(), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) ); ?></pre>
								</div>
							<?php endforeach; ?>
						</div>
					</details>
						...
						<div class="flosc-ivr-entry-grid">
							<?php foreach ( $flosc_import_preview['deleted'] as $flosc_id ) : ?>
								<div class="flosc-ivr-entry-card flosc-ivr-entry-card--db-only">
									<div class="flosc-ivr-entry-card__title"><?php echo esc_html( $flosc_id ); ?></div>
									<pre class="flosc-ivr-entry-card__json"><?php echo esc_html( wp_json_encode( $flosc_db_messages_for_compare[ $flosc_id ] ?? array(), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) ); ?></pre>
								</div>
							<?php endforeach; ?>
						</div>
					</details>
		...
		<div class="flosc-ivr-field-diff-wrap">
			<h4 class="flosc-ivr-field-diff-wrap__title">🔍 Field-Level Differences (<?php echo count( $flosc_field_diffs ); ?> message<?php echo 1 !== count( $flosc_field_diffs ) ? 's' : ''; ?> changed):</h4>
			...
			<details class="flosc-ivr-field-diff-item">
				<summary class="flosc-ivr-field-diff-item__summary">
					▶ <?php echo esc_html( $flosc_msg_id ); ?> — <?php echo count( $flosc_diffs ); ?> field<?php echo 1 !== count( $flosc_diffs ) ? 's' : ''; ?> changed
				</summary>
				<table class="flosc-ivr-field-diff-table">
					...
					foreach ( $flosc_diffs as $flosc_field => $flosc_vals ) :
						...
						if ( '' === $flosc_file_display ) {
							$flosc_file_display = '<em class="flosc-ivr-empty-value">(empty)</em>';
						}
						?>
				...
				<details class="flosc-ivr-full-entry-details">
					...
					<div class="flosc-ivr-full-entry-grid">
						<div class="flosc-ivr-full-entry-card flosc-ivr-full-entry-card--db">
							...
							<pre class="flosc-ivr-entry-card__json"><?php echo esc_html( wp_json_encode( $flosc_db_messages_for_compare[ $flosc_msg_id ] ?? array(), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) ); ?></pre>
						</div>
						<div class="flosc-ivr-full-entry-card flosc-ivr-full-entry-card--file">
							<div class="flosc-ivr-entry-card__title">File Entry</div>
							<pre class="flosc-ivr-entry-card__json"><?php echo esc_html( wp_json_encode( $flosc_file_messages_for_compare[ $flosc_msg_id ] ?? array(), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) ); ?></pre>
						</div>
					</div>
				</details>
			</details>
...
<!-- ============================================ -->
<?php
$flosc_phase_meta  = array(
	'freeline' => array(
		'icon'  => '🌐',
		'label' => 'Freeline',
		'desc'  => 'Visitor (not logged in) — Encourage quiz completion',
	),
	'login'    => array(
		'icon'  => '🔑',
		'label' => 'Login',
		'desc'  => 'Post-quiz + Logged-in — Deliver free lesson, present offer',
	),
	'offer'    => array(
		'icon'  => '🏷️',
		'label' => 'Offer',
		'desc'  => 'Sales pitch — Product offers and promotions',
	),
	'sale'     => array(
		'icon'  => '💳',
		'label' => 'Sale',
		'desc'  => 'Post-purchase — Onboarding and welcome',
	),
	'content'  => array(
		'icon'  => '📚',
		'label' => 'Content',
		'desc'  => 'Ongoing member support and engagement',
	),
);
...
<p class="flosc-ivr-phase-summary">
	...
	foreach ( $flosc_phase_meta as $flosc_pid => $flosc_pm ) {
		...
		if ( $flosc_cnt > 0 ) {
			$flosc_active_phases[] = '<a href="#phase-' . esc_attr( $flosc_pid ) . '" class="flosc-ivr-phase-link">' . esc_html( $flosc_pm['icon'] . ' ' . $flosc_pm['label'] . ' (' . $flosc_cnt . ')' ) . '</a>';
		}
	}
...
<?php
foreach ( $flosc_phase_meta as $flosc_phase_id => $flosc_pm ) :
	$flosc_phase_msg_ids = $flosc_phases[ $flosc_phase_id ] ?? array();
	?>
<div class="flosc-phase-section" id="phase-<?php echo esc_attr( $flosc_phase_id ); ?>">
	...
	<?php
	foreach ( $flosc_phase_msg_ids as $flosc_msg_id ) :
		if ( ! isset( $flosc_messages[ $flosc_msg_id ] ) ) {
			continue;
		}
	...
	<div class="flosc-msg-card <?php echo esc_attr( $flosc_is_open ? 'is-open' : '' ); ?>" id="card-<?php echo esc_attr( $flosc_safe_id ); ?>">
		...
		<div class="flosc-msg-editor">
			<form method="post" action="<?php echo esc_url( $flosc_ivr_management_phase_url ); ?>">
				...
				<table class="form-table">
					...
					<tr>
						...
						<td>
							<select name="message_style">
								<?php foreach ( array( 'default', 'pill', 'button', 'chip', 'card' ) as $flosc_s ) : ?>
					...
					<tr>
						...
						<td>
							...
							<p class="description">Personality and flow variables: <code>{personality_name}</code>, <code>{personality_role}</code>, <code>{flow_name}</code>, <code>{public_title}</code>, <code>{tagline}</code>, <code>{site_name}</code>. Other variables: <code>{name}</code>, <code>{score}</code>, <code>{product_name}</code>, <code>{title}</code>, <code>{price}</code>. Use a new line, <code>\n</code>, or <code>&lt;br&gt;</code> for a visible line break.</p>
						</td>
					</tr>
					<tr>
				...
				<div class="flosc-offer-fields-inner <?php echo esc_attr( 'offer' === ( $flosc_msg['type'] ?? '' ) ? '' : 'flosc-hidden' ); ?>" id="offer-fields-<?php echo esc_attr( $flosc_safe_id ); ?>">
						...
						<tr>
							...
							<td>
								<select name="message_display_format">
									<?php foreach ( array( 'card', 'pill', 'compact', 'banner', 'featured', 'text', 'inline-checkout' ) as $flosc_fmt ) : ?>
				...
				<div class="flosc-concierge-fields-inner <?php echo esc_attr( 'concierge' === ( $flosc_msg['type'] ?? '' ) ? '' : 'flosc-hidden' ); ?>" id="concierge-fields-<?php echo esc_attr( $flosc_safe_id ); ?>">
