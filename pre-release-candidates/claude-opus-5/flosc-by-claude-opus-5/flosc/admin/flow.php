...
$flosc_portability_docs_url    = add_query_arg(
	...
	admin_url( 'admin.php' )
) . '#tab-flow-all-flows-portability-fields';
$flosc_flow_view               = isset( $flosc_get['view'] ) ? sanitize_key( (string) $flosc_get['view'] ) : 'single';
if ( ! in_array( $flosc_flow_view, array( 'single', 'all' ), true ) ) {
...
$flosc_companion_mode_labels             = array(
	'in_chat'   => 'Full-page',
	'companion' => 'Companion',
	'both'      => 'Hybrid',
);
...
$flosc_prompt_panel_counts    = array(
...
<div class="flosc-flow-overview">
	...
	flosc_flow_card(
		'F',
		'Freeline',
		'What visitors see before logging in',
		array(
			array(
	...
	flosc_flow_card(
		'C',
		'Content',
		'What learners see after purchase',
		array(
			array(
	...
	<div class="flosc-info-box flosc-ivr-info-box">
		...
		<table class="widefat striped flosc-ivr-file-table flosc-pack-assets-table">
			...
			<?php endforeach; ?>
			<?php
			foreach ( $flosc_pack_assets['wxr'] as $flosc_pack_wxr ) :
			...
			<?php endforeach; ?>
			<?php
			foreach ( $flosc_pack_assets['media'] as $flosc_pack_media ) :
