...
$flosc_product_token_grant_recurring        = max(
	0,
	intval(
...
$flosc_product_token_grant_recurring_yearly = max(
	0,
	intval(
...
$flosc_product_token_grant_onetime          = max(
	0,
	intval(
...
if ( ! in_array( $flosc_depleted_contact_mode, array( 'message', 'in_chat_form' ), true ) ) {
	$flosc_depleted_contact_mode = 'message';
}

// Products (offers) for this flow — active first.
$flosc_offers       = is_array( $flosc_flow_settings['offers'] ?? null ) ? $flosc_flow_settings['offers'] : array();
$flosc_product_rows = array();
foreach ( $flosc_offers as $flosc_oid => $flosc_offer ) {
	...
	$flosc_tokens    = is_array( $flosc_offer['tokens'] ?? null ) ? $flosc_offer['tokens'] : array();
...
usort(
	$flosc_product_rows,
	static function ( $flosc_a, $flosc_b ) {
...
$flosc_active_count = count(
	array_filter(
		$flosc_product_rows,
		static function ( $flosc_row ) {
