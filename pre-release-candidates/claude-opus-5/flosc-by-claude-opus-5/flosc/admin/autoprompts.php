...
$flosc_autoprompt_docs_anchor = array(
...
function flosc_handle_autoprompts_save() {
	...
	foreach ( $states as $state ) {
		$icons            = $flosc_post[ $state . '_pill_icon' ] ?? array();
		$labels           = $flosc_post[ $state . '_pill_label' ] ?? array();
		$inputs           = $flosc_post[ $state . '_pill_user_input' ] ?? array();
		$trigger_types    = $flosc_post[ $state . '_pill_trigger_type' ] ?? array();
		$trigger_values   = $flosc_post[ $state . '_pill_trigger_value' ] ?? array();
		$flosc_conditions = $flosc_post[ $state . '_pill_conditions' ] ?? array();
		$styles           = $flosc_post[ $state . '_pill_style' ] ?? array();
...
while ( $flosc_ap_prev !== $flosc_ap_raw ) {
	$flosc_ap_prev = $flosc_ap_raw;
	$flosc_ap_raw  = stripslashes_deep( $flosc_ap_raw ); }
$flosc_saved_prompts = $flosc_ap_raw;

$flosc_ah_raw  = $flosc_fs['autoprompt_headers'] ?? array();
$flosc_ah_prev = null;
while ( $flosc_ah_prev !== $flosc_ah_raw ) {
	$flosc_ah_prev = $flosc_ah_raw;
	$flosc_ah_raw  = stripslashes_deep( $flosc_ah_raw ); }
...
$flosc_saved_panel_enabled     = $flosc_fs['autoprompt_panel_enabled'] ?? array();
$flosc_panel_enabled           = array(
...
$flosc_prompts = array(
	'visitor' => is_array( $flosc_saved_prompts['visitor'] ?? null ) ? $flosc_saved_prompts['visitor'] : array(),
	'guest'   => is_array( $flosc_saved_prompts['guest'] ?? null ) ? $flosc_saved_prompts['guest'] : array(),
	'member'  => is_array( $flosc_saved_prompts['member'] ?? null ) ? $flosc_saved_prompts['member'] : array(),
);
...
$flosc_available_conditions = array(
...
<div class="flosc-autoprompt-section flosc-autoprompt-section--<?php echo esc_attr( $flosc_state ); ?>">
	...
	<div class="flosc-ap-preview">
		...
		<div id="preview-<?php echo esc_attr( $flosc_state ); ?>" class="flosc-ap-preview__pills">
			...
			<span class="flosc-preview-pill-chip">
				...
				if ( $flosc_pill['icon'] ) :
					?>
					<span><?php echo esc_html( $flosc_pill['icon'] ); ?></span><?php endif; ?>
	...
	<table class="widefat flosc-ap-table">
		...
		foreach ( $flosc_pills as $flosc_pill ) :
			...
			$flosc_expected_behavior = flosc_autoprompt_expected_behavior_text(
				$flosc_state,
				array(
		...
		array(
			...
			'pills' => array(
				...
				array(
					...
					'trigger_type'  => 'action',
					'trigger_value' => 'open_quiz',
					'condition'     => 'is_visitor',
					'style'         => 'button',
				),
				array(
					'icon'          => '🔐',
					'label'         => 'Login to see my score',
					'user_input'    => 'Sign in / Register',
					'trigger_type'  => 'action',
					'trigger_value' => 'open_registration',
					'condition'     => 'is_visitor && quiz_taken',
					'style'         => 'button',
				),
				array(
					'icon'          => '❓',
				...
				array(
					...
					'user_input'    => 'PURCHASE',
					'trigger_type'  => 'action',
					'trigger_value' => 'checkout_full_access',
					'condition'     => 'is_visitor',
					'style'         => 'button',
				),
				array(
	...
	'guest'   => array(
		array(
			...
			'pills' => array(
				array(
					'icon'          => '💰',
					'label'         => 'See my personalized offer',
					'user_input'    => 'Show me my personalized offer',
					'trigger_type'  => 'offer',
					'trigger_value' => 'full_access',
					'condition'     => 'is_guest',
					'style'         => 'pill',
				),
				array(
					'icon'          => '🎁',
					'label'         => 'Unlock my free lesson',
					'user_input'    => 'Unlock my free lesson',
					'trigger_type'  => 'action',
					'trigger_value' => 'open_lesson_library',
					'condition'     => 'is_guest',
					'style'         => 'button',
				),
				array(
					'icon'          => '📊',
					'label'         => 'View my quiz results',
					'user_input'    => 'Show me my quiz results and recommendations',
					'trigger_type'  => 'ai',
					'trigger_value' => '',
					'condition'     => 'is_guest',
					'style'         => 'pill',
				...
				array(
					...
					'user_input'    => 'Based on my quiz, what should I study first?',
					'trigger_type'  => 'ai',
					'trigger_value' => '',
					'condition'     => 'is_guest',
					'style'         => 'pill',
		...
		array(
			...
			'pills' => array(
				array(
					...
					'user_input'    => 'Open my lesson library',
					'trigger_type'  => 'action',
					'trigger_value' => 'open_lesson_library',
					'condition'     => 'is_member',
					'style'         => 'button',
				...
				array(
					...
					'user_input'    => "I'm ready to start my first lesson",
					'trigger_type'  => 'action',
					'trigger_value' => 'open_lesson_library',
					'condition'     => 'is_member && first_member',
					'style'         => 'button',
				),
				array(
					'icon'          => '📊',
					'label'         => 'My quiz results',
					'user_input'    => 'Show me my quiz results and progress',
					'trigger_type'  => 'ai',
					'trigger_value' => '',
					'condition'     => 'is_member && quiz_taken',
					'style'         => 'pill',
				),
				array(
					'icon'          => '🎯',
				...
				array(
					'icon'          => '🔍',
					'label'         => 'Find R sound lessons',
					'user_input'    => 'Find R sound lessons for me',
					'trigger_type'  => 'ai',
					'trigger_value' => '',
					'condition'     => 'is_member',
					'style'         => 'pill',
				...
				array(
					'icon'          => '🔍',
					'label'         => 'Find TH lessons',
					'user_input'    => 'Find TH lessons for me',
					'trigger_type'  => 'ai',
					'trigger_value' => '',
					'condition'     => 'is_member',
					'style'         => 'pill',
				...
				array(
					...
					'user_input'    => 'Continue my last lesson',
					'trigger_type'  => 'action',
					'trigger_value' => 'continue_lesson',
					'condition'     => 'is_member',
					'style'         => 'button',
				),
				array(
					'icon'          => '🔄',
					'label'         => 'Retake quiz',
					'user_input'    => 'Retake my assessment',
					'trigger_type'  => 'action',
					'trigger_value' => 'open_quiz',
					'condition'     => 'is_member',
					'style'         => 'pill',
...
<?php
$flosc_autoprompts_admin_config = array(
