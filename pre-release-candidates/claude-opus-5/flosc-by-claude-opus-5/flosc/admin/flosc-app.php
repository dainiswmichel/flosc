...
<?php
/*
====  Source documentation — kept in the file for maintainers, NOT emitted to the browser. ====
================================================================================
...
<head>
	...
	if ( ! $flosc_favicon_generic ) {
		$flosc_favicon_generic = FLOSC_PLUGIN_URL . 'assets/img/flosc-icon.png';
	}
	if ( ! $flosc_favicon_32 ) {
		$flosc_favicon_32 = FLOSC_PLUGIN_URL . 'assets/img/flosc-icon-32.png';
	}
	if ( ! $flosc_favicon_180 ) {
		$flosc_favicon_180 = FLOSC_PLUGIN_URL . 'assets/img/flosc-icon-180.png';
	}
	?>
...
';
	...
	<?php ob_start(); ?>
		:root {
			...
			if ( ! in_array( $flosc_avatar_radius, array( '8px', '50%', '4px', '0' ), true ) ) {
...
<body class="<?php echo esc_attr( $flosc_body_classes ); ?>"
	...
	<aside class="flosc-sidebar" id="flosc_app_sidebar">
		...
		$flosc_pb_visitor  = wp_parse_args(
			$flosc_profile_bar['visitor'] ?? array(),
			array(
				'name'          => 'Visitor',
		...
		$flosc_pb_guest    = wp_parse_args(
			$flosc_profile_bar['guest'] ?? array(),
			array(
				'name'          => '',
				'badge'         => 'Guest',
				'icon'          => '',
		...
		$flosc_pb_member   = wp_parse_args(
			$flosc_profile_bar['member'] ?? array(),
			array(
				'name'          => '',
				'badge'         => 'Member',
				'icon'          => '',
	...
	<div class="flosc-modal-overlay" id="flosc_modal_recording">
		<div class="flosc-modal flosc-recording-modal">
			...
			<div class="flosc-modal-body">
				...
				$flosc_enabled_quizzes = flosc_get_setting( 'enabled_quizzes', array() );
	...
	<?php ob_start(); ?>
		...
		if ( empty( $flosc_companion_source ) && ! empty( $flosc_current_flow ) && is_array( $flosc_current_flow ) ) {
			...
			$flosc_companion_handoff_enabled   = $flosc_companion_enabled && in_array( $flosc_companion_mode, array( 'companion', 'both' ), true );
			...
			if ( ! in_array( $flosc_companion_state_storage, array( 'session', 'local' ), true ) ) {
			...
			if ( ! in_array( $flosc_companion_routing_mode, array( 'hub', 'domain_persistence' ), true ) ) {
			...
			$flosc_hub_defaults_rt    = function_exists( 'flosc_companion_hub_defaults_from_flow' )
				...
				: array(
					'fullscreen' => (string) $flosc_app_url,
					'companion'  => home_url( '/' ),
				);
			...
			echo wp_json_encode(
				array(
					...
					'subscriptionMonthlyTokenGrant'  => max(
						0,
						intval(
					...
					'subscriptionYearlyTokenGrant'   => max(
						0,
						intval(
					...
					'guestLinkCheckEmailMessage'     => ( function () {
						$v = flosc_get_setting( 'guest_link_check_email_message', "We've sent you a {link_name} to your email — click it to access this chat as a guest and view your quiz score, free lessons, and upgrade options." );
						$p = null;
						while ( $p !== $v ) {
									$p = $v;
									$v = stripslashes_deep( $v );
						} return $v; } )(),
					'guestLinkWelcomeMessage'        => ( function () {
						$v = flosc_get_setting( 'guest_link_welcome_message', 'Hi, welcome back! Just to confirm: your email address is {email} and you can use your {link_name} {n} more times to access this chat. <a href="{upgrade_url}">Upgrade</a> for complete access.' );
						$p = null;
						while ( $p !== $v ) {
									$p = $v;
									$v = stripslashes_deep( $v );
					...
					'guestLinkProfileConfirmMessage' => ( function () {
						$v = flosc_get_setting( 'guest_link_profile_confirm_message', 'Perfect, {name}! You can always log in directly at {login_url}, update your profile, and upgrade to full access.' );
						$p = null;
						while ( $p !== $v ) {
									$p = $v;
									$v = stripslashes_deep( $v );
