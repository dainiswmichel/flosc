...
$flosc_hub_defaults          = function_exists( 'flosc_companion_hub_defaults_from_flow' )
	? flosc_companion_hub_defaults_from_flow( is_array( $flosc_flow_settings ) ? $flosc_flow_settings : array() )
	: array(
		'fullscreen'            => home_url( '/' ),
		'companion'             => home_url( '/' ),
		'chat_app'              => home_url( '/' ),
...
$flosc_target_include_pages      = array();
$flosc_target_include_posts      = array();
$flosc_target_include_categories = array();
$flosc_target_include_tags       = array();
$flosc_target_include_custom     = array();

$flosc_target_exclude_pages      = array();
$flosc_target_exclude_posts      = array();
$flosc_target_exclude_categories = array();
$flosc_target_exclude_tags       = array();
$flosc_target_exclude_custom     = array();
...
/*
 * This builds the list of pages a companion can be attached to, so anything
 * dropped is a page the admin cannot pick. It used to ask for 500 in one query
 * and defend the number; flosc_get_posts_all() pages through in hundreds instead,
 * so there is no ceiling to defend and no single large query. Ids only, so no
 * post bodies are loaded either way.
 */
$flosc_target_posts = flosc_get_posts_all(
	array(
		'post_type'   => 'post',
		'post_status' => array( 'publish', 'private', 'draft' ),
		'orderby'     => 'title',
...
$flosc_companion_launcher_icons = apply_filters(
...
if ( ! is_array( $flosc_companion_launcher_icons ) || empty( $flosc_companion_launcher_icons ) ) {
	$flosc_companion_launcher_icons = array(
		'product_logo' => 'Product Logo (Chat Logo)',
		'chat'         => 'Chat Bubble',
	);
}
...
if ( ! is_array( $flosc_companion_positions ) || empty( $flosc_companion_positions ) ) {
	$flosc_companion_positions = array( 'bottom-right', 'bottom-left' );
}
$flosc_companion_position_labels = apply_filters(
	'flosc_companion_position_labels',
	array(
...
if ( ! is_array( $flosc_companion_mobile_behaviors ) || empty( $flosc_companion_mobile_behaviors ) ) {
	$flosc_companion_mobile_behaviors = array( 'fullscreen', 'panel' );
}
$flosc_companion_mobile_labels = apply_filters(
	'flosc_companion_mobile_behavior_labels',
	array(
...
if ( ! is_array( $flosc_companion_context_scopes ) || empty( $flosc_companion_context_scopes ) ) {
	$flosc_companion_context_scopes = array( 'basic', 'extended' );
}
$flosc_companion_context_scope_labels = apply_filters(
	'flosc_companion_context_scope_labels',
	array(
...
if ( ! is_array( $flosc_companion_motion_modes ) || empty( $flosc_companion_motion_modes ) ) {
	$flosc_companion_motion_modes = array( 'system', 'reduce', 'full' );
}
$flosc_companion_motion_mode_labels = apply_filters(
	'flosc_companion_motion_mode_labels',
	array(
...
if ( ! is_array( $flosc_companion_state_storages ) || empty( $flosc_companion_state_storages ) ) {
	$flosc_companion_state_storages = array( 'session', 'local' );
}
$flosc_companion_state_storage_labels = apply_filters(
	'flosc_companion_state_storage_labels',
	array(
...
$flosc_companion_extension_hooks = array(
	array(
		'hook'    => 'flosc_companion_defaults',
		'type'    => 'filter',
		'purpose' => 'Override default companion runtime values.',
	),
	array(
		'hook'    => 'flosc_companion_frontend_config',
		'type'    => 'filter',
		'purpose' => 'Adjust the final frontend config payload before init.',
	),
	array(
		'hook'    => 'flosc_companion_allowed_modes',
		'type'    => 'filter',
		'purpose' => 'Define allowed content display modes.',
	),
	array(
		'hook'    => 'flosc_companion_widget_modes',
		'type'    => 'filter',
		'purpose' => 'Choose which modes render widget shell.',
	),
	array(
		'hook'    => 'flosc_companion_allowed_positions',
		'type'    => 'filter',
		'purpose' => 'Allow launcher positions shown in Style tab.',
	),
	array(
		'hook'    => 'flosc_companion_mobile_behaviors',
		'type'    => 'filter',
		'purpose' => 'Control supported mobile open behaviors.',
	),
	array(
		'hook'    => 'flosc_companion_context_scopes',
		'type'    => 'filter',
		'purpose' => 'Control context-handoff scope options.',
	),
	array(
		'hook'    => 'flosc_companion_motion_modes',
		'type'    => 'filter',
		'purpose' => 'Set motion/accessibility mode options.',
	),
	array(
		'hook'    => 'flosc_companion_state_storages',
		'type'    => 'filter',
		'purpose' => 'Set browser storage backends for open-state/cooldown.',
	),
	array(
		'hook'    => 'flosc_companion_launcher_svg_paths',
		'type'    => 'filter',
		'purpose' => 'Override launcher icon SVG path map.',
	),
	array(
		'hook'    => 'flosc_companion_context_params',
		'type'    => 'filter',
		'purpose' => 'Customize iframe context params at launch.',
	),
	array(
		'hook'    => 'flosc_companion_target_rule_types',
		'type'    => 'filter',
		'purpose' => 'Add or constrain targeting rule types.',
	),
	array(
		'hook'    => 'flosc_companion_target_rule_match',
		'type'    => 'filter',
		'purpose' => 'Inject custom rule match logic.',
	),
	array(
		'hook'    => 'flosc_companion_launcher_icon_choices',
		'type'    => 'filter',
		'purpose' => 'Configure launcher icon choices in Style UI.',
	),
	array(
		'hook'    => 'flosc_companion_position_labels',
		'type'    => 'filter',
		'purpose' => 'Override launcher position labels in Style UI.',
	),
	array(
		'hook'    => 'flosc_companion_mobile_behavior_labels',
		'type'    => 'filter',
		'purpose' => 'Override mobile behavior labels in Style UI.',
	),
	array(
		'hook'    => 'flosc_companion_context_scope_labels',
		'type'    => 'filter',
		'purpose' => 'Override context scope labels in Style UI.',
	),
	array(
		'hook'    => 'flosc_companion_motion_mode_labels',
		'type'    => 'filter',
		'purpose' => 'Override motion mode labels in Style UI.',
	),
	array(
		'hook'    => 'flosc_companion_state_storage_labels',
		'type'    => 'filter',
		'purpose' => 'Override state storage labels in Style UI.',
	),
	array(
		'hook'    => 'flosc_companion_numeric_limits',
		'type'    => 'filter',
		'purpose' => 'Set min/max numeric bounds for companion controls and sanitization.',
	),
);
		...
		<tr>
			...
			<td>
				...
				if ( function_exists( 'flosc_resolve_chatlogo_url' ) ) {
					...
					$flosc_identity_chatlogo = flosc_resolve_chatlogo_url( is_array( $flosc_flow_settings ) ? $flosc_flow_settings : array(), false );
				} else {
