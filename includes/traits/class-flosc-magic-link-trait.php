 ...
 *
 * WordPress.org submission kill-switch (does not delete code):
 *   define( 'FLOSC_ENABLE_MAGIC_ACCESS_LINKS', false );
 * in wp-config.php, or:
 *   add_filter( 'flosc_enable_magic_access_links', '__return_false' );
 * When false: no mint, no flosc_magic consume, no guest-link emails with access URLs.
 * SSO/cross-domain flosc_login_token and post-purchase login tokens are separate
 * (see handle_login_token Case 1) — revisit those for .org in the next planning pass.
 *
 * File: includes/traits/class-flosc-magic-link-trait.php
 */
...
trait FLOSC_Magic_Link_Trait {
     ...
     *
     * Default true for production/private deploys. Flip false for WordPress.org zip.
     *
     * @return bool
     */
    private function flosc_magic_access_links_enabled() {
        if (defined('FLOSC_ENABLE_MAGIC_ACCESS_LINKS') && ! FLOSC_ENABLE_MAGIC_ACCESS_LINKS) {
            return false;
        }
        /**
         * Filter: enable/disable MagicLink access links (mint + consume + guest emails).
         *
         * @param bool $enabled Default true unless FLOSC_ENABLE_MAGIC_ACCESS_LINKS is false.
         */
        return (bool) apply_filters('flosc_enable_magic_access_links', true);
    }
    ...
    public function handle_login_token() {
        ...
        if (!empty($get['flosc_magic'])) {
            if (!$this->flosc_magic_access_links_enabled()) {
            ...
            // Optional: promote bare subscriber to configured guest role (not account creation).
    ...
    public function flosc_activate_email_account($user_id, $attach = null) {
        ...
        if (!in_array($member_level, $roles, true)
            && !in_array($guest_level, $roles, true)
            && !in_array('lesaep_learners', $roles, true)
            && !in_array('guest_lesaep_learner', $roles, true)
        ) {
    ...
    private function flosc_mint_magic_access_for_user($user_id, array $args = []) {
        if (!$this->flosc_magic_access_links_enabled()) {
            return new WP_Error(
                ...
                'Magic access links are disabled in this package.'
            );
        }
        $user_id = absint($user_id);
        $user    = $user_id > 0 ? get_userdata($user_id) : false;
    ...
    public function flosc_handle_password_change_revoke_magic_access($password, $user_id, $old_user_data = null) {
        ...
        // For WordPress.org package mode, keep magic links shelved after revocation.
        if (!$this->flosc_magic_access_links_enabled()) {
    ...
    public function ajax_send_guest_link() {
        ...
        if (!$this->flosc_magic_access_links_enabled()) {
            wp_send_json_error(['message' => 'Magic access links are disabled in this package.']);
        }
        ...
        <table class="form-table" role="presentation">
            <tr>
                ...
                <td>
                    ...
                    <?php if ($verified_at !== '') : ?>
                        <p class="description"><?php echo esc_html(sprintf(__('Verified at: %s', 'flosc'), $verified_at)); ?></p>
                    <?php endif; ?>
