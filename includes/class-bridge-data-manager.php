...
class FLOSC_Bridge_Data_Manager {
    ...
    public function handle_purchase_completed($user_id, $purchase_data = []) {
        ...
        if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) flosc_log("FLOSC Bridge: User {$user_id} purchased - exited bridge state");
    }
    ...
    public function flosc_create_bridge_data($user_id, $quiz_id, $scoring_results) {
        ...
        if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) flosc_log("FLOSC Bridge: Created bridge data for user {$user_id}, quiz {$quiz_id}, score {$bridge_data['score']}%");
