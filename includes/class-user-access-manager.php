...
class FLOSC_User_Access_Manager {
    ...
    public function grant_member_access($user_id, $reason = 'quiz_completion') {
        ...
        // Log the grant
        if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) flosc_log("FLOSC: Granted member access to user {$user_id} - Reason: {$reason}");
    ...
    public function revoke_member_access($user_id) {
        ...
        if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) flosc_log("FLOSC: Revoked member access from user {$user_id}");
...
if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) flosc_log('FLOSC WishList Member API error: ' . $e->getMessage());
                    }
                }
            }
