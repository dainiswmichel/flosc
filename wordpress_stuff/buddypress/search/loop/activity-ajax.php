<?php
/**
 * Search results template — Activity items (AJAX / inline search variant)
 *
 * Overrides buddyboss-platform/.../search/loop/activity-ajax.php
 * Child theme override: yourtheme/buddypress/search/loop/activity-ajax.php
 *
 * Change from core: strips page-builder shortcodes (Divi, WPBakery, etc.)
 * from the activity intro snippet before display.
 *
	...
	<a href='<?php echo esc_url( add_query_arg( array( 'no_frame' => '1' ), bp_get_activity_thread_permalink() ) ); ?>'>
		...
		<div class="item">
			...
			<?php if ( bp_activity_has_content() ) : ?>
				<?php
				// Strip page-builder shortcodes before rendering the intro snippet.
				$intro = dainis_strip_builder_shortcodes_from( bp_get_activity_content_body() );
