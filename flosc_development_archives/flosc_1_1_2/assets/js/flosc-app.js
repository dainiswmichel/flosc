 ...
 * v1.1.2: Fixed carousel overflow, user status response for admin
 */

// v9.4.7: Clear FLOSC-specific localStorage on version change
(function() {
    const FLOSC_JS_VERSION = '1.1.2';
    try {
...
class floscApp {
    ...
    floscShowUserAutoPrompts() {
        ...
        // v1.1.0: Backend already filtered by phase and conditions
        // Frontend just needs to collect all autoprompts and render them
        // The backend returns messages for the appropriate phases (login+offer for guests, sale+content for members)
        const applicable = [];
        
        for (const msg of autoPrompts) {
            // v1.1.0: For visitors, only show messages from freeline phase
            if (this.state === 'visitor') {
            ...
            // v1.1.0: For guests/members, backend already filtered - include all autoprompts
            // Re-evaluate conditions on frontend for dynamic state (like message_count)
            if (!msg.conditions || msg.conditions === 'always' || this.evaluateCondition(msg.conditions)) {
                applicable.push(msg);
            }
        }
        
        // v1.1.0: Fallback - if no matches, show ALL autoprompts from API response
        if (applicable.length === 0) {
            console.log('FLOSC: No conditional matches, showing all autoprompts from API');
            applicable.push(...autoPrompts);
        }
        ...
        console.log('FLOSC: User autoprompts for', this.state, ':', applicable.map(r => r.name));
     ...
     * 
     * WHAT IT DOES:
     * - Horizontal scrolling container for pills/cards
     * - Shows prev/next arrows ONLY when content overflows
     * - Infinite loop: end wraps to start, start wraps to end
     * - Supports click, swipe, and drag
     * 
     * HOW IT WORKS:
     * 1. OVERFLOW CHECK: scrollWidth > clientWidth? → show arrows
     * 2. SCROLL AMOUNT: one item width + gap
     * 3. NAVIGATION: scrollBy() for smooth movement
     * 4. INFINITE LOOP:
     *    - At end + click next → jump to start
     *    - At start + click prev → jump to end
     * 
     * DOM STRUCTURE EXPECTED:
     * <div class="flosc-carousel-container">
     *   <button class="flosc-carousel-prev">‹</button>
     *   <div class="flosc-carousel-track"> <!-- overflow-x: auto -->
     *     <div class="item">...</div>
     *     <div class="item">...</div>
     *   </div>
     *   <button class="flosc-carousel-next">›</button>
     * </div>
     * 
     * CSS REQUIREMENTS:
     * - .flosc-carousel-track: display:flex, overflow-x:auto, gap:Npx
     * - .flosc-carousel-arrow: display:none by default
     * - Items must have fixed/min width (no flex-shrink)
     * 
     * MTS-2026-02-02: [CAROUSEL-FIX] Previous failures:
     * 1. Used clone nodes for "infinite" scroll (unnecessary complexity)
     * 2. Used CSS transforms instead of native scroll (overcomplicated)
     * 3. Forgot to add 'has-overflow' class to container
     * 
     * Current approach: Native browser scroll only!
     * - scrollWidth > clientWidth → has overflow → show arrows
     * - Click arrow → scrollBy() or scrollTo() for loop
     * - No clones, no transforms, no state management
     * ============================================================================
     */
    initCarouselOverflow(container) {
        ...
        if (!track || !prevBtn || !nextBtn) {
            console.warn('[FLOSC Carousel] Missing elements:', { track: !!track, prevBtn: !!prevBtn, nextBtn: !!nextBtn });
            return;
        }

        // --- HELPERS ---
        
        // Get width of first item + gap (how far to scroll per click)
        const getScrollStep = () => {
        ...
        const carouselContainer = container.querySelector('.flosc-carousel-container');
        
        const checkOverflow = () => {
            const hasOverflow = track.scrollWidth > track.clientWidth + 2;
            
            if (hasOverflow) {
                carouselContainer?.classList.add('has-overflow');
                prevBtn.style.display = 'flex';
                nextBtn.style.display = 'flex';
            } else {
                carouselContainer?.classList.remove('has-overflow');
        ...
        track.addEventListener('touchend', e => {
            ...
            if (diff > 50) scrollNext();      // Swipe left → next
            else if (diff < -50) scrollPrev(); // Swipe right → prev
        });
        ...
        // Recheck when track size changes (images load, content changes)
