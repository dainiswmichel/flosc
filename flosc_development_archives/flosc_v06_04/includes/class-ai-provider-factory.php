...
class FLOSC_AI_Provider_Factory {
    ...
    /**
     * Build system prompt by merging base + phase-specific prompts + knowledge files (v04_04/v04_05)
     */
    public function build_system_prompt($phase = '', $context = []) {
        ...
        // 3. Load AI knowledge files (v04_05)
        $configuration_content = $this->load_configuration_files();
    ...
    /**
     * Load AI knowledge files
     * 
     * @since 4.0.5
     * @updated 6.0.3 - Read from uploads directory (with fallback to plugin dir)
     */
    private function load_configuration_files() {
        $content = '';
        
        // v6.0.3: Primary location is now uploads directory
        $upload_dir = wp_upload_dir();
        $primary_dir = trailingslashit($upload_dir['basedir']) . 'flosc-ai/';
        
        // Fallback to plugin directory (for backwards compatibility / migration)
        $fallback_dir = FLOSC_PLUGIN_DIR . 'ai_configuration_files/';
        
        // Load from primary directory first
        if (is_dir($primary_dir)) {
            $content .= $this->load_files_from_dir($primary_dir);
        }
        
        // Also load from fallback if it exists and has files (migration support)
        if (is_dir($fallback_dir) && !get_option('flosc_knowledge_files_migrated', false)) {
            $fallback_content = $this->load_files_from_dir($fallback_dir);
     ...
     * @since 6.0.3
     */
    private function load_files_from_dir($dir) {
        if (!is_dir($dir)) {
            return '';
        }
