...
<form method="post" class="ivr-message-form">
    ...
    <table class="form-table">
        ...
        <tr>
            ...
            <td>
                <select name="message_type" id="message_type" required onchange="toggleMessageTypeFields(this.value)">
                    ...
                    <option value="suggested_user_autoprompt" <?php selected($msg['type'] ?? '', 'suggested_user_autoprompt'); ?>>Suggested User AutoPrompt (user input button)</option>
        ...
        <!-- Suggested User AutoPrompt Fields -->
        <tr class="suggested-user-autoprompt-field" style="display: <?php echo ($msg['type'] ?? '') === 'suggested_user_autoprompt' ? 'table-row' : 'none'; ?>;">
...
function toggleMessageTypeFields(type) {
    const suggestedUserAutopromptFields = document.querySelectorAll('.suggested-user-autoprompt-field');
    suggestedUserAutopromptFields.forEach(field => {
        field.style.display = (type === 'suggested_user_autoprompt') ? 'table-row' : 'none';
    });
}
