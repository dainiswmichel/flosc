...
class floscApp {
    ...
    floscRenderUserAutoPrompts(replies) {
        ...
        const buttonsHtml = replies.map(r => {
            ...
            return `
            <button class="flosc-style-pill${offerClass}" data-message="${this.escapeHtml(r.name)}"${offerAttr}>
        ...
        container.querySelectorAll('button.flosc-style-pill, button.flosc-style-chip, button.flosc-style-button, button.flosc-style-card, button.flosc-style-feature').forEach(btn => {
            const handler = () => {
                ...
                if (offerId) {
                    // v3.0.5: Offer pill — show a chat bubble then trigger the offer
    ...
    async startInChatQuiz(quizId = 'default') {
        ...
        try {
            ...
            const response = await this.authFetch(`${this.config.apiUrl}/quiz?id=${quizId}`, { credentials: 'same-origin' });
            const data = await response.json();
    ...
    openCheckout(offerId) {
        ...
        // v1.6.9: Support PayPal + Stripe (show modal for either/both)
        if (this.config.stripeKey || this.config.paypalClientId) {
            ...
            // Show payment modal (handles both Stripe and PayPal)
                ...
                createOrder: async () => {
                    ...
                    try {
                        ...
                        if (!data.order_id && (data.code === 'rest_cookie_invalid_nonce' || (data.message || '').match(/cookie|not allowed/i))) {
                            this.log('[FLOSC] PayPal create-order nonce failed, refreshing...');
                        ...
                        // v1.7.6: Show error both in card-errors AND inside PayPal container
