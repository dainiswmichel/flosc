...
class FLOSC_Session_Manager {
     ...
     */
    public function create_session($user_id, $title = 'New Chat') {
        $sessions = get_user_meta($user_id, $this->meta_key, true);
        ...
        // v6.0.1: Use cryptographically random session ID to prevent enumeration
        // Format: Unix timestamp + random suffix for uniqueness and rough ordering
        $session_id = time() . '_' . wp_generate_password(8, false, false);
        
        // Ensure uniqueness (very unlikely collision, but check anyway)
        $existing_ids = array_column($sessions, 'id');
        $attempts = 0;
        while (in_array($session_id, $existing_ids) && $attempts < 5) {
            $session_id = time() . '_' . wp_generate_password(8, false, false);
            $attempts++;
        }
        ...
        $sessions[] = $session;
        update_user_meta($user_id, $this->meta_key, $sessions);
     ...
     */
    public function get_session($session_id, $user_id = null) {
        if (!$user_id) {
     ...
     */
    public function add_message($session_id, $role, $content, $user_id = null) {
        if (!$user_id) {
            $user_id = get_current_user_id();
        }
        ...
        foreach ($sessions as &$session) {
            if ($session['id'] == $session_id) {
                ...
                if ($session['title'] === 'New Chat' && $role === 'user') {
                    $session['title'] = $this->generate_title($content);
                }
    ...
    public function delete_session($session_id, $user_id = null) {
        ...
        update_user_meta($user_id, $this->meta_key, array_values($sessions));
        return true;
    }
