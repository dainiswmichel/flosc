 ...
 * v9.3.6: Quiz scoring fix, PromptPanel for logged-in, preset CSS fixes
 */

// v9.3.6: Clear FLOSC-specific localStorage on version change
(function() {
    const FLOSC_JS_VERSION = '9.3.6';
    try {
...
class floscApp {
    ...
    initCarousel(container) {
        ...
        // v9.3.5: True infinite carousel - clone items for seamless loop
        const cloneCount = Math.min(items.length, 3);
        
        // Clone first items and append to end
        for (let i = 0; i < cloneCount; i++) {
            const clone = items[i].cloneNode(true);
            clone.classList.add('flosc-carousel-clone');
            track.appendChild(clone);
        }
        
        // Clone last items and prepend to start
        for (let i = items.length - 1; i >= Math.max(0, items.length - cloneCount); i--) {
            const clone = items[i].cloneNode(true);
            clone.classList.add('flosc-carousel-clone');
            track.insertBefore(clone, track.firstChild);
        }

        // Re-attach click handlers to clones
        track.querySelectorAll('.flosc-carousel-clone').forEach(clone => {
            const msgName = clone.dataset?.message;
            if (msgName) {
                clone.addEventListener('click', () => this.handleSuggestedReply(msgName));
            }
        });

        // Calculate item width and initial position
        const getItemWidth = () => items[0].offsetWidth + 10; // width + gap
        let itemWidth = getItemWidth();
        const initialOffset = cloneCount * itemWidth;
        
        // Set initial scroll position (after prepended clones)
        track.scrollLeft = initialOffset;

        let isScrolling = false;

        // Reset position when reaching cloned areas
        const handleInfiniteScroll = () => {
            if (isScrolling) return;
            
            itemWidth = getItemWidth(); // Recalculate in case of resize
            const scrollLeft = track.scrollLeft;
            const maxScroll = track.scrollWidth - track.clientWidth;
            const jumpThreshold = itemWidth * 0.5;

            if (scrollLeft < jumpThreshold) {
                isScrolling = true;
                track.style.scrollBehavior = 'auto';
                track.scrollLeft = maxScroll - initialOffset - jumpThreshold;
                track.style.scrollBehavior = 'smooth';
                setTimeout(() => isScrolling = false, 50);
            } else if (scrollLeft > maxScroll - jumpThreshold) {
                isScrolling = true;
                track.style.scrollBehavior = 'auto';
                track.scrollLeft = initialOffset + jumpThreshold;
                track.style.scrollBehavior = 'smooth';
                setTimeout(() => isScrolling = false, 50);
            }
        };
        ...
        const scrollNext = () => {
            ...
            setTimeout(handleInfiniteScroll, 350);
        };
...
Purchased: ${ctx.purchased}
    ...
    bindElements() {
        ...
        // Ensure fallback for missing elements (log for debugging)
        if (!this.chatMessages) {
            console.error('[FLOSC] chatMessages element (#flosc_output_chat_responses) NOT FOUND!');
        }
        if (!this.chatInput) {
            console.error('[FLOSC] chatInput element (#flosc_input_chat_field) NOT FOUND!');
        }
        if (!this.sendBtn) {
            console.error('[FLOSC] sendBtn element (#flosc_input_chat_send_button) NOT FOUND!');
        }
        if (!this.voiceBtn) {
            console.error('[FLOSC] voiceBtn element (#flosc_input_chat_voice_button) NOT FOUND!');
        }
