// Handle manual permalink resave (safe flush)
if (isset($_POST['flosc_resave_permalinks']) && check_admin_referer('flosc_resave_permalinks_nonce')) {
    flush_rewrite_rules(false); // Safe flush, do not hard flush
    echo '<div class="notice notice-success"><p>✓ Permalinks resaved successfully. Your app should now be accessible.</p></div>';
}
<?php
/**
...
<div style="background: <?php echo $rules_exist ? '#d1fae5' : '#fef3c7'; ?>; border: 1px solid <?php echo $rules_exist ? '#10b981' : '#f59e0b'; ?>; border-radius: 6px; padding: 16px 20px; margin-bottom: 24px;">
    ...
    <?php if (!$rules_exist): ?>
        <form method="post" style="display: inline;">
            <?php wp_nonce_field('flosc_resave_permalinks_nonce'); ?>
            <button type="submit" name="flosc_resave_permalinks" class="button button-primary">
                🔄 Resave Permalinks
            </button>
        </form>
        <p class="description" style="margin-top: 8px;">Click to resave permalinks if your app URL is not working. This is safe and takes 1 second.</p>
    <?php endif; ?>
</div>
