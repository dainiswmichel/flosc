...
class FLOSC_Framework {
    ...
    private function load_dependencies() {
        // Core components
        require_once FLOSC_PLUGIN_DIR . 'includes/class-flow-manager.php'; // v1.2.4: Multi-flow system
...
The {product_name} Team";
    ...
    /**
     * v1.1.9: Handle custom domain mapping
     * 
     * If a custom domain is configured (e.g., flosc.ai) and the current request
     * is from that domain, set the flosc_app query var to render the FLOSC app.
     * 
     * This allows FLOSC admins to point any domain to their WordPress install
     * and have it serve the FLOSC app directly.
     * 
     * Server requirements:
    ...
    public function handle_custom_domain() {
        ...
        $custom_domain = strtolower(trim($custom_domain));
        ...
        if ($current_host === $custom_domain || $current_host === 'www.' . $custom_domain) {
            // Set query var so handle_app_route() will render the app
    ...
    /**
     * v1.2.1: Check if current request is for the FLOSC app
     * 
     * Returns true if:
     * - Request is via custom domain (from flosc_custom_domain setting)
     * - Request is via slug route (flosc_app query var)
     * 
     * This is the single source of truth for "should we render FLOSC?"
     * Used by handle_app_route() and enqueue_assets()
     */
    public function is_flosc_request() {
        // Check 1: Standard slug-based routing
        if (get_query_var('flosc_app')) {
            return true;
        }
        
        // Check 2: Custom domain routing (from admin settings)
        $custom_domain = get_option('flosc_custom_domain', '');
        if (!empty($custom_domain)) {
            $custom_domain = strtolower(preg_replace('#^https?://#', '', trim($custom_domain)));
            $custom_domain = rtrim($custom_domain, '/');
            $current_host = strtolower($_SERVER['HTTP_HOST'] ?? '');
    ...
    /**
     * v1.1.9: Get the appropriate app URL (custom domain or slug-based)
     */
    ...
    public function add_admin_menu() {
        // v1.2.4: Main FLOSC menu - clicking goes to Flows page
        // Non-admin users with flow access see the menu
        $user_flows = flosc_flows()->get_user_flows();
        $can_see_menu = current_user_can('manage_options') || !empty($user_flows);
        
        if (!$can_see_menu) {
            return; // User has no access to any flows
        }
        
        // Determine capability for main menu
        $main_capability = current_user_can('manage_options') ? 'manage_options' : 'edit_others_posts';
        ...
        add_submenu_page(
            ...
            $main_capability,
            'flosc-flow-edit',
            [$this, 'render_flow_edit_page']
        );

        // Admin-only submenu items (Global Settings)
        if (current_user_can('manage_options')) {
            ...
            // 14. IVR Editor (dedicated page - hidden from menu)
            add_submenu_page(
    ...
    /**
     * Render Flows Page (v1.2.4)
     */
    public function render_flows_page() {
        require_once FLOSC_PLUGIN_DIR . 'admin/flows.php';
    }
    
    /**
     * Render Flow Edit Page (v1.2.4)
     */
    public function render_flow_edit_page() {
        require_once FLOSC_PLUGIN_DIR . 'admin/flow-edit.php';
    }
    ...
    public function enqueue_admin_assets($hook) {
        ...
        if (strpos($hook, 'flosc') === false && $hook !== 'toplevel_page_flosc-settings') {
            return;
        }
