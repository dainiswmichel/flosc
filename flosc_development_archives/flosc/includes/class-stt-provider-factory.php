...
class FLOSC_STT_Provider_Factory {
    ...
    private function assemblyai_transcribe($audio_path, $options = []) {
        ...
        if (is_wp_error($upload_response)) {
            error_log('FLOSC AssemblyAI Upload Error: ' . $upload_response->get_error_message());
    ...
    private function openai_whisper_transcribe($audio_path, $options = []) {
        ...
        if (is_wp_error($response)) {
            error_log('FLOSC Whisper Error: ' . $response->get_error_message());
            return $response;
        }
    ...
    private function deepgram_transcribe($audio_path, $options = []) {
        ...
        if (is_wp_error($response)) {
            error_log('FLOSC Deepgram Error: ' . $response->get_error_message());
            return $response;
        }
