# FLOSC v1.1.8 - OFFER Phase Enhancement
...
### 🎯 Comprehensive OFFER Phase System

The OFFER phase now supports multiple display formats, configurable in admin and via IVR messages.
...
| `featured` | Large card with features list | Main OTO presentation |
| `text` | Simple text with link | Subtle mentions |
| `inline-checkout` | Stripe form in chat | Frictionless purchase |

#### Admin Configuration (Offers Tab):
- **Display Format** selector per offer
- **Icon/Emoji** customization
- **Badge Text** (e.g., "Most Popular", "50% OFF")
- **Savings Text** (e.g., "Save $50!")
- **Guarantee Text** for featured cards

#### IVR Message Support:
```markdown
## My Offer
MessageName: my_offer_001
MessageType: offer
OfferID: full_access
DisplayFormat: featured  ← NEW
Icon: 🎁
Timer: 3600
MessageContent: Your offer content here...
MessageConditions: is_guest
```

### 💳 Enhanced Checkout Flow
- **Inline Checkout**: Stripe card form embedded directly in chat
- **Payment Success**: Auto-refresh after successful payment
- **External Redirects**: Support for ClickBank and other external checkouts
- **Return State**: Pending purchase tracking for redirect flows

### 🔧 Technical Improvements
- `credentials: 'same-origin'` on all fetch calls
- Offer tracking with display format metadata
- Configurable offer timer (seconds)
- Panel offers rendering method
