/**
 * FLOSC Framework - JavaScript State Machine v2.0.3
 *
...
class FLOSC {
    constructor() {
        ...
        this.data = {
            ...
            noCount: 0, // v2.0.3: Track "no" responses
            offerExpiresAt: null, // v2.0.3: Offer expiry timestamp
        };
    ...
    async init() {
        ...
        // v2.0.3: Handle referral tracking
        if (this.config.enableReferralTracking && this.config.referralCode && this.config.isLoggedIn) {
            await this.saveReferralCode();
        }

        // v2.0.3: Check for cooldown
        if (this.config.enableCooldown && this.config.isCooledDown) {
            await this.showCooldownMessage();
            return;
        }
    ...
    async runOffer() {
        ...
        // v2.0.3: Start countdown timer if enabled
        if (this.config.enableOfferExpiry) {
            const expiryMinutes = this.config.offerExpiryMinutes;
            this.data.offerExpiresAt = Date.now() + (expiryMinutes * 60 * 1000);
            this.saveSession();
            await this.botMessage(`⏰ This offer expires in ${expiryMinutes} minutes`);
            this.startCountdown();
        }

        this.showReplies([
            { text: `Buy Now (${price})`, action: () => this.transition('SALE'), id: 'buy-button' },
            { text: 'Maybe later', action: () => this.handleNoResponse() },
        ]);
    }
    ...
    showReplies(buttons) {
        ...
        buttons.forEach(btn => {
            ...
            if (btn.id) {
                btnEl.id = btn.id; // v2.0.3: Support button IDs
            }
    ...
    saveSession() {
        const sessionData = {
            ...
            noCount: this.data.noCount, // v2.0.3
            offerExpiresAt: this.data.offerExpiresAt, // v2.0.3
        };
    ...
    async restoreSession() {
        ...
        try {
            ...
            this.data.noCount = session.noCount || 0; // v2.0.3
            this.data.offerExpiresAt = session.offerExpiresAt; // v2.0.3
        } catch (error) {
    ...
    /**
     * Save referral code when user logs in
     */
    async saveReferralCode() {
        try {
            const response = await fetch(this.config.restUrl + '/save-referral', {
                method: 'POST',
                headers: {
            ...
            const result = await response.json();
            if (result.success) {
                console.log('FLOSC: Referral code saved:', result.referral_code);
            }
        } catch (error) {
            console.error('FLOSC: Error saving referral:', error);
        }
    }

    // ========================================================================
    // v2.0.3: COOLDOWN LOGIC
    // ========================================================================

    /**
     * Show cooldown message and disable quiz
     */
    async showCooldownMessage() {
        const endsAt = this.config.cooldownEndsAt;
        const now = Math.floor(Date.now() / 1000);
        const hoursLeft = Math.ceil((endsAt - now) / 3600);

        const message = this.config.cooldownMessage.replace('{hours}', hoursLeft);
        await this.botMessage(message);
        await this.botMessage('Take a break and come back later! 😊');
    }

    /**
     * Handle "no" response (Maybe later, etc)
     * Tracks count and applies cooldown if threshold reached
     */
    async handleNoResponse() {
        await this.botMessage('No problem! Come back anytime.');
        ...
        try {
            ...
            if (result.cooldown_applied) {
                await this.botMessage('⏱️ You\'ve declined a few times. Take some time to think it over!');
                const hours = this.config.cooldownDurationHours;
                await this.botMessage(`Come back in ${hours} hours to try again.`);

                // Disable further interaction
                this.clearReplies();
            }
        } catch (error) {
            console.error('FLOSC: Error tracking no response:', error);
        }
    }

    // ========================================================================
    // v2.0.3: OFFER EXPIRY COUNTDOWN
    // ========================================================================

    /**
     * Start countdown timer for offer expiry
     */
    startCountdown() {
        ...
        this.countdownInterval = setInterval(() => {
            const now = Date.now();
            const timeLeft = this.data.offerExpiresAt - now;
    ...
    /**
     * Update countdown display in status text
     */
    updateCountdownDisplay(timeLeft) {
        ...
        this.elements.statusText.textContent = display;
    }
    ...
    async handleOfferExpired() {
        ...
        await this.botMessage('Refresh the page to start over.');

        this.elements.statusText.textContent = 'Offer expired';
    }
}
