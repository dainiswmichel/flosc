...
class FLOSC_WooCommerce_Hooks {
     ...
     * v2.0.3: Now includes referral tracking
     *
     * @param int $user_id
     * @param int $order_id
    ...
    private function log_purchase($user_id, $order_id, $method) {
        ...
        // v2.0.3: Get referral code if available
        $referral_code = get_user_meta($user_id, 'flosc_referral_code', true);
        ...
        // v2.0.3: Add referral if present
        if ($referral_code) {
            $purchase_data['referral_code'] = $referral_code;

            // Mark conversion in user meta
            update_user_meta($user_id, 'flosc_referral_converted', '1');
            update_user_meta($user_id, 'flosc_referral_conversion_order', $order_id);

            error_log("FLOSC: Referral conversion tracked - {$referral_code} → user {$user_id} → order {$order_id}");
        }
