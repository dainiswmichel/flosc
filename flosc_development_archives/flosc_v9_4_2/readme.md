# FLOSC v9.4.2 - Security Hardening Release

**Freeline-Login-Offer-Sale-Content** - A WordPress framework for quiz-based learning and conversational sales funnels.

**Version:** 9.4.2  
**Status:** ✅ PRODUCTION READY - Security hardened with signed cookies and rate limiting  
...
**Signed Cookies (P1 - Critical)**
- Pre-login quiz scores now use HMAC-signed cookies
- Prevents score forgery by malicious users
- Uses WordPress AUTH_SALT for cryptographic signing
- New helper methods: `sign_cookie_data()`, `verify_signed_cookie()`, `set_signed_cookie()`, `get_signed_cookie()`

**Rate-Limited Public Endpoints (P1 - Critical)**
- All public REST endpoints now use `check_public_endpoint_permission()`
- Logged-in users: 60 requests/hour per endpoint
- Visitors: 30 requests/hour per endpoint
...
- `chat-style-light.css` / `chat-style-dark.css` - Variable definitions only

---

## Previous Versions

### v9.3.9 - Clean CSS Architecture
