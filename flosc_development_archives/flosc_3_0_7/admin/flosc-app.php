...
<body class="flosc-app<?php echo is_admin_bar_showing() ? ' admin-bar' : ''; ?>"
    ...
    <script>
        ...
        window.FLOSC_CONFIG = <?php 
            ...
            // v3.0.7: Admin-configured AutoPromptPanel pills (from AutoPromptPanel tab in WP dashboard)
            // Always merged with IVR-defined pills so admins can add/remove without editing IVR files
            'adminAutoprompts' => (function() use ($flow_settings) {
                $raw     = $flow_settings['autoprompts'] ?? [];
                $headers = $flow_settings['autoprompt_headers'] ?? [];
                ...
                foreach ($states as $s) {
                    foreach (($raw[$s] ?? []) as $i => $p) {
                        ...
                        $out[$name] = [
                            ...
                            '_pill_label'  => $p['label']        ?? $p['user_input'] ?? '',
