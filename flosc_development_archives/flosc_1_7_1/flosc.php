...
The {product_name} Team";
    ...
    public function enqueue_assets() {
        ...
        // v1.6.9: PayPal JS SDK if configured
...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7: [link]. It has the complete IPA guide and video walkthrough. Ready to dive in?\"",
    ...
    public function paypal_capture_order($request) {
        ...
        // Verify the captured order belongs to this user
        $custom_id = $capture_result['custom_id'] ?? '';
        if ($custom_id) {
            $parts = explode('|', $custom_id);
            $captured_user_id = isset($parts[0]) ? intval(str_replace('user:', '', $parts[0])) : 0;
            if ($captured_user_id && $captured_user_id !== $user_id) {
                return new WP_Error('user_mismatch', 'Payment does not belong to this user', ['status' => 403]);
            }
        }
