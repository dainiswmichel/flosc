    ...
    async submitRecording() {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.analysis) {
                    this.addMessage('assistant', result.analysis.feedback);
                    
                    // If there's a free lesson, offer it
                    if (result.analysis.free_lesson) {
                        setTimeout(() => {
                            this.addMessage('assistant', `Ready to try your free lesson on "${result.analysis.free_lesson.lesson}"? This will help you master the sound you missed.`);
                        }, 1500);
                    }
                    
                    // If score is not perfect, offer upgrade
                    if (result.analysis.score < 100 && this.state === 'free') {
                        setTimeout(() => {
                            this.addMessage('assistant', `Want to unlock all lessons and master every sound? Upgrade to get full access! 🚀`);
                        }, 3000);
                    }
                } else {
