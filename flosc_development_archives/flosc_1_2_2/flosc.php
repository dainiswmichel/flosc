...
The {product_name} Team";
    ...
    public function add_rewrite_rules() {
        // v1.2.2: Register rewrite rules for all active flows
        $flows = flosc_flows()->get_all_flows();
        
        foreach ($flows as $flow) {
            if (!empty($flow['slug']) && $flow['status'] === 'active') {
                add_rewrite_rule(
                    '^' . preg_quote($flow['slug'], '/') . '/?$',
                    'index.php?flosc_app=1&flosc_flow=' . $flow['id'],
                    'top'
                );
            }
        }
        
        // Fallback: legacy slug from settings (if no flows defined yet)
        if (empty($flows)) {
     ...
     */
    /**
     * v1.2.2: Get the current flow based on request (slug or custom domain)
    ...
    public function add_admin_menu() {
        // v1.2.2: Check if user has access to any flows
        $user_flows = flosc_flows()->get_user_flows();
        $has_flow_access = !empty($user_flows) || current_user_can('manage_options');
        
        // Main FLOSC menu - now goes to Flows list
        add_menu_page(
            'FLOSC',
            'FLOSC',
            $has_flow_access ? 'edit_others_posts' : 'manage_options',
            'flosc-flows',
        ...
        // 1. Flows (visible to Editors with flow access)
        add_submenu_page(
            'flosc-flows',
            'Flows',
            'Flows',
        ...
        // Hidden: Flow Edit page (accessible via direct URL)
        add_submenu_page(
        ...
        // 2. Settings (all tabs - visible to admins only)
        add_submenu_page(
            'flosc-flows',
            'Settings',
            'Settings',
        ...
        // 3. Product (redirect to Settings > Product tab)
        add_submenu_page(
            'flosc-flows',
            'Product Settings',
            'Product',
    ...
    public function enqueue_admin_assets($hook) {
        ...
        // v1.2.2: Include flosc-flows pages
        if (strpos($hook, 'flosc') === false && 
            $hook !== 'toplevel_page_flosc-settings' && 
            $hook !== 'toplevel_page_flosc-flows') {
            return;
        }
