...
<table class="form-table">
    <tr>
        <th scope="row"><label for="flosc_email_on_quiz_complete">Send on Quiz Completion</label></th>
        <td>
            <label>
                <input type="checkbox" id="flosc_email_on_quiz_complete" name="flosc_email_on_quiz_complete" 
                       value="1" <?php checked(get_option('flosc_email_on_quiz_complete', true)); ?>>
    ...
    <tr>
        <th scope="row"><label for="flosc_email_congrats_threshold">Congratulations Email Threshold</label></th>
        <td>
            <input type="number" id="flosc_email_congrats_threshold" name="flosc_email_congrats_threshold" 
                   value="<?php echo esc_attr(get_option('flosc_email_congrats_threshold', 80)); ?>" 
    ...
    <tr>
        <th scope="row"><label for="flosc_email_encouragement_threshold">Encouragement Email Threshold</label></th>
        <td>
            <input type="number" id="flosc_email_encouragement_threshold" name="flosc_email_encouragement_threshold" 
                   value="<?php echo esc_attr(get_option('flosc_email_encouragement_threshold', 60)); ?>" 
                   min="0" max="100" class="small-text"> %
    ...
    <tr>
        <th scope="row"><label for="flosc_email_reengagement_days">Re-engagement Email</label></th>
        <td>
            <input type="number" id="flosc_email_reengagement_days" name="flosc_email_reengagement_days" 
                   value="<?php echo esc_attr(get_option('flosc_email_reengagement_days', 7)); ?>" 
                   min="1" class="small-text"> days of inactivity
            <label style="margin-left: 15px;">
                <input type="checkbox" name="flosc_email_reengagement_enabled" value="1" 
                       <?php checked(get_option('flosc_email_reengagement_enabled', false)); ?>>
                Enable
            </label>
    ...
    <tr>
        <th scope="row"><label for="flosc_email_weekly_summary">Weekly Progress Summary</label></th>
        <td>
            <label>
                <input type="checkbox" id="flosc_email_weekly_summary" name="flosc_email_weekly_summary" 
                       value="1" <?php checked(get_option('flosc_email_weekly_summary', false)); ?>>
...
function flosc_send_quiz_results_email($user_email, $quiz_data) {
    if (!get_option('flosc_email_on_quiz_complete', true)) return;
    
    $score = $quiz_data['score'];
    $congrats_threshold = get_option('flosc_email_congrats_threshold', 80);
    $encouragement_threshold = get_option('flosc_email_encouragement_threshold', 60);
    ...
    $subject = get_option('flosc_email_subject', '');
    $body = get_option('flosc_email_body', '');
    ...
    $placeholders = [
        ...
        '{app_link}' => home_url('/' . get_option('flosc_app_slug') . '/'),
...
function flosc_send_reengagement_emails() {
    if (!get_option('flosc_email_reengagement_enabled', false)) return;
    
    $days = get_option('flosc_email_reengagement_days', 7);
...
<table class="form-table">
    <tr>
        <th scope="row"><label for="flosc_email_provider">Email Provider</label></th>
        <td>
            <select id="flosc_email_provider" name="flosc_email_provider" disabled>
    ...
    <tr>
        <th scope="row"><label for="flosc_email_from_name">From Name</label></th>
        <td>
            <input type="text" id="flosc_email_from_name" name="flosc_email_from_name" 
                   value="<?php echo esc_attr(get_option('flosc_email_from_name', get_option('flosc_product_name', 'FLOSC App'))); ?>" 
                   class="regular-text">
    ...
    <tr>
        <th scope="row"><label for="flosc_email_from_address">From Email Address</label></th>
        <td>
            <input type="email" id="flosc_email_from_address" name="flosc_email_from_address" 
                   value="<?php echo esc_attr(get_option('flosc_email_from_address', get_option('admin_email'))); ?>" 
                   class="regular-text">
