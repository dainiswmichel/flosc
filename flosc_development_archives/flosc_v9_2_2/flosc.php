...
The {product_name} Team";
    ...
    public function register_settings() {
        ...
        // Chat Style Settings (v9.0.6)
        register_setting('flosc_settings', 'flosc_chat_style_preset');
        register_setting('flosc_settings', 'flosc_chat_style_font');
        register_setting('flosc_settings', 'flosc_chat_style_scale');
        register_setting('flosc_settings', 'flosc_chat_style_theme');
        register_setting('flosc_settings', 'flosc_chat_style_custom_css');
    }
    ...
    public function enqueue_assets() {
        ...
        // Our assets
        // v07.07: Use filemtime() for automatic cache busting on file changes
        wp_enqueue_style('flosc-app', FLOSC_PLUGIN_URL . 'assets/css/flosc-app.css', [], filemtime(FLOSC_PLUGIN_DIR . 'assets/css/flosc-app.css'));

        // v9.0.5: Chat styling system
    ...
    /**
     * Enqueue chat styling (v9.0.5)
     */
    private function enqueue_chat_style() {
        // Get settings
        $preset = get_option('flosc_chat_style_preset', 'flosc');
        $font = get_option('flosc_chat_style_font', 'system');
        $scale = get_option('flosc_chat_style_scale', 100);
        $theme = get_option('flosc_chat_style_theme', 'default');
        $custom_css = get_option('flosc_chat_style_custom_css', '');

        // Enqueue preset CSS
        $preset_file = "chat-style-{$preset}.css";
        $preset_path = FLOSC_PLUGIN_DIR . 'assets/css/' . $preset_file;

        if (file_exists($preset_path)) {
            wp_enqueue_style(
                'flosc-chat-style',
                FLOSC_PLUGIN_URL . 'assets/css/' . $preset_file,
                ['flosc-app'],
                filemtime($preset_path)
            );
        }

        // Build inline CSS with variable overrides
        $inline_css = ":root {\n";

        // Scaling
        if ($scale != 100) {
            $inline_css .= "    --flosc-scale: {$scale}%;\n";
        }

        $inline_css .= "}\n";

        // Font attribute for HTML element
        if ($font !== 'system') {
            $inline_css .= "[data-flosc-font=\"{$font}\"] .messages {\n";
            $inline_css .= "    font-family: var(--flosc-font-family);\n";
            $inline_css .= "}\n";
        }

        // Theme attribute
        if ($theme !== 'default') {
            $inline_css .= "[data-flosc-theme=\"{$theme}\"] .messages {\n";
            $inline_css .= "    /* Theme applied via CSS variables */\n";
            $inline_css .= "}\n";
        }
        ...
        // Add inline styles
        if (!empty($inline_css)) {
            wp_add_inline_style('flosc-chat-style', $inline_css);
        }
    }
...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7: [link]. It has the complete IPA guide and video walkthrough. Ready to dive in?\"",
    ...
    /**
     * Get IVR messages for current phase and context (v9.2.2)
     */
 ...
 */
/**
 * Import IVR messages with smart merge strategy
 * 
 * @param string $mode 'merge' (default) or 'replace'
 * @return array ['success' => bool, 'stats' => array, 'message' => string]
 */
function flosc_import_ivr_to_database($mode = 'merge') {
    ...
    // AUTO-BACKUP: Save current database state before import
    $backup_result = flosc_export_ivr_backup();
    
    // Get current database state
    $current_messages = get_option('flosc_ivr_messages', []);
    $current_phases = get_option('flosc_ivr_phases', []);
    
    $incoming_messages = $config['messages'] ?? [];
    $incoming_phases = $config['phases'] ?? [];
    ...
    if ($mode === 'replace') {
        // REPLACE MODE: Delete everything and use only incoming data
        $final_messages = $incoming_messages;
        $final_phases = $incoming_phases;
        
        // Track deletions
        foreach ($current_messages as $msg_id => $msg_data) {
            if (!isset($incoming_messages[$msg_id])) {
                $stats['deleted'][] = $msg_id;
            }
        }
        
    } else {
        // MERGE MODE (default): Smart merge preserving database-only messages
        $final_messages = $current_messages;
        $final_phases = $current_phases;
        
        // Add/Update from incoming
        foreach ($incoming_messages as $msg_id => $msg_data) {
            if (isset($current_messages[$msg_id])) {
                // Update existing message
                $stats['updated'][] = $msg_id;
            } else {
                // Add new message
                $stats['added'][] = $msg_id;
            }
            $final_messages[$msg_id] = $msg_data;
        }
        
        // Preserve database-only messages
        foreach ($current_messages as $msg_id => $msg_data) {
            if (!isset($incoming_messages[$msg_id])) {
                $stats['preserved'][] = $msg_id;
                // Already in final_messages, just track it
            }
        }
        
        // Merge phases
        foreach ($incoming_phases as $phase_name => $message_ids) {
            if (!isset($final_phases[$phase_name])) {
                $final_phases[$phase_name] = [];
            }
            // Add incoming messages to phase
            foreach ($message_ids as $msg_id) {
                if (!in_array($msg_id, $final_phases[$phase_name])) {
                    $final_phases[$phase_name][] = $msg_id;
                }
            }
        }
        
        // Preserve database-only phase assignments
        foreach ($current_phases as $phase_name => $message_ids) {
            foreach ($message_ids as $msg_id) {
                if (isset($final_messages[$msg_id]) && !in_array($msg_id, $final_phases[$phase_name] ?? [])) {
                    if (!isset($final_phases[$phase_name])) {
                        $final_phases[$phase_name] = [];
                    }
                    $final_phases[$phase_name][] = $msg_id;
    ...
    update_option('flosc_ivr_last_import_mode', $mode);
    update_option('flosc_ivr_last_import_stats', $stats);
    
    // Generate summary message
    $message = $mode === 'replace' ? 'Full replacement import' : 'Smart merge import';
    $message .= sprintf(
        ' - Added: %d, Updated: %d, Preserved: %d, Deleted: %d',
        count($stats['added']),
        count($stats['updated']),
        count($stats['preserved']),
        count($stats['deleted'])
    );
    if ($backup_result) {
...
MD;
    ...
    // v9.2.2: Import IVR messages to database (safe merge mode on first activation)
    $result = flosc_import_ivr_to_database('merge');
