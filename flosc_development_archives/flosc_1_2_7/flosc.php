...
The {product_name} Team";
    ...
    /**
     * v1.2.3: Get the current flow based on request (slug or custom domain)
     * 
     * This is the single source of truth for "should we render FLOSC?"
     * Returns flow config array or null if no match.
     * 
     * Used by is_flosc_request(), handle_app_route(), enqueue_assets(), get_product_config()
     */
    public function get_current_flow() {
        // v1.2.7: Delegate to Flow Manager for consistency
        return flosc_flows()->get_current_flow_frontend();
    }
    ...
    public function add_admin_menu() {
        ...
        add_submenu_page(null, 'Edit Flow', '', $has_flow_access ? 'edit_others_posts' : 'manage_options', 'flosc-flow-edit', [$this, 'render_flow_edit_page']);
        add_submenu_page(null, 'IVR Editor', '', 'manage_options', 'flosc-ivr', [$this, 'render_ivr_editor']);
    }
...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7: [link]. It has the complete IPA guide and video walkthrough. Ready to dive in?\"",
    ...
    public function get_ivr_messages($request) {
        ...
        // v1.2.3: Get IVR data from current flow's IVR file (not global cache)
        $parser = FLOSC_IVR_Parser::flosc_instance();
        $config = $parser->flosc_load_config(); // This now loads flow-specific IVR
