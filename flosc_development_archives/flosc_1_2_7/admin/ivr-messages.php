<?php
/**
 * FLOSC IVR Messages Tab - Full CRUD Editor (v1.2.6)
 * 
 ...
 * v1.2.6: Flow-aware IVR file selection
 */
...
function flosc_run_ivr_diagnostics() {
    ...
    if (is_wp_error($api_response)) {
        ...
        update_option('flosc_api_last_check', $michel_timestamp);
    } else {
    ...
    if ($uploaded_file['error'] === UPLOAD_ERR_OK) {
        ...
        if (pathinfo($filename, PATHINFO_EXTENSION) !== 'md') {
            ...
            if (move_uploaded_file($uploaded_file['tmp_name'], $target_path)) {
                update_option('flosc_active_ivr_file', $filename);
...
if (isset($_POST['flosc_change_active_file']) && isset($_POST['ivr_file_select'])) {
    ...
    if (file_exists($file_path)) {
        ...
        if ($editing_flow_id) {
            ...
            // Fallback to global
            update_option('flosc_active_ivr_file', $selected_file);
...
if ($editing_flow_data && !empty($editing_flow_data['ivr_file'])) {
    ...
    $active_ivr_file = get_option('flosc_active_ivr_file', 'flosc_default_ivr.md');
}
...
<div class="flosc-diagnostics-panel" style="background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 8px; padding: 20px; margin-bottom: 25px;">
    <h3 style="margin-top: 0; display: flex; align-items: center; gap: 10px;">
        ...
        <?php if ($editing_flow_data): ?>
            <span style="font-size: 12px; font-weight: normal; background: #dbeafe; color: #1e40af; padding: 2px 8px; border-radius: 4px;">
                Flow: <?php echo esc_html($editing_flow_data['product']['name'] ?? $editing_flow_id); ?>
            </span>
        <?php endif; ?>
    </h3>
    ...
    <div style="background: #e7f3ff; border: 1px solid #b3d7ff; border-radius: 6px; padding: 15px; margin-bottom: 20px;">
        <div style="margin-bottom: 15px;">
            <label style="font-weight: 600; display: block; margin-bottom: 8px;">
                📁 <?php echo $editing_flow_data ? 'Flow IVR File' : 'Active IVR Messages MD file'; ?>:
                <?php if ($active_ivr_file): ?>
                    <code style="margin-left: 8px; background: #dcfce7; color: #166534; padding: 2px 8px; border-radius: 4px;"><?php echo esc_html($active_ivr_file); ?></code>
                <?php endif; ?>
            </label>
            <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                <form method="post" style="display: flex; align-items: center; gap: 10px;">
                    <?php wp_nonce_field('flosc_change_active_file'); ?>
                    <select name="ivr_file_select" style="padding: 8px 12px; border-radius: 4px; border: 1px solid #ccc; min-width: 250px;">
                        <?php foreach ($available_ivr_files as $file): ?>
                        <option value="<?php echo esc_attr($file); ?>" <?php selected($active_ivr_file, $file); ?>>
                            <?php echo esc_html($file); ?>
                            <?php if ($file === $active_ivr_file): ?> ✓<?php endif; ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                    <button type="submit" name="flosc_change_active_file" class="button button-primary">
                        Set as Active
                    </button>
                </form>
                <span style="font-size: 12px; color: #667;">
                    <?php if ($editing_flow_data): ?>
                        Changes will be saved to this flow only.
                    <?php else: ?>
                        Select a file, then click "Set as Active" to change which file is used.
                    <?php endif; ?>
                </span>
            </div>
        </div>
        ...
        <!-- Upload New File -->
        <div style="border-top: 1px dashed #b3d7ff; padding-top: 15px;">
            <label style="font-weight: 600; display: block; margin-bottom: 8px;">
                📤 Upload New IVR Messages MD file:
            </label>
            <form method="post" enctype="multipart/form-data" style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                ...
                <input type="file" name="ivr_file_upload" accept=".md" style="padding: 5px;">
                <button type="submit" name="flosc_upload_ivr_file" class="button button-secondary">
                    Upload & Set as Active
                </button>
                <span style="font-size: 12px; color: #667;">
                    Upload a .md file (filename should start with "ivr", e.g., ivr-french-class.md)
                </span>
            </form>
        </div>
    </div>
