<?php
/**
 * FLOSC Chat Styling Tab v1.2.7
 * 
 * Flow-aware styling with global fallback.
 */
...
// Handle reset action (resets flow-specific overrides)
if (isset($_POST['flosc_reset_chat_style']) && check_admin_referer('flosc_reset_chat_style_nonce')) {
    if ($flow_id) {
        flosc_flows()->save_settings([
            'chat_style_preset' => '',
            'chat_style_bubble' => '',
            'chat_style_accent' => '',
            'chat_style_font' => '',
            'chat_style_scale' => '',
            'chat_style_custom_css' => '',
        ], $flow_id);
        echo '<div class="notice notice-success"><p>✓ Flow styling reset to use global defaults.</p></div>';
    }
}

// v1.2.7: Get values with flow-first/global-fallback
$current_preset = flosc_get_setting('chat_style_preset', 'light', $flow_id);
$current_bubble = flosc_get_setting('chat_style_bubble', 'subtle-notch', $flow_id);
$current_accent = flosc_get_setting('chat_style_accent', '#2563eb', $flow_id);
$current_font = flosc_get_setting('chat_style_font', 'system', $flow_id);
$current_scale = flosc_get_setting('chat_style_scale', 100, $flow_id);
$custom_css = flosc_get_setting('chat_style_custom_css', '', $flow_id);

// Check for flow-specific values
$has_preset = flosc_has_setting('chat_style_preset', $flow_id);
$has_bubble = flosc_has_setting('chat_style_bubble', $flow_id);
$has_accent = flosc_has_setting('chat_style_accent', $flow_id);
$has_font = flosc_has_setting('chat_style_font', $flow_id);
...
<p class="description">Customize the appearance of your chat interface. Empty values use global defaults.</p>

<!-- Reset Button -->
<form method="post" style="margin-bottom: 24px;">
    <?php wp_nonce_field('flosc_reset_chat_style_nonce'); ?>
    <button type="submit" name="flosc_reset_chat_style" class="button" onclick="return confirm('Reset this flow to use global styling?');">
        ↺ Reset to Global
    </button>
</form>
...
<table class="form-table">
    ...
    <tr>
        ...
        <td>
            <input type="color" name="flosc_chat_style_accent" id="flosc_chat_style_accent" value="<?php echo esc_attr($current_accent); ?>" style="width: 60px; height: 36px; padding: 0; border: 1px solid #ccc; border-radius: 4px;">
