...
// v1.2.7: Use flow-aware settings helpers
$flow_id = $GLOBALS['flosc_editing_flow'] ?? null;

$base_prompt = flosc_get_setting('ai_base_prompt', '', $flow_id);
if (empty($base_prompt)) {
    $product_name = $GLOBALS['flosc_editing_flow_data']['product']['name'] ?? 'FLOSC App';
    $base_prompt = "You are " . $product_name . ", an AI assistant. Your mission is to help users through personalized guidance. Be helpful, friendly, specific, and action-oriented.";
}
$ai_provider = flosc_get_setting('ai_provider', 'ivr', $flow_id);
$ai_response_mode = flosc_get_setting('ai_response_mode', 'enhanced', $flow_id);

// API keys (flow can override globals)
$openai_key = flosc_get_setting('openai_api_key', '', $flow_id);
$anthropic_key = flosc_get_setting('anthropic_api_key', '', $flow_id);
$xai_key = flosc_get_setting('xai_api_key', '', $flow_id);

// Check which settings are flow-specific vs global
$has_flow_provider = flosc_has_setting('ai_provider', $flow_id);
$has_flow_openai = flosc_has_setting('openai_api_key', $flow_id);
$has_flow_anthropic = flosc_has_setting('anthropic_api_key', $flow_id);
$has_flow_xai = flosc_has_setting('xai_api_key', $flow_id);
?>
...
<table class="form-table">
    <tr>
        <th scope="row"><label for="fs_ai_provider">Primary AI Provider</label></th>
        <td>
            <select name="fs_ai_provider" id="fs_ai_provider">
                <option value="" <?php selected($ai_provider, ''); ?>>— Use Global Default —</option>
            ...
            <?php if (!$has_flow_provider): echo flosc_global_hint('ai_provider'); endif; ?>
            <p class="description">IVR uses scripted messages only. AI providers require API keys.</p>
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="fs_openai_api_key">OpenAI API Key</label></th>
        <td>
            <input type="password" id="fs_openai_api_key" name="fs_openai_api_key" value="<?php echo esc_attr($openai_key); ?>" class="regular-text" placeholder="Leave empty to use global">
            <?php if (!$has_flow_openai): echo flosc_global_hint('openai_api_key'); endif; ?>
            <p class="description">Get your key at <a href="https://platform.openai.com/api-keys" target="_blank">platform.openai.com</a></p>
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="fs_anthropic_api_key">Anthropic API Key</label></th>
        <td>
            <input type="password" id="fs_anthropic_api_key" name="fs_anthropic_api_key" value="<?php echo esc_attr($anthropic_key); ?>" class="regular-text" placeholder="Leave empty to use global">
            <?php if (!$has_flow_anthropic): echo flosc_global_hint('anthropic_api_key'); endif; ?>
    ...
    <tr>
        <th scope="row"><label for="fs_xai_api_key">xAI API Key</label></th>
        <td>
            <input type="password" id="fs_xai_api_key" name="fs_xai_api_key" value="<?php echo esc_attr($xai_key); ?>" class="regular-text" placeholder="Leave empty to use global">
            <?php if (!$has_flow_xai): echo flosc_global_hint('xai_api_key'); endif; ?>
            <p class="description">Get your key at <a href="https://console.x.ai" target="_blank">console.x.ai</a></p>
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="fs_ai_base_prompt">Base System Prompt</label></th>
        <td>
            <textarea id="fs_ai_base_prompt" name="fs_ai_base_prompt" rows="6" class="large-text"><?php echo esc_textarea($base_prompt); ?></textarea>
...
<table class="form-table">
    <tr>
        <th scope="row"><label for="fs_ai_response_mode">Response Mode</label></th>
        <td>
            <select name="fs_ai_response_mode" id="fs_ai_response_mode">
                <option value="" <?php selected($ai_response_mode, ''); ?>>— Use Global Default —</option>
