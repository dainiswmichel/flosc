...
function flosc_handle_autoprompts_save() {
    ...
    foreach ($states as $state) {
        $panel_headers[$state] = sanitize_text_field($_POST['panel_header_' . $state] ?? 'Try these AutoPrompts!');
    }

    foreach ($states as $state) {
        $icons          = $_POST[$state . '_pill_icon']           ?? [];
        $labels         = $_POST[$state . '_pill_label']          ?? [];
        $inputs         = $_POST[$state . '_pill_user_input']     ?? [];
        $trigger_types  = $_POST[$state . '_pill_trigger_type']   ?? [];
        $trigger_values = $_POST[$state . '_pill_trigger_value']  ?? [];
...
$saved_prompts  = $fs['autoprompts'] ?? [];
$saved_headers  = $fs['autoprompt_headers'] ?? [];
$panel_headers  = [
    'visitor' => $saved_headers['visitor'] ?? 'Try these AutoPrompts!',
    'guest'   => $saved_headers['guest']   ?? 'Try these AutoPrompts!',
    'member'  => $saved_headers['member']  ?? 'You\'re a member! Here\'s what\'s available:',
];
...
$ivr_defaults = [
    ...
    'guest' => [
        ['icon'=>'🎁','label'=>'View my free lesson',       'user_input'=>'View my free lesson!',          'trigger_type'=>'action', 'trigger_value'=>'open_free_lesson',  'conditions'=>'is_guest && quiz_taken && !lesson_viewed',             'style'=>'pill'],
        ['icon'=>'🎁','label'=>'View my free lessons',      'user_input'=>'View my free lessons!',         'trigger_type'=>'action', 'trigger_value'=>'open_free_lesson',  'conditions'=>'is_guest && quiz_taken && free_lessons_count > 1',     'style'=>'pill'],
        ['icon'=>'📊','label'=>'Review my score',           'user_input'=>'Review my score',               'trigger_type'=>'ai',     'trigger_value'=>'',                  'conditions'=>'is_guest && quiz_taken',                              'style'=>'pill'],
        ['icon'=>'🔄','label'=>'Retake my free lesson',     'user_input'=>'Retake my free lesson',         'trigger_type'=>'action', 'trigger_value'=>'open_free_lesson',  'conditions'=>'is_guest && lesson_viewed',                           'style'=>'pill'],
        ['icon'=>'🎁','label'=>'Show me the special offer', 'user_input'=>'Show me the special offer',     'trigger_type'=>'offer',  'trigger_value'=>'full_access',       'conditions'=>'is_guest && !offer_shown_full_access',                'style'=>'pill'],
        ...
        ['icon'=>'🎁','label'=>'What offers are available?','user_input'=>'What offers are available?',    'trigger_type'=>'offer',  'trigger_value'=>'full_access',       'conditions'=>'is_guest && message_count >= 3',                     'style'=>'pill'],
        ['icon'=>'🎉','label'=>'Upgrade for full access',   'user_input'=>'Upgrade for full access',       'trigger_type'=>'action', 'trigger_value'=>'checkout_full_access','conditions'=>'is_guest',                                          'style'=>'button'],
        ['icon'=>'💬','label'=>'What\'s included?',         'user_input'=>'What\'s included?',             'trigger_type'=>'ai',     'trigger_value'=>'',                  'conditions'=>'is_guest && offer_shown_full_access',                 'style'=>'pill'],
    ],
    'member' => [
        ['icon'=>'🎓','label'=>'Start my first lesson',     'user_input'=>'Start my first premium lesson', 'trigger_type'=>'action', 'trigger_value'=>'open_lesson_library','conditions'=>'is_member && first_message_after_purchase',          'style'=>'button'],
        ['icon'=>'📊','label'=>'See my full score',         'user_input'=>'See my full score breakdown',   'trigger_type'=>'ai',     'trigger_value'=>'',                  'conditions'=>'is_member && quiz_taken',                             'style'=>'pill'],
        ['icon'=>'🔄','label'=>'Retake quiz',               'user_input'=>'Retake quiz to set my baseline','trigger_type'=>'action', 'trigger_value'=>'open_quiz',         'conditions'=>'is_member',                                           'style'=>'button'],
        ['icon'=>'📚','label'=>'Browse all lessons',        'user_input'=>'Browse all lessons',            'trigger_type'=>'action', 'trigger_value'=>'open_lesson_library','conditions'=>'is_member',                                          'style'=>'button'],
        ['icon'=>'📊','label'=>'View my complete score',    'user_input'=>'View my complete score',        'trigger_type'=>'ai',     'trigger_value'=>'',                  'conditions'=>'is_member && quiz_taken',                             'style'=>'button'],
        ['icon'=>'📝','label'=>'See other quizzes',         'user_input'=>'See other quizzes',             'trigger_type'=>'action', 'trigger_value'=>'open_quiz_library', 'conditions'=>'is_member',                                           'style'=>'pill'],
        ['icon'=>'▶️','label'=>'Continue where I left off', 'user_input'=>'Continue where I left off',     'trigger_type'=>'action', 'trigger_value'=>'open_last_lesson',  'conditions'=>'is_member && has_incomplete_lesson',                  'style'=>'pill'],
        ['icon'=>'🆘','label'=>'I need help',               'user_input'=>'I need help',                   'trigger_type'=>'action', 'trigger_value'=>'open_support',      'conditions'=>'is_member',                                           'style'=>'pill'],
    ],
];
...
<div class="flosc-autoprompt-section" style="background:<?php echo $sc['color']; ?>; border:1px solid <?php echo $sc['border']; ?>; border-radius:6px; padding:20px; margin-bottom:24px;">
    <h3 style="margin-top:0;"><?php echo $sc['icon']; ?> <?php echo esc_html($sc['label']); ?></h3>
    ...
    <table class="widefat flosc-ap-table" style="border-collapse:collapse; max-width:1100px; font-size:13px;">
        <thead>
            <tr style="background:#f0f0f1;">
                <th style="width:50px; padding:6px 8px;">Icon</th>
                <th style="padding:6px 8px;">Label</th>
                <th style="padding:6px 8px;">User Input <span style="font-weight:normal; color:#787c82;">(sent to AI; blank=label)</span></th>
