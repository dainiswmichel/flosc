...
### 🔧 Bug Fixes (P0)
- **TASK-601:** Fixed `ivr-settings.php` calling `instance()` instead of `flosc_instance()` (from v1.0.6)
- **TASK-602:** Fixed `set_signed_cookie()` — now uses array syntax with `SameSite=Lax`
- **TASK-603:** Removed `session_start()` from `store_quiz_result()` — uses signed cookies (from v1.0.6)
