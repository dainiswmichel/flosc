...
- **Global IVR Responses:** "Are you there?" and other `always` condition messages now work for ALL user states (visitor, guest, member)
- **Phase Fallback:** If IVR match not found in current phase, searches freeline phase for `always` condition matches

### 🎠 Carousel Auto-Rotation
- **Visitor Carousel:** Cards now auto-rotate every 4 seconds for visitors
- **Loop Behavior:** Carousel loops back to start when reaching the end
- **Manual Override:** Timer resets when user manually clicks arrows or swipes

### 🎯 Panel Differentiation
- **IntroPanel:** For visitors (cards, auto-rotate)
- **PromptPanel:** For guests (pills)
- **MemberPromptPanel:** For members (pills)
