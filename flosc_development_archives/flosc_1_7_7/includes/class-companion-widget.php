...
class FLOSC_Companion_Widget {
    ...
    public function maybe_render_widget() {
        ...
        // Generate inline accent color override if set
        $accent_css = '';
        if (!empty($settings['accent_color'])) {
            $accent = sanitize_hex_color($settings['accent_color']);
            if ($accent) {
                $accent_css = sprintf(
                    '<style>.flosc-companion-widget{--companion-accent:%s;--companion-accent-hover:%s;}</style>',
                    esc_attr($accent),
        ...
        <?php echo $accent_css; ?>
        <script>
        window.FLOSC_COMPANION = <?php echo wp_json_encode([
