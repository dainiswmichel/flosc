...
- **Quiz state tracking** — Stores quiz results in user meta for personalization
- **Bridge state detection** — Tracks users who completed quiz but haven't purchased
- **Weakness analysis** — Calculates weakest category from quiz results
- **External quiz hooks** — `flosc_external_quiz_score` action for any quiz plugin

### 🔌 Quiz Integration Manager (New)
- **External plugin support** — LearnDash, Tutor LMS hooks built-in
- **REST API** — `POST /wp-json/flosc/v1/external-quiz` for custom integrations
- **Quiz registry** — Register quiz metadata for richer personalization
- **Lesson mapping** — Map question IDs to FLOSC lesson numbers
- **Shortcode** — `[flosc_quiz_results]` to display user's quiz history

### 🎯 How External Quiz Integration Works
```php
// Option 1: Direct API
FLOSC_Quiz_Manager::submit_score($user_id, 'my_quiz', [
    'score' => 75,
    'correct_items' => ['q1', 'q3', 'q5'],
    'incorrect_items' => ['q2', 'q4'],
]);

// Option 2: Action hook
do_action('flosc_external_quiz_score', $user_id, 'my_quiz', $score_data);

// Option 3: REST API
POST /wp-json/flosc/v1/external-quiz
{ "user_id": 123, "quiz_id": "my_quiz", "score_data": {...} }
```
