...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7: [link]. It has the complete IPA guide and video walkthrough. Ready to dive in?\"",
    ...
    public function store_quiz_result($request) {
        ...
        // v1.0.6 TASK-603: Store in signed cookie for visitors (not PHP session - avoids "headers sent" errors)
        if (!is_user_logged_in()) {
            $quiz_data = [
                'quiz_id' => $quiz_id,
                'score' => $score,
        ...
        if (is_user_logged_in()) {
            ...
            if (FLOSC_DEBUG) error_log("FLOSC v1.0.6: Quiz stored for user {$user_id}, score {$score}%, fired flosc_quiz_completed");
        }
    ...
    public function handle_process_quiz($request) {
        ...
        if (is_user_logged_in()) {
            ...
            if (FLOSC_DEBUG) error_log("FLOSC v1.0.6: Quiz completed for user {$user_id} with score {$analysis['score']}%");
        }
