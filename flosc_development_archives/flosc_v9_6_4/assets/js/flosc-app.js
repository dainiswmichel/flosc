...
class floscApp {
    ...
    showSuggestedReplies() {
        ...
        // v9.6.4: Fallback for ALL user types if no prompts found
        if (applicable.length === 0) {
            console.log('FLOSC: Using hardcoded fallback replies for', this.state);
            if (this.state === 'visitor') {
                applicable.push(
                    {
                        name: 'fallback_help',
                ...
                // Logged-in users (guest/member) get different fallbacks
                applicable.push(
                    {
                        name: 'fallback_continue',
                        user_input: '📚 Continue learning',
                        content: "Let's continue where you left off!",
                        style: 'pill'
                    },
                    {
                        name: 'fallback_help',
    ...
    handleSuggestedReply(messageName) {
        ...
        if (!msg) {
            ...
            return; // v9.6.4: NO fallbacks - IVR.md ONLY
        }
...
Purchased: ${ctx.purchased}
    ...
    restartChat() {
        // v9.6.4: Add spinning animation to button
        const restartBtn = document.getElementById('flosc_app_restart_chat');
        if (restartBtn) {
