...
<body class="flosc-app<?php echo is_admin_bar_showing() ? ' admin-bar' : ''; ?>"
    ...
    <script>
        ...
        window.FLOSC_CONFIG = <?php 
            ...
            // Per-flow autoprompt pills — sourced from the IVR file, stored per flow in WP DB
            'autoprompts' => (function() use ($flow_settings, $ivr_config) {
                ...
                // If admin hasn't saved yet, seed from IVR messages
                if (empty(array_filter($raw))) {
                    $seeded = ['visitor' => [], 'guest' => [], 'member' => []];
                    foreach (($ivr_config['messages'] ?? []) as $msg) {
