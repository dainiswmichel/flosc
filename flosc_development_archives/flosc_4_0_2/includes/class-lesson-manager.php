...
class FLOSC_Lesson_Manager {
     ...
     * "show all lessons" returns the full library (e.g. LeSAEp 65 + FLOSC Sample 10 = 75).
    ...
    public function get_all_lessons() {
        ...
        if ( empty( $categories ) ) {
            ...
            if ( empty( $single ) ) {
                error_log( '[FLOSC Lessons] No lesson categories configured. Set lesson_groups or lessons_category.' );
                return [];
            }
            $categories = [ $single ];
        }
        ...
        if ( empty( $cat_ids ) ) {
            error_log( '[FLOSC Lessons] None of the configured categories exist: ' . implode( ', ', $categories ) );
            return [];
        }
        ...
        error_log( '[FLOSC Lessons] v3.0.8 get_all_lessons: categories=' . implode( ',', $categories ) . ' found=' . $query->found_posts );
