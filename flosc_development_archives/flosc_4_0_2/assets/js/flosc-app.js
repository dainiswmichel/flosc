...
class floscApp {
    ...
    floscShowUserAutoPrompts() {
        ...
        const messages = Object.values(this.ivr.messages);
        let autoPrompts = messages.filter(m => m.type === 'suggested_user_autoprompt');

        // v3.0.3: If API returned 0 autoprompts, fall back to inline config.ivrMessages
        // (always available, embedded in page HTML). Phase-filter them to current phase.
        if (autoPrompts.length === 0 && this.config.ivrMessages) {
            this.log('FLOSC: No autoprompts from API — trying inline ivrMessages fallback');
            const inlineMessages = Object.values(this.config.ivrMessages);
            const guestPhases = ['login', 'offer', 'freeline'];
            const memberPhases = ['sale', 'content', 'freeline'];
            const allowedPhases = (this.ivr.phase === 'login' || this.ivr.phase === 'offer')
                ? guestPhases
                : (this.ivr.phase === 'sale' || this.ivr.phase === 'content')
                    ? memberPhases
                    : [this.ivr.phase, 'freeline'];
            autoPrompts = inlineMessages.filter(m =>
                m.type === 'suggested_user_autoprompt' &&
                (!m.phase || allowedPhases.includes(m.phase))
            );
            this.log('FLOSC: Inline fallback found', autoPrompts.length, 'autoprompts for phases:', allowedPhases);
        }

        this.log('FLOSC: Total suggested_user_autoprompt messages:', autoPrompts.length);

        // v3.0.5: Synthesize pill entries from config.offers that have pill format enabled.
        // These become real autoprompt pills alongside IVR-defined ones.
        // Panel mapping: guest→prompt, member→prompt, intro→intro, both→matches any
        const offerPills = [];
        if (Array.isArray(this.config.offers)) {
            for (const offer of this.config.offers) {
                const pillFmt = offer.display_formats?.pill;
                if (!pillFmt || !pillFmt.enabled) continue;
                // Skip offers already shown/purchased
                if (this.offers.shownOffers?.has(offer.id)) continue;

                const pillLabel = pillFmt.label || offer.pill_label || offer.name || '';
                if (!pillLabel) continue;

                // Determine which panel this pill targets
                const offerPanel = (pillFmt.target_panel || 'guest').toLowerCase();
                // Map offer target_panel to panel field used by filtering below
                let mappedPanel = 'prompt'; // default: guest/member panels
                if (offerPanel === 'intro') mappedPanel = 'intro';
                else if (offerPanel === 'both') mappedPanel = null; // null = show in any panel

                const pillPhrase = pillFmt.phrase || offer.pill_phrase || offer.reveal_phrase || pillLabel;

                offerPills.push({
                    name: '__offer_pill_' + offer.id,
                    type: 'suggested_user_autoprompt',
                    user_input: pillPhrase,
                    icon: pillFmt.icon || offer.pill_icon || offer.meta?.icon || '',
                    panel: mappedPanel,
                    conditions: pillFmt.condition || offer.condition || '',
                    // v3.0.5: Flag so click handler knows to call showOffer() not floscHandleUserAutoPrompt()
                    _offer_pill: true,
                    _offer_id: offer.id,
                    // For display in the button
                    _pill_label: pillLabel,
                });
            }
            if (offerPills.length > 0) {
                this.log('FLOSC: Synthesized', offerPills.length, 'offer pill(s) from config.offers');
            }
        }
        ...
        // v3.0.7: Always merge admin-configured pills from WP dashboard (they supplement IVR pills)
        const adminPills = Object.values(this.config.adminAutoprompts || {});
        if (adminPills.length > 0) {
            this.log('[FLOSC] Merging', adminPills.length, 'admin-configured autoprompts');
            // Remove IVR pills that have the same user_input as an admin pill (admin overrides IVR)
            const adminInputs = new Set(adminPills.map(p => (p.user_input || '').toLowerCase()));
            const dedupedIVR = autoPrompts.filter(p => !adminInputs.has((p.user_input || '').toLowerCase()));
            autoPrompts = [...dedupedIVR, ...adminPills];
        }

        // v3.0.5: Combine IVR autoprompts + offer pills into one list for unified filtering
        const allPills = [...autoPrompts, ...offerPills];

        for (const msg of allPills) {
            // v1.2.5: Check panel field first - if set, use it for filtering
            if (msg.panel !== undefined && msg.panel !== null) {
            ...
            // v3.0.5: panel === null means "both" (show in any panel)
        ...
        if (applicable.length === 0) {
            this.log('FLOSC: No panel-filtered matches, trying fallback');
            for (const msg of allPills) {
                if (this.state === 'visitor') {
                    if (!msg.phase || msg.phase === 'freeline') {
                        applicable.push(msg);
        ...
        if (applicable.length === 0) {
            this.log('FLOSC: No user autoprompts from IVR for state:', this.state);
            return;
        }

        this.log('FLOSC: User autoprompts for', this.state, '(panel:', targetPanel, '):', applicable.map(r => r.name));
    ...
    _renderAdminTestPanel() {
        document.getElementById('flosc_input_user_autoprompts_panel')?.remove();

        const container = document.querySelector('.flosc-input-bottom')
                       || document.querySelector('.flosc-input-area')
                       || document.getElementById('flosc_user_autoprompts');
        if (!container) return;

        const adminPrompts = this.config.adminAutoprompts || {};
        const testOffers   = this.config.adminTestOffers  || [];

        // Group pills by state (pill names follow pattern: admin_visitor_N, admin_guest_N, admin_member_N)
        const groups = { visitor: [], guest: [], member: [] };
        Object.values(adminPrompts).forEach(p => {
            const m = (p.name || '').match(/^admin_(visitor|guest|member)_/);
            if (m) groups[m[1]].push(p);
        });
        ...
        const stateConfig = {
            ...
            member:  { label: 'LeSAEp Learner Pills',  bg: '#f0fdf4', border: '#86efac', color: '#166534', emoji: '🟢' },
        };
        ...
        for (const [state, cfg] of Object.entries(stateConfig)) {
            ...
            <div class="flosc-admin-pill-group" style="background:${cfg.bg};border:1px solid ${cfg.border};border-radius:8px;padding:7px 10px;margin-bottom:5px;">
                <div style="font-size:10px;font-weight:700;color:${cfg.color};text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px;">${cfg.emoji} ${cfg.label}</div>
        ...
        panel.innerHTML = `
            <div class="prompt-panel-header">
                <div class="prompt-panel-eyebrow">🧪 Admin Test Mode — all states visible</div>
                <div class="prompt-panel-title">Admin Panel</div>
            </div>
            <div class="prompt-panel-body" style="display:flex;flex-direction:column;gap:0;">
                ${offersHtml}${sectionsHtml}
            </div>`;
    ...
    floscRenderUserAutoPrompts(replies) {
        ...
        const buttonsHtml = replies.map(r => {
            ...
            const triggerAttrs = r._admin_pill && triggerType && triggerType !== 'ai'
                ? ` data-trigger-type="${this.escapeHtml(triggerType)}" data-trigger-value="${this.escapeHtml(triggerValue)}"`
                : '';
            return `
            <button class="flosc-style-pill${offerClass}" data-message="${this.escapeHtml(r.name)}"${offerAttr}${triggerAttrs}>
        ...
        container.innerHTML = `
            <div class="prompt-panel-header">
                <div class="prompt-panel-eyebrow">${this.escapeHtml(eyebrowText)}</div>
        ...
        if (closeBtn) {
            closeBtn.addEventListener('click', () => {
                container.remove();
                // v1.9.1: Remember dismissal so panel doesn't reappear on every chat response
                this._panelDismissed = true;
                this.addMessage('user', `Hide ${panelName}`);
                this.addMessage('assistant', `${panelName} hidden. Type "Show ${panelName}" to bring it back.`);
            });
        }
    ...
    async _generateAIWelcome(ivrWelcomeMsg) {
        ...
        const badge = badgeUrl ? `<div style="display:flex;justify-content:center;width:100%;margin:16px 0"><img src="${badgeUrl}" alt="${productName}" style="max-width:200px;border-radius:12px"></div>` : '';
    ...
    async startInChatQuiz(quizId = 'default') {
        ...
        try {
            ...
            //         lesaep_pronunciation for the LeSAEp flow when no explicit setting is saved
    ...
    async openFilteredLessons(topic) {
        ...
        try {
            ...
            if (!lessons.length) {
                this.addMessage('assistant', `No lessons found for "<strong>${this.escapeHtml(displayTopic)}</strong>". Try a different keyword, or say <em>show me all lessons</em>.`);
                return;
            }
            // Temporarily rename the header to reflect the filter
            const savedRender = this.renderLessonList.bind(this);
            const header      = `📚 Lessons: ${displayTopic}`;
            const listHtml = `
                <div class="flosc-lesson-list">
                    <div class="flosc-lesson-list-header">
                        <h3>${this.escapeHtml(header)}</h3>
                        <p>${lessons.length} ${lessons.length === 1 ? 'lesson' : 'lessons'} found</p>
                    </div>
                    <div class="flosc-lesson-items flosc-lesson-items--compact">
                        ${lessons.map(l => `
                            <div class="flosc-lesson-item flosc-lesson-item--title-only"
                                 onclick="window.FLOSC.viewLesson(${parseInt(l.id)})">
                                <span class="flosc-lesson-item-title">${this.escapeHtml(l.title)}</span>
                                <span class="flosc-lesson-item-arrow">›</span>
                            </div>
                        `).join('')}
                    </div>
    ...
    openCheckout(offerId) {
        ...
        if (processor === 'free' || (Number(pricing.price) === 0 && processor !== 'paypal' && processor !== 'stripe')) {
            ...
            this.authFetch(this.config.apiUrl + '/purchase', {
                ...
                if (d.success) this.handlePurchaseSuccess(offerId, d);
...
Purchased: ${ctx.purchased}
    ...
    async storeQuizScore(result) {
        ...
        const quizData = {
            ...
            quizId: this.quiz.id || 'flosc_sample_text_based_quiz'
        };
        ...
        try {
            await this.authFetch(this.config.apiUrl + '/store-score', {
                ...
                body: JSON.stringify({
                    score: result.score,
                    quiz_id: this.quiz.id || 'flosc_sample_text_based_quiz',
                    quiz_type: 'sequence',
        ...
        if (hasPayPal && paypalContainer) {
            ...
            const paypalButtonsInstance = paypal.Buttons({
                ...
                createOrder: async () => {
                    ...
                    // v3.0.7: Pre-flight nonce refresh — ensures nonce is valid for current
                    // logged-in session. Critical when user registers mid-session: the page
                    // loaded with a visitor nonce that is invalid for their new WP session.
                ...
                onApprove: async (data, actions) => {
                    try {
                        ...
                        // v1.7.7: Handle INSTRUMENT_DECLINED (per PayPal official pattern)
                        // PHP now forwards PayPal's details array directly
                ...
                onCancel: () => {
                    this.log('[FLOSC-CHECKOUT] PayPal cancelled by user');
                    // v1.7.6: Re-render PayPal buttons after cancel so user can try again
                },
            });
            ...
            // Defer by one rAF so modal layout is painted before PayPal measures the container
            requestAnimationFrame(() => setTimeout(renderPayPalButtons, 50));
