...
class FLOSC_RAG_Manager {
    ...
    public function execute_tool($tool_name, $input, $access_level, $category_id = 0) {
        ...
        try {
            ...
            if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) error_log("FLOSC RAG Error: " . $e->getMessage());
    ...
    private function search_knowledge_base($query, $access_level, $category = 'all') {
        ...
        if (!is_dir($kb_path)) {
            if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) error_log("FLOSC RAG: Knowledge base directory not found at {$kb_path}");
