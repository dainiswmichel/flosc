...
Purchased: ${ctx.purchased}
    ...
    showPaymentModal(offerId) {
        ...
        if (hasPayPal && paypalContainer) {
            // v5.0.4 FIX: Close previous PayPal instance before re-rendering.
            // innerHTML = '' clears the DOM but PayPal keeps its component mounted internally.
            // Re-rendering into an already-mounted container causes "Load failed" every time
            // the modal is opened after the first time.
            ...
            // v5.0.4: Store instance so we can .close() it before the next render
            ...
            paypalButtonsInstance.render(paypalContainer).catch(err => {
                ...
                paypalContainer.innerHTML =
                    ...
                    '<button onclick="window.floscApp && window.floscApp.showPaymentModal(\'' + offerId + '\')" ' +
