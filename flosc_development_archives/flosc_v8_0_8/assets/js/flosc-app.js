 ...
 * v8.0.8: Clear all localStorage including session keys on version change to ensure welcome message displays
 */

// v8.0.8: Clear ALL localStorage on version change
(function() {
    const FLOSC_JS_VERSION = '8.0.8';
    try {
        const stored = localStorage.getItem('flosc_js_version');
        if (stored !== FLOSC_JS_VERSION) {
            // Clear ALL keys - complete fresh start
            localStorage.clear();
            localStorage.setItem('flosc_js_version', FLOSC_JS_VERSION);
            console.log('FLOSC v8.0.8: Complete storage cleared - fresh session');
        }
    } catch(e) {
...
class floscApp {
    ...
    async init() {
        ...
        try {
            ...
            if (this.state === 'visitor') {
                console.log('[FLOSC] Restoring visitor messages...');
    ...
    startIVR() {
            console.log('FLOSC: Starting IVR for phase:', this.ivr.phase);
            console.log('FLOSC: Total messages available:', Object.keys(this.ivr.messages).length);
        
            // v8.0.5: Check if ANY messages loaded
            if (Object.keys(this.ivr.messages).length === 0) {
                console.error('FLOSC v8.0.5: CRITICAL - No IVR messages loaded!');
                console.error('FLOSC: Check WordPress error_log for PHP parsing errors');
            
                // Show emergency message immediately
                const emergencyMsg = `Hi! I'm experiencing a configuration issue. ` +
                    `The system couldn't load messages. Please contact support.`;
                this.addMessage('assistant', emergencyMsg);
                return;
            }
        ...
        // FALLBACK: If no IVR messages render, show a default welcome
        setTimeout(() => {
            const hasMessages = this.chatMessages?.querySelector('.flosc-message');
            if (!hasMessages) {
                    console.warn('FLOSC: No IVR messages matched conditions');
                    console.warn('FLOSC: Phase:', this.ivr.phase);
                    console.warn('FLOSC: Context:', this.ivr.context);
                
                const fallbackMsg = "Hi! I'm your assistant. How can I help you today?";
    ...
    checkAutoMessages() {
        ...
        if (!this.chatMessages?.querySelector('.flosc-message')) {
            console.warn('FLOSC: No auto messages matched conditions');
        }
    }
