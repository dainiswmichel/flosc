 ...
 * v9.0.3: Added first_show_session to context, cleared old visitor messages on first session
 */

// v9.0.3: Clear FLOSC-specific localStorage on version change
(function() {
    const FLOSC_JS_VERSION = '9.0.3';
    try {
        ...
        if (stored !== FLOSC_JS_VERSION) {
            ...
            console.log('FLOSC v9.0.3: Storage cleared - fresh session');
        }
    } catch(e) {
...
class floscApp {
    ...
    async init() {
        ...
        try {
            ...
            // v9.0.3: Only restore visitor messages if NOT first session (prevents old messages blocking welcome)
            if (this.state === 'visitor' && !this.ivr.context.first_show_session) {
                console.log('[FLOSC] Restoring visitor messages from previous session...');
                this.restoreVisitorMessages();
            } else if (this.state === 'visitor' && this.ivr.context.first_show_session) {
                console.log('[FLOSC] First session - clearing old visitor messages');
                try {
                    localStorage.removeItem('flosc_visitor_messages');
                } catch(e) {
                    console.warn('FLOSC: Could not clear visitor messages', e);
                }
            }
    ...
    buildIVRContext() {
        ...
        // v9.0.3: Check if this is first session by looking at localStorage (use unified session key)
        const sessionKey = 'flosc_session_' + this.getSessionKey();
        const hasSessionKey = !!localStorage.getItem(sessionKey);
        const firstShowSession = !hasSessionKey;
        ...
        this.ivr.context = {
            ...
            first_show_session: firstShowSession,  // v9.0.3: Added back for welcome message conditions
    ...
    startIVR() {
        console.log('FLOSC v9.0.3: Starting IVR for phase:', this.ivr.phase);
    ...
    showSuggestedReplies() {
        ...
        // Otherwise: try to find replies that match conditions
        for (const msg of suggestedReplies) {
        ...
        if (applicable.length === 0) {
            console.log('FLOSC: No conditional matches, showing all phase replies');
            for (const msg of suggestedReplies) {
                if (!msg.phase || msg.phase === this.ivr.phase) {
                    applicable.push(msg);
    ...
    renderSuggestedReplies(replies) {
        ...
        if (closeBtn) {
            closeBtn.addEventListener('click', () => {
                container.remove();
                this.addMessage('assistant', 'Hide IntroPanel');
            });
        }
    ...
    handleSuggestedReply(messageName) {
        console.log('FLOSC: Suggested reply clicked:', messageName);
        ...
        // Fallbacks for common visitor buttons (v05.05-style)
        const fallbacks = {
            'get_started_001': {
                user_input: 'Get started',
                assistant_response: "Welcome! I'm here to help you. The best way to begin is by taking our free quiz. Ready to get started?",
                action: null
            },
            'start_quiz_001': {
                user_input: 'Start free quiz',
                assistant_response: 'Opening the quiz…',
                action: 'open_quiz'
            },
            'login_to_see_score_001': {
                user_input: 'Log in to see my score',
                assistant_response: 'Please log in to view your score.',
                action: 'open_registration'
            },
            'how_it_works_001': {
                user_input: 'How does it work?',
                assistant_response: "Here's how it works: Take a quick quiz to assess your level, get personalized feedback, and unlock content based on your results. Want to try?",
                action: null
            }
        };

        const fb = fallbacks[messageName];
        if (!fb) {
            console.warn('FLOSC: No IVR message or fallback found for', messageName);
            // Last resort: treat messageName as a prompt and call API
            const prompt = messageName.replace(/_/g, ' ');
        ...
        console.log('FLOSC: Using fallback for', messageName);
        this.addMessage('user', fb.user_input);
        this.ivr.messageCount++;
        this.ivr.lastInteraction = Date.now();
        this.ivr.context.first_show_session = false;
        ...
        setTimeout(() => {
            this.hideTyping();
            this.addMessage('assistant', fb.assistant_response);
            if (fb.action) {
                this.performIVRAction(fb.action);
            }
            this.showSuggestedReplies();
        }, 400);
    }
...
Purchased: ${ctx.purchased}
    ...
    restartChat() {
        const restartBtn = document.getElementById('flosc_app_restart_chat');
        if (restartBtn) {
            restartBtn.classList.add('flosc-restart-loading');
            restartBtn.disabled = true;
        }
        this.addMessage('assistant', 'Restarting chat…');
        ...
        // Clear session tracking so first_show_session becomes TRUE again
        const sessionKey = 'flosc_session_' + this.getSessionKey();
        localStorage.removeItem(sessionKey);
        // Also remove legacy key format to be safe
        const legacyKey = 'flosc_session_' + (this.user?.id || 'visitor');
        ...
        // Optional: full page reload to ensure clean state
        setTimeout(() => {
            try {
                window.location.reload();
            } catch (e) {
                console.warn('FLOSC: Could not reload page after restart', e);
            }
        }, 300);
    }
    ...
    async sendMessage() {
        console.log('[FLOSC] sendMessage() called');

        const message = this.chatInput?.value?.trim();
        console.log('[FLOSC] Message value:', message);

        if (!message) {
            console.warn('[FLOSC] No message to send, returning');
            return;
        }
        ...
        // v8.0.6: Always add user message FIRST (before onUserMessage check)
        // This ensures the chat is responsive and user sees their message immediately
        console.log('[FLOSC] Adding user message to chat');
        ...
        if (this.onUserMessage(message)) {
            console.log('[FLOSC] onUserMessage handled special command, returning');
            return;
        }
        
        console.log('[FLOSC] Showing typing indicator');
        this.showTyping();
        
        // Try to find IVR response locally first
        console.log('[FLOSC] Attempting to find IVR response...');
        const responseMsg = this.findIVRResponse(message);

        if (responseMsg) {
            console.log('[FLOSC] IVR response found:', responseMsg);
            setTimeout(() => {
                this.hideTyping();
                this.showIVRMessage(responseMsg);

                if (this.state === 'visitor') {
                    this.saveVisitorMessage('assistant', responseMsg.content);
                }
        ...
        console.warn('[FLOSC] No IVR match found, calling API fallback');
        console.log('[FLOSC] API URL:', this.config.apiUrl);
        console.log('[FLOSC] Nonce:', this.config.nonce);

        try {
            console.log('[FLOSC] Calling API with message:', message);
            const assistantMessage = await this.callAPI(message);
            console.log('[FLOSC] API response received:', assistantMessage);
            ...
            if (assistantMessage) {
                ...
                if (this.state === 'visitor') {
                    this.saveVisitorMessage('assistant', assistantMessage);
                }

                this.showSuggestedReplies();
            } else {
                const errorMsg = "I'm having trouble responding right now. Please try again.";
                console.warn('[FLOSC] No assistant message, showing error');
                this.addMessage('assistant', errorMsg);
            }
        } catch (error) {
            console.error('[FLOSC] API fallback failed:', error);
            console.error('[FLOSC] Error details:', error.message, error.stack);
            this.hideTyping();
    ...
    findIVRResponse(userMessage) {
        ...
        // Try to match user input to a suggested_reply message
        const matchingReply = messages.find(m => 
            m.type === 'suggested_reply' && 
            m.user_input && 
            m.user_input.toLowerCase() === userMessage.toLowerCase()
        );
        
        if (matchingReply && this.evaluateCondition(matchingReply.conditions)) {
            return matchingReply;
        }
        
        // Fallback: return null (will show default message)
        return null;
    }
