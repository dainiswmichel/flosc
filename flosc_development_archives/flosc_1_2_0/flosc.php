...
The {product_name} Team";
    ...
    public function handle_app_route() {
        // v1.2.0: Check for custom domain FIRST (direct HTTP_HOST check)
        // This is more reliable than query vars which WordPress can override
        $custom_domain = get_option('flosc_custom_domain', '');
        if (!empty($custom_domain)) {
            ...
            // If we're on the custom domain, render the FLOSC app
            if ($current_host === $custom_domain || $current_host === 'www.' . $custom_domain) {
