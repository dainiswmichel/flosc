...
$enabled_quizzes = $flow_settings['enabled_quizzes'] ?? [];
if (!is_array($enabled_quizzes)) $enabled_quizzes = [];

// v3.0.0: Build quiz label map from quiz type factory
$quiz_label_map = [];
if (class_exists('FLOSC_Quiz_Type_Factory')) {
    $all_quiz_types = FLOSC_Quiz_Type_Factory::get_all_quiz_types();
    foreach ($all_quiz_types as $qid => $qt) {
        $quiz_label_map[$qid] = $qt['name'] ?? ucwords(str_replace(['_', 'flosc-', 'flosc_'], [' ', '', ''], $qid));
    }
}
