 ...
 * v07.07: Added localStorage version management
 */

// v07.07: Clear localStorage on version change (runs BEFORE class loads)
(function() {
    const FLOSC_JS_VERSION = '7.0.7';
    try {
        const stored = localStorage.getItem('flosc_js_version');
        if (stored !== FLOSC_JS_VERSION) {
            // Version changed - clear all FLOSC data to prevent corruption
            const keys = [];
