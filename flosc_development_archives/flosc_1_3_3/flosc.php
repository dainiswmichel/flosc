...
// v1.3.3: Version-based auto-flush on plugin update (file replacement)
...
function flosc_version_flush_check() {
    ...
    if (version_compare(FLOSC_VERSION, $last_flushed_version, '>')) {
        // Version changed - schedule flush
        update_option('flosc_needs_flush', true);
        update_option('flosc_last_flushed_version', FLOSC_VERSION);
        
        if (FLOSC_DEBUG) {
            error_log("FLOSC: Version change detected ({$last_flushed_version} → " . FLOSC_VERSION . ") - scheduling permalink flush");
        }
    }
}
...
The {product_name} Team";
    ...
    /**
     * Rewrite Rules for Virtual Page - v1.2.9: IVR-file-based flows
     */
    public function add_rewrite_rules() {
        // v1.2.9: Register rewrite rules from IVR file settings
        ...
        if (is_dir($ivr_dir)) {
            ...
            foreach ($files as $file) {
                ...
                if (!empty($flow_settings['slug']) && ($flow_settings['status'] ?? 'active') === 'active') {
                    add_rewrite_rule(
                        '^' . preg_quote($flow_settings['slug'], '/') . '/?$',
...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7: [link]. It has the complete IPA guide and video walkthrough. Ready to dive in?\"",
    ...
    /**
     * v1.3.1: Sandbox Purchase - Fun "Pay What You Want" testing
     * Grants flosc_sandbox membership level without real payment
     */
    public function handle_sandbox_purchase($request) {
        ...
        // Grant flosc_sandbox membership level
        update_user_meta($user_id, '_flosc_member_level', 'flosc_sandbox');
        ...
        $sandbox_offer = [
            ...
            'member_level' => 'flosc_sandbox',
            'features' => ['all_lessons', 'all_quizzes', 'ai_chat', 'premium_content'],
        ];
        $access_manager->grant_access($user_id, $sandbox_offer);
        
        // Log the sandbox purchase
        if (FLOSC_DEBUG) {
            error_log("FLOSC Sandbox: User {$user_id} purchased sandbox access for {$formatted_amount} (Transaction: {$transaction_id})");
        }
