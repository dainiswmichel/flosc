<?php
/**
 * FLOSC Admin Settings Page v1.3.3
 * 
 ...
 * v1.3.3: All Flows = fully expanded inline editing, Domain field, version-based auto-flush
 */
    ...
    <form method="post" class="flosc-settings-form">
        ...
        <?php if ($active_tab === 'identity'): ?>
            ...
            <div style="max-width: 1000px;">
                ...
                <div class="flosc-flow-block" style="background: white; border: 2px solid <?php echo $is_current ? '#4f46e5' : '#e5e7eb'; ?>; border-radius: 10px; padding: 25px; margin-bottom: 20px;">
                    ...
                    <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding: 15px 20px; border-radius: 8px; color: white; margin-bottom: 20px;">
                        <div style="font-size: 11px; opacity: 0.9; margin-bottom: 3px;">🌐 This flow serves:</div>
                        ...
                        <div style="font-size: 11px; opacity: 0.8; margin-top: 5px;">
                            Domain → Slug → IVR: 
                            <strong><?php echo esc_html($settings['domain'] ?: '(WordPress default)'); ?></strong> → 
                            <code style="background: rgba(255,255,255,0.2); padding: 1px 5px; border-radius: 3px;">/<?php echo esc_html($settings['slug'] ?? ''); ?>/</code> → 
                            <code style="background: rgba(255,255,255,0.2); padding: 1px 5px; border-radius: 3px;"><?php echo esc_html($ivr_file); ?></code>
                        </div>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        ...
                        <div>
                            <div style="margin-bottom: 15px;">
                                <label style="display: block; font-weight: 600; margin-bottom: 5px; font-size: 13px;">Domain</label>
                                ...
                                <p style="font-size: 11px; color: #6b7280; margin: 4px 0 0;">Custom domain that serves this flow (leave empty for WordPress default)</p>
                            </div>
                        ...
                        <div>
                            <div style="margin-bottom: 15px;">
                                <label style="display: block; font-weight: 600; margin-bottom: 5px; font-size: 13px;">URL Slug</label>
                                <div style="display: flex; align-items: center; gap: 5px;">
                                    <span style="color: #6b7280; font-size: 13px;">/</span>
                                    <input type="text" name="<?php echo $prefix; ?>slug" 
                                           value="<?php echo esc_attr($settings['slug'] ?? ''); ?>"
                                           placeholder="myapp"
                                           style="width: 120px; padding: 8px 12px; border: 1px solid #d1d5db; border-radius: 6px;">
                                    <span style="color: #6b7280; font-size: 13px;">/</span>
                                </div>
                            </div>
            ...
            <div class="card" style="max-width: 900px; padding: 20px; margin-top: 20px;">
                ...
                <?php $full_url = !empty($flow_settings['domain']) ? 'https://' . $flow_settings['domain'] . '/' : $flow_url; ?>
                <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding: 20px; border-radius: 8px; color: white; margin-bottom: 25px;">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 20px;">
                        <div>
                            <div style="font-size: 12px; opacity: 0.9; margin-bottom: 5px;">🌐 This flow serves:</div>
                            <div style="font-family: monospace; font-size: 18px; font-weight: 600;">
                                <a href="<?php echo esc_url($full_url); ?>" target="_blank" style="color: white; text-decoration: none;">
                                    <?php echo esc_html($full_url); ?> ↗
                                </a>
                            </div>
                            <div style="font-size: 12px; opacity: 0.8; margin-top: 8px;">
                                Domain → Slug → IVR: 
                                <strong><?php echo esc_html($flow_settings['domain'] ?? '(WordPress default)'); ?></strong> → 
                                <code style="background: rgba(255,255,255,0.2); padding: 2px 6px; border-radius: 3px;">/<?php echo esc_html($flow_settings['slug'] ?? ''); ?>/</code> → 
                                <code style="background: rgba(255,255,255,0.2); padding: 2px 6px; border-radius: 3px;"><?php echo esc_html($selected_ivr); ?></code>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <table class="form-table">
                    <tr>
                        <th><label for="flow_domain">Domain</label></th>
                        <td>
                            <input type="text" id="flow_domain" name="flow_domain" class="regular-text"
                                   value="<?php echo esc_attr($flow_settings['domain'] ?? ''); ?>"
                                   placeholder="e.g., flosc.ai or lesaep.com">
                            <p class="description">Custom domain that serves this flow. Leave empty to use WordPress default URL.</p>
                        </td>
                    </tr>
                    <tr>
