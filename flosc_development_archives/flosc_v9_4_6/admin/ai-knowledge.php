...
<table class="form-table">
    <tr>
        <th scope="row"><label for="flosc_ai_personality_name">AI Name</label></th>
        <td>
            <input type="text" id="flosc_ai_personality_name" name="flosc_ai_personality_name" 
                   value="<?php echo esc_attr(get_option('flosc_ai_personality_name', get_option('flosc_product_name', 'FLOSC'))); ?>" 
                   class="regular-text">
    ...
    <tr>
        <th scope="row"><label for="flosc_ai_personality_role">AI Role</label></th>
        <td>
            <input type="text" id="flosc_ai_personality_role" name="flosc_ai_personality_role" 
                   value="<?php echo esc_attr(get_option('flosc_ai_personality_role', 'pronunciation coach and learning assistant')); ?>" 
                   class="large-text">
    ...
    <tr>
        <th scope="row"><label for="flosc_ai_personality_traits">Personality Traits</label></th>
        <td>
            <textarea id="flosc_ai_personality_traits" name="flosc_ai_personality_traits" rows="3" class="large-text"><?php 
                echo esc_textarea(get_option('flosc_ai_personality_traits', 'Encouraging, patient, specific, action-oriented. You celebrate small wins and make learning feel achievable. You break down complex concepts into simple steps.')); 
            ?></textarea>
    ...
    <tr>
        <th scope="row"><label for="flosc_ai_mission">AI Mission Statement</label></th>
        <td>
            <textarea id="flosc_ai_mission" name="flosc_ai_mission" rows="4" class="large-text"><?php 
                echo esc_textarea(get_option('flosc_ai_mission', 'Your mission is to help users improve their pronunciation through personalized guidance, instant feedback, and encouragement. You assess their current level, identify specific areas for improvement, recommend appropriate lessons, and keep them motivated on their learning journey.')); 
            ?></textarea>
...
<table class="form-table">
    <tr>
        <th scope="row"><label for="flosc_ai_context_awareness">AI Context Awareness</label></th>
        <td>
            <textarea id="flosc_ai_context_awareness" name="flosc_ai_context_awareness" rows="5" class="large-text code"><?php 
                echo esc_textarea(get_option('flosc_ai_context_awareness', 'You have access to special knowledge files that contain:
...
This information is NOT in your training data. Always reference these files when answering questions about our content, approach, or offerings.')); 
    ...
    <tr>
        <th scope="row"><label for="flosc_ai_freeline_restrictions">Freeline (Visitor) Access Rules</label></th>
        <td>
            <textarea id="flosc_ai_freeline_restrictions" name="flosc_ai_freeline_restrictions" rows="4" class="large-text"><?php 
                echo esc_textarea(get_option('flosc_ai_freeline_restrictions', 'For visitors (not logged in):
...
- Guide them toward taking the quiz to get personalized help')); 
    ...
    <tr>
        <th scope="row"><label for="flosc_ai_member_access">Member Access Rules</label></th>
        <td>
            <textarea id="flosc_ai_member_access" name="flosc_ai_member_access" rows="4" class="large-text"><?php 
                echo esc_textarea(get_option('flosc_ai_member_access', 'For logged-in members:
...
- Can guide them through lessons they have access to')); 
    ...
    <tr>
        <th scope="row"><label for="flosc_ai_boundaries">AI Boundaries & Limitations</label></th>
        <td>
            <textarea id="flosc_ai_boundaries" name="flosc_ai_boundaries" rows="4" class="large-text"><?php 
                echo esc_textarea(get_option('flosc_ai_boundaries', 'What you should NOT do:
...
<div class="card" style="max-width: 100%;">
    ...
    <?php if (empty($files)): ?>
        <p style="color: #666; font-style: italic;">No knowledge files yet. Upload your first file above to get started.</p>
        ...
        <table class="widefat" style="margin-top: 10px;">
            ...
            <tbody>
                <?php foreach ($files as $file): ?>
                    ...
                    $access_level = get_option('flosc_knowledge_access_' . md5($file), 'public');
                    ?>
                    <tr>
                        <td>
...
function flosc_inject_ai_personality() {
    $personality = "# WHO YOU ARE\n\n";
    $personality .= "Name: " . get_option('flosc_ai_personality_name', 'FLOSC') . "\n";
    $personality .= "Role: " . get_option('flosc_ai_personality_role', 'pronunciation coach') . "\n\n";
    $personality .= "## Personality Traits\n";
    $personality .= get_option('flosc_ai_personality_traits', '') . "\n\n";
    $personality .= "## Your Mission\n";
    $personality .= get_option('flosc_ai_mission', '') . "\n\n";
...
function flosc_inject_knowledge_context($user_phase) {
    ...
    foreach ($files as $file) {
        ...
        if (file_exists($filepath)) {
            $access = get_option('flosc_knowledge_access_' . md5($file), 'public');
    ...
    $context .= get_option('flosc_ai_boundaries', '') . "\n\n";
    
    return $context;
}

function flosc_get_knowledge_files($access_filter = 'all') {
...
if (isset($_FILES['orientation_file']) && wp_verify_nonce($_POST['flosc_ai_knowledge_nonce'], 'flosc_ai_knowledge')) {
    ...
    if ($file['error'] === UPLOAD_ERR_OK && pathinfo($file['name'], PATHINFO_EXTENSION) === 'md') {
        ...
        if (move_uploaded_file($file['tmp_name'], $destination)) {
            // Store access level
            update_option('flosc_knowledge_access_' . md5($filename), $access_level);
...
if (isset($_GET['toggle_access'])) {
    ...
    $option_key = 'flosc_knowledge_access_' . md5($filename);
    $current = get_option($option_key, 'public');
