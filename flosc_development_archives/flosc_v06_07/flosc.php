...
// Plugin constants
// v06.07: Critical fix - restored 'plugins_loaded' hook (v06.04-v06.06 used 'init' causing early initialization errors)
define('FLOSC_VERSION', '6.0.7');
define('FLOSC_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('FLOSC_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * UUID helper compatibility shim
 * Ensures plugin works on WP/PHP versions without wp_generate_uuid4()
 */
if (!function_exists('flosc_generate_uuid4')) {
    function flosc_generate_uuid4() {
        ...
        // Fallback for very old PHP: use uniqid with extra entropy
        return uniqid('', true);
    }
}
...
class FLOSC_Framework {
    ...
    private function check_rate_limit($endpoint, $limit = 20, $window = 3600) {
        ...
        if (is_user_logged_in()) {
            ...
            if (empty($visitor_id)) {
                $visitor_id = flosc_generate_uuid4();
...
The {product_name} Team";
     ...
     * Register Settings
     * 
     * @updated 6.0.3 - Added sanitize callbacks for all settings
     */
    public function register_settings() {
        // v6.0.3: Helper for common sanitize callbacks
        $sanitize_text = ['sanitize_callback' => 'sanitize_text_field'];
        $sanitize_textarea = ['sanitize_callback' => 'sanitize_textarea_field'];
        $sanitize_url = ['sanitize_callback' => 'esc_url_raw'];
        $sanitize_bool = ['sanitize_callback' => [$this, 'sanitize_boolean']];
        $sanitize_slug = ['sanitize_callback' => 'sanitize_title'];
        $sanitize_hex = ['sanitize_callback' => 'sanitize_hex_color'];
        $sanitize_int = ['sanitize_callback' => 'absint'];
        
        // Product Settings
        register_setting('flosc_settings', 'flosc_app_slug', $sanitize_slug);
        register_setting('flosc_settings', 'flosc_product_name', $sanitize_text);
        register_setting('flosc_settings', 'flosc_product_tagline', $sanitize_text);
        register_setting('flosc_settings', 'flosc_product_emoji', $sanitize_text);
        register_setting('flosc_settings', 'flosc_product_logo', $sanitize_url);
        register_setting('flosc_settings', 'flosc_primary_color', $sanitize_hex);
        register_setting('flosc_settings', 'flosc_share_text', $sanitize_textarea);
        
        // AI Provider
        register_setting('flosc_settings', 'flosc_ai_provider', $sanitize_text);
        register_setting('flosc_settings', 'flosc_openai_api_key', $sanitize_text);
        register_setting('flosc_settings', 'flosc_anthropic_api_key', $sanitize_text);
        register_setting('flosc_settings', 'flosc_xai_api_key', $sanitize_text);

        // v04_04: AI Configuration
        register_setting('flosc_ai_settings', 'flosc_ai_provider', $sanitize_text);
        register_setting('flosc_ai_settings', 'flosc_openai_api_key', $sanitize_text);
        register_setting('flosc_ai_settings', 'flosc_anthropic_api_key', $sanitize_text);
        register_setting('flosc_ai_settings', 'flosc_xai_api_key', $sanitize_text);
        register_setting('flosc_ai_settings', 'flosc_ai_base_prompt', $sanitize_textarea);
        
        // STT Provider
        register_setting('flosc_settings', 'flosc_stt_provider', $sanitize_text);
        register_setting('flosc_settings', 'flosc_assemblyai_api_key', $sanitize_text);
        register_setting('flosc_settings', 'flosc_deepgram_api_key', $sanitize_text);
        register_setting('flosc_settings', 'flosc_custom_stt_endpoint', $sanitize_url);

        // Quiz Type System
        register_setting('flosc_settings', 'flosc_quiz_type', $sanitize_text);
        ...
        foreach ($quiz_types as $quiz_id => $quiz_type) {
            register_setting('flosc_settings', 'flosc_quiz_content_' . $quiz_id, $sanitize_textarea);
            ...
            foreach ($settings_fields as $field_key => $field_config) {
                register_setting('flosc_settings', 'flosc_quiz_' . $quiz_id . '_' . $field_key, $sanitize_text);
            }
            ...
            foreach (array_keys($templates) as $template_key) {
                register_setting('flosc_settings', 'flosc_quiz_' . $quiz_id . '_template_' . $template_key, $sanitize_textarea);
            }
        }
        
        // Analytics
        register_setting('flosc_settings', 'flosc_ga4_id', $sanitize_text);
        
        // BuddyBoss
        register_setting('flosc_settings', 'flosc_buddyboss_group_id', $sanitize_int);
        
        // Lessons
        register_setting('flosc_settings', 'flosc_lessons_category', $sanitize_text);
        register_setting('flosc_settings', 'flosc_oto_offer_id', $sanitize_int);
        
        // Email
        register_setting('flosc_settings', 'flosc_email_subject', $sanitize_text);
        register_setting('flosc_settings', 'flosc_email_body', $sanitize_textarea);

        // Messages (v3.0.4)
        register_setting('flosc_settings', 'flosc_welcome_message', $sanitize_textarea);
        register_setting('flosc_settings', 'flosc_get_started_message', $sanitize_textarea);
        register_setting('flosc_settings', 'flosc_how_it_works_message', $sanitize_textarea);
        register_setting('flosc_settings', 'flosc_what_you_learn_message', $sanitize_textarea);

        // Payments (v5.0.6)
        register_setting('flosc_settings', 'flosc_stripe_enabled', $sanitize_bool);
        register_setting('flosc_settings', 'flosc_stripe_mode', $sanitize_text);
        register_setting('flosc_settings', 'flosc_stripe_test_pk', $sanitize_text);
        register_setting('flosc_settings', 'flosc_stripe_test_sk', $sanitize_text);
        register_setting('flosc_settings', 'flosc_stripe_live_pk', $sanitize_text);
        register_setting('flosc_settings', 'flosc_stripe_live_sk', $sanitize_text);

        // v05_09: ClickBank settings
        register_setting('flosc_settings', 'flosc_clickbank_enabled', $sanitize_bool);
        register_setting('flosc_settings', 'flosc_clickbank_mode', $sanitize_text);
        register_setting('flosc_settings', 'flosc_clickbank_vendor', $sanitize_text);
        register_setting('flosc_settings', 'flosc_clickbank_secret', $sanitize_text);
        register_setting('flosc_settings', 'flosc_clickbank_product', $sanitize_text);
        register_setting('flosc_settings', 'flosc_clickbank_access_level', $sanitize_text);

        // AI Base Prompt (v5.0.6 - added to flosc_settings group)
        register_setting('flosc_settings', 'flosc_ai_base_prompt', $sanitize_textarea);
    }
...
// Start the plugin
// v06.07: Restored 'plugins_loaded' hook (init was causing early initialization errors)
