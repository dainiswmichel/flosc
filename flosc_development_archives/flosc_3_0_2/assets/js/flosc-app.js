...
class floscApp {
    ...
    async loadIVRMessages() {
        try {
            ...
            if (!response.ok) {
                ...
                // v3.0.2: Fall back to inline ivrMessages (always available, full IVR set)
                // instead of empty emergency fallback that has zero autoprompts
                this.ivr.messages = this.config.ivrMessages || this.getFallbackMessages();
                this.log('[FLOSC] Using inline ivrMessages fallback. Count:', Object.keys(this.ivr.messages).length);
                return;
            }
            ...
            if (data.success && data.messages && data.messages.length > 0) {
                ...
                // v3.0.2: Fall back to inline ivrMessages instead of empty emergency fallback
                this.ivr.messages = this.config.ivrMessages || this.getFallbackMessages();
            }
    ...
    floscShowUserAutoPrompts() {
        ...
        // v3.0.2: Merge API-loaded messages with inline config messages.
        // If the REST API failed (cross-domain, nonce, network), this.ivr.messages
        // may only have the emergency fallback. The inline config.ivrMessages always
        // has the full IVR set from the correct file, embedded in the page HTML.
        const mergedMessages = { ...(this.config.ivrMessages || {}), ...this.ivr.messages };
        const messages = Object.values(mergedMessages);
        const autoPrompts = messages.filter(m => m.type === 'suggested_user_autoprompt');

        this.log('FLOSC: Total suggested_user_autoprompt messages:', autoPrompts.length, '(from', Object.keys(this.ivr.messages).length, 'API +', Object.keys(this.config.ivrMessages || {}).length, 'inline)');
    ...
    floscRenderUserAutoPrompts(replies) {
        ...
        const buttonsHtml = replies.map(r => `
            <button class="flosc-style-pill" data-message="${this.escapeHtml(r.name)}">
...
Purchased: ${ctx.purchased}
    ...
    async sendMessage(directMessage = null, { executeActions = true } = {}) {
        ...
        try {
            ...
            if (response) {
                const msgEl = this.addMessage('assistant', response, true);
                ...
                if (msgEl && this.user?.isAdmin) {
                    this.addAdminFeedbackButtons(msgEl, message, response);
                }
