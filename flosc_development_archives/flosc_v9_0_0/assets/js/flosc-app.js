 ...
 * v9.0.0: Clean three-tier business logic - visitor/guest/member, proper quiz-login-lesson flow
 */

// v9.0.0: Clear FLOSC-specific localStorage on version change
(function() {
    const FLOSC_JS_VERSION = '9.0.0';
    try {
        ...
        if (stored !== FLOSC_JS_VERSION) {
            ...
            console.log('FLOSC v9.0.0: Storage cleared - fresh session');
        }
    } catch(e) {
...
class floscApp {
    ...
    buildIVRContext() {
        ...
        // Note: Don't set first_show_session here - check DOM instead (see startIVR)
    }
    ...
    startIVR() {
        console.log('FLOSC v8.0.9: Starting IVR for phase:', this.ivr.phase);
    ...
    showSuggestedReplies() {
        ...
        // v8.0.9: First, try to find replies that match conditions
        ...
        // v8.0.9: If no matches, grab ANY suggested replies for current phase
        if (applicable.length === 0) {
            console.log('FLOSC: No conditional matches, showing all phase replies');
            for (const msg of suggestedReplies) {
                if (!msg.phase || msg.phase === this.ivr.phase) {
                    applicable.push(msg);
                    if (applicable.length >= 4) break; // Limit to 4 suggestions
                }
            }
        }
        
        // v8.0.9: If STILL nothing, show hardcoded fallbacks
