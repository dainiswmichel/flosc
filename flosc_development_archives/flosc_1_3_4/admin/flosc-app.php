...
<body class="flosc-app"
    ...
    <?php
    // =========================================================================
    // ADMIN VERIFICATION PANEL (v1.3.4)
    // =========================================================================
    // Shows IVR file info when admin visits a flow URL for verification
    // Dismissible via localStorage so it only shows once per session
    // =========================================================================
    if (current_user_can('manage_options') && !empty($ivr_file)): 
        $ivr_basename = basename($ivr_file);
        $flow_settings = get_option("flosc_flow_{$ivr_basename}", []);
        $flow_name = $flow_settings['name'] ?? pathinfo($ivr_basename, PATHINFO_FILENAME);
        $flow_tagline = $flow_settings['tagline'] ?? '';
        $flow_slug = $flow_settings['slug'] ?? sanitize_title(pathinfo($ivr_basename, PATHINFO_FILENAME));
    ?>
    <div id="flosc_admin_verification" class="flosc-admin-verification" style="
        position: fixed;
        top: 0;
        left: 0;
        right: 0;
    ...
    ">
        <div style="display: flex; align-items: center; gap: 15px; flex-wrap: wrap;">
            ...
            <span>
                <strong>IVR:</strong> <?php echo esc_html($ivr_basename); ?>
            </span>
            <span>
                <strong>Slug:</strong> /<?php echo esc_html($flow_slug); ?>/
            </span>
            <?php if ($flow_name): ?>
            <span>
                <strong>Name:</strong> <?php echo esc_html($flow_name); ?>
            </span>
            <?php endif; ?>
            <?php if ($flow_tagline): ?>
            <span style="opacity: 0.8;">
                "<?php echo esc_html($flow_tagline); ?>"
            </span>
            <?php endif; ?>
        </div>
        <button onclick="this.parentElement.style.display='none'; localStorage.setItem('flosc_admin_verified_<?php echo esc_js($ivr_basename); ?>', '1');" style="
    ...
    <script>
        ...
        if (localStorage.getItem('flosc_admin_verified_<?php echo esc_js($ivr_basename); ?>')) {
            document.getElementById('flosc_admin_verification').style.display = 'none';
        }
    </script>
    <?php endif; ?>
        ...
        $ivr_version = file_exists($ivr_file) ? filemtime($ivr_file) : time();
            ...
            if (empty($ivr_config['messages'])) {
                if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) {
                    error_log('FLOSC v1.2.3: WARNING - No IVR messages loaded from ' . $ivr_filename);
                    error_log('FLOSC v1.2.3: Config keys: ' . implode(', ', array_keys($ivr_config)));
                }
            
                // Force re-parse if empty
                if (file_exists($ivr_file)) {
                    if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) {
                        error_log('FLOSC v1.2.3: ' . $ivr_filename . ' exists, forcing re-parse...');
                    }
                    $markdown = file_get_contents($ivr_file);
                    $ivr_config = $ivr_parser->flosc_parse($markdown);
                    if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) {
                        error_log('FLOSC v1.2.3: Re-parse complete. Messages: ' . count($ivr_config['messages'] ?? []));
                    }
                } else {
                    if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) {
                        error_log('FLOSC v1.2.3: ERROR - IVR file not found at: ' . $ivr_file);
                    }
                }
            } else {
                if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) {
