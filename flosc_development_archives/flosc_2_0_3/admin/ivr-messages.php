...
<?php if ($import_preview !== null): ?>
    ...
    <div class="flosc-import-preview" style="background: #fff3cd; padding: 20px; border-left: 5px solid #ffc107; margin-bottom: 20px;">
        ...
        <?php
        // v2.0.0: Field-level diff table for updated messages
        $field_diffs = $import_preview['field_diffs'] ?? [];
        if (!empty($field_diffs)): ?>
        <div style="background: white; padding: 15px; margin: 15px 0; border: 1px solid #ddd;">
            <h4 style="margin-top: 0;">🔍 Field-Level Differences (<?php echo count($field_diffs); ?> message<?php echo count($field_diffs) !== 1 ? 's' : ''; ?>):</h4>
            <?php foreach ($field_diffs as $msg_id => $diffs): ?>
            <details style="margin-bottom: 10px; border: 1px solid #e0e0e0; border-radius: 4px;">
                <summary style="padding: 8px 12px; background: #f7f7f7; cursor: pointer; font-weight: 600;">
