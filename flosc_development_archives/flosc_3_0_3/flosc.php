    ...
    public function get_floscflow_identity() {
        ...
        if ($flow) {
            ...
            return [
                ...
                'price'           => $id['price'] ?? get_option('flosc_product_price', '25'),
...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7. Ready to dive in?\"",
    ...
    public function create_payment_intent($request) {
        ...
        // v1.6.3: Fall back to flow identity price (in cents) if no Stripe price_id configured
        $price_or_amount = $price_id;
        $currency = 'usd';
        if (!$price_or_amount) {
            $identity = $this->get_floscflow_identity();
            $raw_price = floatval($identity['price'] ?? 0);
            if ($raw_price <= 0) {
    ...
    public function paypal_create_order($request) {
        ...
        if ($amount <= 0) {
            // Fallback to flow identity price (set in admin Identity tab)
            $identity = $this->get_floscflow_identity();
            $amount = floatval($identity['price'] ?? 0);
        }
        if ($amount <= 0) {
