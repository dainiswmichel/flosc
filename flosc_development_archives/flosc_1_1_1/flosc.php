...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7: [link]. It has the complete IPA guide and video walkthrough. Ready to dive in?\"",
    ...
    private function generate_user_status_response($context) {
        // v1.1.1: Check actual WordPress state directly, not just context
        // This prevents any issues with context not being passed correctly
        $is_logged_in = is_user_logged_in();
        $user_id = get_current_user_id();
        
        // v1.1.0: Visitor (not logged in)
        if (!$is_logged_in) {
        ...
        // v1.1.1: FLOSC Admin (WordPress admin) - check first!
        ...
        $is_member = $member_access->flosc_is_member($user_id);
        $member_levels = $member_access->flosc_get_user_member_levels($user_id);
