    ...
    private function evaluate_single($condition) {
        ...
        switch ($condition) {
            ...
            case 'is_visitor':
                return !is_user_logged_in();
            case 'is_member':
                // Member = has purchased / has access
                return !empty($this->context['purchased']) || !empty($this->context['is_member']);
            case 'is_guest':
                // Guest = logged in but not a member yet
                return is_user_logged_in() && empty($this->context['purchased']) && empty($this->context['is_member']);
            case 'purchased':
