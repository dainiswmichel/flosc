...
class FLOSC_Flow_Manager {
    ...
    public function get_available_ivr_files() {
        ...
        // Get all *_ivr.md files in base directory
        $ivr_files = glob($base_path . '*_ivr.md');
        foreach ($ivr_files as $file) {
            $files[] = basename($file);
        }
        
        // Also check for legacy ivr.md (backward compatibility)
        if (file_exists($base_path . 'ivr.md')) {
            $files[] = 'ivr.md';
        }
    ...
    public function maybe_migrate_from_legacy() {
        ...
        if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) {
            error_log('FLOSC v1.2.6: Created ' . count($default_flows) . ' default flows');
        }
