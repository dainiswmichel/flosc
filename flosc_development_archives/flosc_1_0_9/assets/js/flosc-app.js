 ...
 * v1.0.9: User status query, IVR message naming cleanup
 */

// v9.4.7: Clear FLOSC-specific localStorage on version change
(function() {
    const FLOSC_JS_VERSION = '1.0.9';
    try {
...
class floscApp {
    ...
    floscShowUserAutoPrompts() {
        ...
        // v9.3.6: Always show panel for all user states (floscVisitorIntroPanel, floscGuestPromptPanel, floscMemberPromptPanel)
        ...
        if (applicable.length === 0) {
            console.log('FLOSC: No user autoprompts from IVR for phase:', this.ivr.phase, 'state:', this.state);
    ...
    floscCleanupUserAutoPrompts() {
        // v1.0.8: Clear auto-rotate timer
        if (this.carouselAutoRotateTimer) {
            clearInterval(this.carouselAutoRotateTimer);
            this.carouselAutoRotateTimer = null;
        }
    ...
    floscRenderUserAutoPrompts(replies) {
        ...
        // v1.0.1: Carousel structure with hidden arrows (shown on overflow)
        container.innerHTML = `
            ...
            <div class="prompt-panel-body">
                <div class="flosc-carousel-container">
                    <button class="flosc-carousel-arrow flosc-carousel-prev" aria-label="Previous" disabled>‹</button>
        ...
        // v1.0.8: Initialize carousel with overflow detection + auto-rotate for visitors only
        const enableAutoRotate = (this.state === 'visitor');
        this.initCarouselOverflow(container, enableAutoRotate);
    }

    /**
     * v1.0.1: Initialize carousel based on content overflow (not item count)
     * Arrows hidden by default, shown only when content overflows
     */
    initCarouselOverflow(container, enableAutoRotate = false) {
        ...
        const checkOverflow = () => {
            const needsCarousel = track.scrollWidth > track.clientWidth + 5; // 5px tolerance
        ...
        // Simple carousel navigation - scroll by 80% of visible width
        const scrollAmount = () => track.clientWidth * 0.8;

        prevBtn.addEventListener('click', () => {
            track.scrollBy({ left: -scrollAmount(), behavior: 'smooth' });
            setTimeout(checkOverflow, 400);
            // v1.0.8: Reset auto-rotate timer on manual interaction
            if (this.carouselAutoRotateTimer) {
                clearInterval(this.carouselAutoRotateTimer);
                startAutoRotate();
            }
        ...
        this.activeEventListeners.set(window, handleResize);

        // v1.0.8: Auto-rotation for visitor carousel
        const startAutoRotate = () => {
            if (!enableAutoRotate) return;
            
            this.carouselAutoRotateTimer = setInterval(() => {
                const atEnd = track.scrollLeft >= track.scrollWidth - track.clientWidth - 5;
                if (atEnd) {
                    // Loop back to start
