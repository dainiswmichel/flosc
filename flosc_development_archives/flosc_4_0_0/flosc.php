    ...
    private function render_flosc_app() {
        ...
        if (current_user_can('manage_options') && !empty($_GET['flosc_test_offer'])) {
            ...
            if (wp_verify_nonce($nonce, 'flosc_test_offer_' . $oid)) {
                $test_offer_id = $oid;
                $all_raw_offers = $this->sale_manager->offer_manager->get_all_offers($flow_id);
        ...
        if ( is_user_logged_in() && current_user_can( 'manage_options' ) ) {
            $all_raw = $this->sale_manager->offer_manager->get_all_offers( $flow_id );
