...
class floscApp {
    constructor() {
        ...
        // IntroPanel auto-hide timer
        this.introPanelTimer = null;
    ...
    async init() {
        ...
        // Restore visitor messages from localStorage (for chat persistence across refreshes)
        if (this.state === 'visitor') {
    ...
    bindEvents() {
        ...
        this.messageInput?.addEventListener('input', () => {
            ...
            // v04_03: Update last interaction to pause inactivity timers while typing
            if (this.ivr) {
        ...
        // IntroPanel close button (v3.0.8)
        const introPanelClose = document.getElementById('introPanelClose');
        if (introPanelClose) {
            introPanelClose.addEventListener('click', () => this.handleIntroPanelClose());
        }
    ...
    setupUI() {
        if (this.state !== 'visitor' && this.user) {
            ...
            if (this.greetingTitle) {
                this.greetingTitle.textContent = `Welcome back, ${this.user.name?.split(' ')[0] || 'there'}!`;
            }
        }

        // IntroPanel: Check if user manually closed it
        const introPanelClosed = localStorage.getItem('flosc_intro_panel_closed') === 'true';
        const introPanel = document.getElementById('introPanel');

        if (introPanel) {
            if (introPanelClosed) {
                // User manually closed it - keep it hidden
                introPanel.style.display = 'none';
            } else {
                // Show IntroPanel and start 300-second auto-hide timer (resets on each page load)
                introPanel.style.display = 'block';
                this.introPanelTimer = setTimeout(() => {
                    this.hideIntroPanel();
    ...
    /**
     * Handle prompt card actions - configurable via IVR settings
     */
    async handlePromptCardAction(action) {
        // Special case: start-quiz opens modal directly
        ...
        if (phaseConfig && phaseConfig.prompt_cards) {
            // Find the card config for this action
            cardConfig = phaseConfig.prompt_cards.find(card => card.action === action);
        }

        // Fallback to default messages if no IVR config
        if (!cardConfig) {
            console.warn('Using fallback message for action:', action);
            const fallbacks = {
                'get-started': {
                    user_message: 'Get started',
                    assistant_response: this.config.messages?.getStarted || "Welcome! I'm here to help you. The best way to begin is by taking our free quiz. Ready to get started?",
                    enabled: true,
                    send_to_ai: false
                },
                'how-it-works': {
                    user_message: 'How does it work?',
                    assistant_response: this.config.messages?.howItWorks || "Here's how it works: Take a quick quiz to assess your level, get personalized feedback, and unlock content based on your results. Want to try?",
                    enabled: true,
                    send_to_ai: false
                },
                'what-you-learn': {
                    user_message: 'What will I learn?',
                    assistant_response: this.config.messages?.whatYouLearn || "You'll learn through personalized lessons tailored to your needs. Take the quiz to see what's recommended for you!",
                    enabled: true,
                    send_to_ai: false
                }
            };
            ...
            if (!cardConfig) {
                console.warn('No config or fallback found for action:', action);
                return;
            }
        }
        ...
        if (!cardConfig.enabled) {
            console.log('Prompt card disabled:', action);
            return;
        }
        ...
        // Add user message
        this.addMessage('user', cardConfig.user_message);

        // Track user interaction
        this.onUserInteraction();
        this.trackEvent('prompt_card_clicked', { action });

        // Determine response method
        if (cardConfig.send_to_ai) {
            // Send to AI
            this.typingIndicator?.classList.add('show');

            try {
                const response = await this.api('ai-query', 'POST', { message: cardConfig.user_message });
                ...
                this.typingIndicator?.classList.remove('show');
                this.addMessage('assistant', "I'm having trouble responding right now. Please try again in a moment.");
            }
        } else {
            // Use configured response with slight thinking delay
            this.typingIndicator?.classList.add('show');
    ...
    async sendMessage() {
        ...
        // IVR Command System (v3.0.8) - Process before AI
        // IntroPanel commands
        if (lowerMessage === 'show intropanel') {
            this.addMessage('user', message);
            this.addMessage('assistant', 'Here\'s the IntroPanel again!');
            this.showIntroPanel();
        ...
        if (lowerMessage === 'hide intropanel') {
            this.addMessage('user', message);
        ...
        if (lowerMessage === 'show profile status' ||
            lowerMessage === 'profile status' ||
            lowerMessage === 'my profile status' ||
            lowerMessage === 'what is my profile status') {
            this.addMessage('user', message);
        ...
        if (lowerMessage === 'show token count') {
            this.addMessage('user', message);
            this.showTokenCount();
            this.messageInput.value = '';
            this.autoResizeTextarea();
        ...
        if (lowerMessage === 'show quiz score') {
            this.addMessage('user', message);
            this.showQuizScore();
            this.messageInput.value = '';
            this.autoResizeTextarea();
        ...
        if (lowerMessage === 'show lessons available') {
            this.addMessage('user', message);
            this.showLessonsAvailable();
            this.messageInput.value = '';
            this.autoResizeTextarea();
            return;
        }
        ...
        // v04_03: Track user interaction for IVR
        this.onUserInteraction();
        ...
        if (storedScore) {
            try {
                ...
                if (scoreData.timestamp && (Date.now() - scoreData.timestamp) < 3600000) {
                    ...
                    // Clear visitor messages from localStorage (now logged in)
                    localStorage.removeItem('flosc_visitor_messages');

                    // v04_03: Transition to login phase (logged in, not yet offered)
     ...
     */
    showUpgradeOffer() {
        // v05_07: Mark offer as shown (persist to server)
        if (this.user?.id) {
        ...
        this.user.offerShown = true;

        // v05_07: Use IVR config if available
        const offerConfig = this.ivr?.config?.offer;
        let message;
        ...
        // v04_03: Transition to offer phase (presenting the sales pitch)
    ...
    addMessage(role, content) {
        ...
        if (role === 'user') {
            ...
            // v05_08: Parse markdown and sanitize to prevent XSS
        ...
        // Store message in localStorage for visitors (for persistence across refreshes)
        if (this.state === 'visitor') {
            this.storeVisitorMessage(role, content);
        }
    }

    /**
     * Store visitor message in localStorage
     */
    storeVisitorMessage(role, content) {
        try {
            const stored = localStorage.getItem('flosc_visitor_messages');
            ...
            console.error('Failed to store visitor message:', e);
        }
    }

    /**
     * Restore visitor messages from localStorage
     */
    restoreVisitorMessages() {
        try {
            ...
            // Check if messages are recent (within last 24 hours)
            ...
            if (recentMessages.length > 0) {
                ...
                recentMessages.forEach(msg => {
                    // Render message without re-storing it
                    const emoji = this.config.product?.emoji || '🎯';
                    const userInitial = this.user?.name?.charAt(0).toUpperCase() || 'U';
                ...
                // Messages are old, clear them
                localStorage.removeItem('flosc_visitor_messages');
            }
        } catch (e) {
            console.error('Failed to restore visitor messages:', e);
            localStorage.removeItem('flosc_visitor_messages');
        }
    }
    ...
    // v05_08: Sanitize HTML to prevent XSS attacks
    sanitizeHtml(html) {
            ...
            if (result.success) {
                ...
                if (result.analysis) {
                    ...
                    if (this.state === 'visitor') {
                        ...
                        // Mark quiz as taken and transition to login phase
                        localStorage.setItem('flosc_quiz_taken', 'true');
                        this.transitionToPhase('login');
    ...
    async processPayment() {
        ...
        try {
            ...
            if (error) {
                ...
                // v04_03: Transition to sale phase (post-purchase onboarding)
                this.transitionToPhase('sale');
    ...
    showIntroPanel() {
        ...
        if (panel) {
            ...
            // Clear the manual close flag (user wants to see it again)
            localStorage.removeItem('flosc_intro_panel_closed');

            // Start auto-hide timer when showing IntroPanel
            if (this.introPanelTimer) {
                clearTimeout(this.introPanelTimer);
            }
            this.introPanelTimer = setTimeout(() => {
                this.hideIntroPanel();
            }, 300000); // 300 seconds
    ...
    handleIntroPanelClose() {
        // Cancel auto-hide timer if user manually closes
        if (this.introPanelTimer) {
            clearTimeout(this.introPanelTimer);
            this.introPanelTimer = null;
        }

        // Remember that user manually closed it (persist across refreshes)
        localStorage.setItem('flosc_intro_panel_closed', 'true');
    ...
    // IVR Status Commands (v3.0.8)
    showProfileStatus() {
        const tokenName = FLOSC_CONFIG.tokenName || 'tokens';
        const responses = {
            visitor: `You are a Visitor.\n\nNext steps:\n1. Take the free quiz\n2. Create account to see your score\n3. Get your FREE personalized lesson + 100 ${tokenName}\n4. Upgrade for full access\n\nStart now!`,

            free: `You are a Guest.\n\nStatus:\n• Quiz score: ${this.user?.lastQuizScore || 'N/A'}%\n• Free lesson: ${this.freeLessonDelivered ? 'Delivered' : 'Pending'}\n• ${tokenName.charAt(0).toUpperCase() + tokenName.slice(1)}: ${this.user?.tokens || 0}/100 remaining\n\nUse your ${tokenName} to explore your free lesson.\n\nUpgrade to Member:\n• 1000 ${tokenName}/month\n• Full lesson library\n• Unlimited quiz retakes\n\nType "Upgrade" to unlock everything!`,

            paid: `You are a Member. 🎉\n\nStatus:\n• Full lesson library access\n• ${tokenName.charAt(0).toUpperCase() + tokenName.slice(1)}: ${this.user?.tokens || 0}/1000 this month\n• Tokens reset: [billing date]\n\nYou have full access to all lessons and features!`
        };

        this.addMessage('assistant', responses[this.state] || responses.visitor);
    }
    ...
    showTokenCount() {
        ...
        if (this.state === 'visitor') {
            this.addMessage('assistant', `You don't have ${tokenName} yet. Complete the quiz and create an account to earn 100 ${tokenName}!`);
        } else if (this.state === 'free') {
            this.addMessage('assistant', `You have ${this.user?.tokens || 0}/100 ${tokenName} remaining.\n\nUpgrade to Member for 1000 ${tokenName}/month!`);
        } else if (this.state === 'paid') {
            this.addMessage('assistant', `You have ${this.user?.tokens || 0}/1000 ${tokenName} this month.\n\nYour ${tokenName} reset on [billing date].`);
        }
    }

    showQuizScore() {
        if (!this.user?.lastQuizScore) {
            this.addMessage('assistant', 'You haven\'t taken the quiz yet. Click "Start free quiz" to get started!');
        } else {
            this.addMessage('assistant', `Your last quiz score: ${this.user.lastQuizScore}%\n\n${this.user.lastQuizScore >= 70 ? 'Great work!' : 'Keep practicing - you\'re making progress!'}`);
        }
    }

    showLessonsAvailable() {
        if (this.state === 'visitor') {
            this.addMessage('assistant', 'Take the quiz first to unlock your personalized free lesson!');
        } else if (this.state === 'free') {
            this.addMessage('assistant', `Your Free Lesson:\n• ${this.config.product?.name || 'Introduction'} Lesson 1\n\nUpgrade to unlock the full course library with ${this.config.product?.lessonCount || '20+'} lessons!`);
        } else if (this.state === 'paid') {
            this.addMessage('assistant', `Full Course Library Available:\n\n${this.config.product?.name || 'Complete Course'}\n• ${this.config.product?.lessonCount || '20+'} comprehensive lessons\n• Personalized learning path\n• Unlimited access\n\nType "My lessons" to see your personalized path.`);
        }
    }
    ...
    determineFLOSCPhase() {
        // Determine current FLOSC phase based on user state
        if (this.user.purchased || this.user.access === 'full') {
            return 'content';
        }

        if (this.user.funnelCompleted && this.state !== 'visitor') {
            return 'content'; // They have access to content
        }

        if (this.state === 'visitor') {
            // Check if quiz was taken (stored in localStorage)
            const quizTaken = localStorage.getItem('flosc_quiz_taken') === 'true';
            return quizTaken ? 'login' : 'freeline';
        }

        // Logged in
        if (!this.user.offerShown) {
            return 'login'; // Show results + free lesson
        }

        if (!this.user.purchased) {
            return 'offer'; // Show sales pitch
        }

        if (this.user.purchased && !this.user.onboarded) {
            return 'sale'; // Post-purchase onboarding
        }
    ...
    startIVR() {
        // v05_07: Don't show initial message if IntroPanel is visible
        const introPanel = document.getElementById('introPanel');
    ...
    replaceIVRVariables(text) {
        return text
            .replace(/{name}/g, this.user?.name?.split(' ')[0] || 'there')
            .replace(/{score}/g, this.user?.quizScore || '0')
            .replace(/{product_name}/g, this.config.product?.name || 'the course')
            .replace(/{price}/g, this.config.product?.price || '$47')
            .replace(/{product_description}/g, this.config.product?.description || '');
    }

    onUserInteraction() {
        // Called whenever user sends a message or interacts
        this.ivr.lastInteraction = Date.now();
        this.ivr.messageCount++;

        // Clear existing inactivity timers
        this.ivr.inactivityTimers.forEach(timer => clearTimeout(timer));
        this.ivr.inactivityTimers = [];

        // Check triggered messages
        this.checkTriggeredMessages();

        // Check pool messages
        this.checkPoolMessages();
    ...
    checkTriggeredMessages() {
        const phaseConfig = this.ivr.config[this.ivr.phase];
        if (!phaseConfig || !phaseConfig.triggered_messages) return;

        phaseConfig.triggered_messages.forEach(msg => {
            if (!msg.enabled) return;

            if (this.evaluateConditions(msg.conditions)) {
                // Show message
                this.addMessage('assistant', this.replaceIVRVariables(msg.text));
    ...
    checkPoolMessages() {
        const phaseConfig = this.ivr.config[this.ivr.phase];
        if (!phaseConfig || !phaseConfig.pool_messages) return;

        // Filter messages that meet conditions
        const eligible = phaseConfig.pool_messages.filter(msg =>
            msg.enabled && this.evaluateConditions(msg.conditions)
        );

        if (eligible.length > 0) {
            // Pick random message from eligible pool
    ...
    evaluateConditions(conditions) {
        if (!conditions || Object.keys(conditions).length === 0) {
            return false; // No conditions = not eligible
        }

        // Message count condition
        if (conditions.message_count !== undefined) {
            if (this.ivr.messageCount !== conditions.message_count) {
                return false;
            }
        }

        // Session time condition (minutes)
        if (conditions.session_minutes !== undefined) {
            const elapsed = (Date.now() - this.ivr.sessionStart) / 1000 / 60;
            if (elapsed < conditions.session_minutes) {
                return false;
            }
        }

        // Inactivity condition (seconds)
        if (conditions.inactivity_seconds !== undefined) {
            const elapsed = (Date.now() - this.ivr.lastInteraction) / 1000;
            if (elapsed < conditions.inactivity_seconds) {
                return false;
            }
        }

        // Quiz taken condition
        if (conditions.quiz_taken !== undefined) {
            const quizTaken = localStorage.getItem('flosc_quiz_taken') === 'true';
            if (quizTaken !== conditions.quiz_taken) {
                return false;
            }
        }
    ...
    performIVRAction(action) {
        switch(action) {
            case 'open_quiz':
                ...
                break;
            case 'open_login':
                setTimeout(() => this.openLoginGate(), 1000);
                break;
            case 'open_payment':
                setTimeout(() => this.openPaymentModal(), 1000);
                break;
            case 'warn_close':
                // Show countdown and schedule close
                setTimeout(() => this.closeChat(), 30000);
                break;
        }
    }
    ...
    closeChat() {
        ...
        this.ivr.sequentialIndex = 0;
        this.ivr.initialMessageShown = false;
    }
    ...
    scheduleInactivityChecks() {
        ...
        // Find all inactivity-based triggers
        phaseConfig.triggered_messages.forEach(msg => {
            if (!msg.enabled) return;
            if (!msg.conditions.inactivity_seconds) return;

            const timer = setTimeout(() => {
                // Check if condition still valid
                const elapsed = (Date.now() - this.ivr.lastInteraction) / 1000;
                if (elapsed >= msg.conditions.inactivity_seconds) {
                    this.addMessage('assistant', this.replaceIVRVariables(msg.text));
                    if (msg.action) {
                        this.performIVRAction(msg.action);
                    }
                }
            }, msg.conditions.inactivity_seconds * 1000);

            this.ivr.inactivityTimers.push(timer);
        });
    }

    transitionToPhase(newPhase) {
        // Change FLOSC phase and show initial message
        this.ivr.phase = newPhase;
        this.ivr.initialMessageShown = false;
        this.ivr.sequentialIndex = 0;
        this.startIVR();
    }
