 ...
 * v9.4.2: Security hardening - signed cookies, rate-limited endpoints
 */

// v9.4.2: Clear FLOSC-specific localStorage on version change
(function() {
    const FLOSC_JS_VERSION = '9.4.8';
    try {
...
class floscApp {
    constructor() {
        ...
        localStorage.removeItem('flosc_prompts_hidden');
    ...
    cleanupSuggestedReplies() {
        ...
        // Keep the placeholder container (it's in the PHP template) and just clear it.
        const existing = document.getElementById('flosc_output_chat_suggested_replies');
        if (existing) {
            existing.innerHTML = '';
            // Reset to its base class so layout stays predictable
            existing.className = 'flosc-suggested-replies-container';
        }
    }
    ...
    renderSuggestedReplies(replies) {
        ...
        // Respect user hide preference
        if (localStorage.getItem('flosc_prompts_hidden') === 'true') return;
        ...
        let container = document.getElementById('flosc_output_chat_suggested_replies');
        if (!container) {
            container = document.createElement('div');
            container.id = 'flosc_output_chat_suggested_replies';
            container.className = 'flosc-suggested-replies-container';
            const chatContainer = document.getElementById('flosc_chat_container');
            const composer = document.getElementById('flosc_input_composer');
            if (chatContainer && composer) {
                chatContainer.insertBefore(container, composer);
            } else if (chatContainer) {
                chatContainer.appendChild(container);
            }
        }

        // Turn placeholder into a panel
        container.className = 'flosc-suggested-replies-container prompt-panel prompt-panel-inline';
        container.innerHTML = `
            ...
            <div class="prompt-panel-body">
                <div class="flosc-carousel-container">
                    ...
                    <div class="flosc-carousel-track" role="list">
                        ${replies.map(r => `
                            <button class="flosc-style-${r.style || 'pill'}" role="listitem" data-message="${this.escapeHtml(r.name)}">
        ...
        // Button click handlers (track listeners for cleanup)
        ...
        // Close button: just hide panel (no fake chat messages)
        const closeBtn = container.querySelector('.prompt-panel-close');
        if (closeBtn) {
            closeBtn.addEventListener('click', () => {
                localStorage.setItem('flosc_prompts_hidden', 'true');
                container.innerHTML = '';
                container.className = 'flosc-suggested-replies-container';
            });
        }

        // Initialize carousel (bounded, no "zip back")
    ...
    initCarousel(container) {
        ...
        const updateButtons = () => {
            ...
            nextBtn.disabled = left >= Math.max(0, maxScroll - 1);
        };

        const scrollByStep = (dir) => {
            const step = getStep();
            track.scrollBy({ left: dir * step, behavior: 'smooth' });
            // Update shortly after scroll starts
...
Purchased: ${ctx.purchased}
    ...
    bindElements() {
        ...
        this.mobileMenuBtn = document.getElementById('flosc_app_mobile_menu_button');
    ...
    setupUI() {
        ...
        if (this.sidebar) {
            ...
            if (isMobile) {
                ...
                if (sidebarOpen) this.sidebar.classList.add('open');
            } else {
                const collapsed = localStorage.getItem('flosc_sidebar_collapsed') === 'true';
        ...
        if (this.sidebar && this.sidebarToggle) {
            ...
            this.sidebarToggle.textContent = isMobile ? (open ? '✕' : '☰') : (collapsed ? '☰' : '✕');
        }
    ...
    toggleSidebar() {
        ...
        if (isMobile) {
            ...
            // Desktop: collapse to a slim rail
