...
class floscApp {
    ...
    async init() {
        ...
        try {
            ...
            // v1.7.9: Visitor bar DISABLED by default per floscAdmin feedback 2026-02m-14d.
            // Enable via flosc_visitor_bar_enabled option in wp-admin.
    ...
    buildIVRContext() {
        ...
        this.ivr.context = {
            ...
            // Product info (from FLOSC_CONFIG, set by floscAdmin)
            product_name: this.config.product?.name || 'this program',
            price: this.config.product?.price || '',
            discount_price: this.config.product?.discount_price || '',
            customer_count: this.config.product?.customer_count || '',
    ...
    async loadOfferContentSource(msg, offer, displayFormat) {
        ...
        try {
            ...
            if (htmlFile) {
                ...
                const resp = await fetch(`${this.config.apiUrl}/offer-content?source=html&file=${encodeURIComponent(htmlFile)}`, { credentials: 'same-origin' });
                if (resp.ok) {
                ...
                const resp = await fetch(`${this.config.apiUrl}/offer-content?source=woo&product=${encodeURIComponent(wooProduct)}`, { credentials: 'same-origin' });
                if (resp.ok) {
                    const data = await resp.json();
                    if (data.html) content = data.html;
                ...
                const resp = await fetch(`${this.config.apiUrl}/offer-content?source=post&id=${encodeURIComponent(postId)}`, { credentials: 'same-origin' });
                if (resp.ok) {
                    const data = await resp.json();
                    if (data.html) content = data.html;
                }
    ...
    async startInChatQuiz(quizId = 'default') {
        ...
        try {
            ...
            const response = await fetch(`${this.config.apiUrl}/quiz?id=${quizId}`, { credentials: 'same-origin' });
            const data = await response.json();
    ...
    handleVisitorMenuAction(action) {
        ...
        switch (action) {
            ...
            case 'quiz':
                ...
                this.sendMessage('start free quiz');
                break;
            default:
                this.log('[FLOSC] Unknown visitor action:', action);
        }
    }
    ...
    async processSandboxPayment(offerId, productId = '') {
        ...
        try {
            ...
            if (!result.success && ((result.message || '').match(/cookie|not allowed/i) || result.code === 'rest_cookie_invalid_nonce')) {
                ...
                if (refreshed) {
                    const retryResponse = await fetch(`${this.config.apiUrl}/sandbox-purchase`, {
                        method: 'POST',
                        credentials: 'same-origin',
                        headers: {
                            'Content-Type': 'application/json',
...
Purchased: ${ctx.purchased}
    ...
    async refreshNonce() {
        try {
            const res = await fetch(this.config.restUrl + 'nonce', {
                credentials: 'same-origin'
            });
            if (res.ok) {
    ...
    bindEvents() {
        ...
        const profileBtn = document.getElementById('flosc_app_profile_button');
        const profileDropdown = document.getElementById('flosc_app_profile_dropdown');
        ...
        // Visitor profile button dropdown toggle
        const visitorProfileBtn = document.getElementById('flosc_visitor_profile_button');
        const visitorProfileDropdown = document.getElementById('flosc_visitor_profile_dropdown');
        if (visitorProfileBtn && visitorProfileDropdown) {
            visitorProfileBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                visitorProfileDropdown.classList.toggle('open');
                visitorProfileBtn.setAttribute('aria-expanded', visitorProfileDropdown.classList.contains('open'));
            });

            // Handle visitor menu item clicks
            visitorProfileDropdown.querySelectorAll('[data-action]').forEach(item => {
                item.addEventListener('click', (e) => {
                    e.preventDefault();
                    const action = e.target.dataset.action;
                    this.handleVisitorMenuAction(action);
                    visitorProfileDropdown.classList.remove('open');
                });
            });
        }
        ...
        document.addEventListener('click', (e) => {
            if (profileDropdown && !e.target.closest('.user-profile-card')) {
                profileDropdown.classList.remove('open');
                if (profileBtn) profileBtn.setAttribute('aria-expanded', 'false');
            }
            if (visitorProfileDropdown && !e.target.closest('#flosc_visitor_profile_card')) {
                visitorProfileDropdown.classList.remove('open');
                if (visitorProfileBtn) visitorProfileBtn.setAttribute('aria-expanded', 'false');
            }
        });
    ...
    async sendMessage(directMessage = null) {
    ...
    findIVRResponse(userMessage) {
        ...
        const keywordMatch = allMessages.find(m => {
        ...
        if (keywordMatch) {
            this.log('[FLOSC-FIND] Keyword match found:', keywordMatch.name, 'via keywords');
            return keywordMatch;
        }
    ...
    addMessage(role, content, isHtml = false) {
        ...
        if (role === 'user') {
            ...
            // v1.7.1: Format assistant messages with markdown support
    ...
    async callAPI(message) {
        ...
        if (this.state !== 'visitor' && !this.currentSession) {
            try {
                const sessionRes = await fetch(this.config.apiUrl + '/sessions', {
                    method: 'POST',
                    credentials: 'same-origin',
                    headers: {
                        'Content-Type': 'application/json',
