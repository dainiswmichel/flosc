 ...
 * v1.1.5: Verified credentials fix for user status check
 */

// v9.4.7: Clear FLOSC-specific localStorage on version change
(function() {
    const FLOSC_JS_VERSION = '1.1.5';
    try {
...
class floscApp {
    ...
    /**
     * ============================================================================
     * CAROUSEL - v1.1.3 (Complete Rewrite)
     * ============================================================================
     * MTS-2026-02-02: Previous versions failed due to:
     * 1. Running overflow check before DOM was ready
     * 2. Overcomplicated logic with clones and transforms
     * 
     * This version: DEAD SIMPLE
     * - Check overflow after DOM renders (using multiple timing methods)
     * - Native scroll only - no transforms, no clones
    ...
    initCarouselOverflow(container) {
        ...
        console.log('[FLOSC Carousel] Initializing. Items:', track.children.length);

        // Fixed scroll amount per click
        const SCROLL_STEP = 200;

        // Check if content overflows and show/hide arrows
        const updateArrowVisibility = () => {
            const hasOverflow = track.scrollWidth > track.clientWidth + 5;
            
            console.log('[FLOSC Carousel] Overflow check:', {
            ...
            if (hasOverflow) {
                ...
                carouselEl.classList.remove('has-overflow');
        ...
        // MTS-2026-02-02: Run overflow check multiple times to catch all render timing scenarios
        // 1. Immediate check (sync)
        updateArrowVisibility();
        
        // 2. After paint (rAF)
        requestAnimationFrame(updateArrowVisibility);
        
        // 3. After layout settles (short delay)
        setTimeout(updateArrowVisibility, 100);
        
        // 4. After fonts/images load (longer delay)
...
Purchased: ${ctx.purchased}
    ...
    findIVRResponse(userMessage) {
        // MTS-2026-02-02: [FIX] Check BOTH API-loaded messages AND full config messages
        // API only returns phase-filtered messages, but global messages like user_status_check
        // need to be found in the full config set
