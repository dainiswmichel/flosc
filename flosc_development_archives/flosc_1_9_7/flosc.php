...
The {product_name} Team";
    ...
    public function handle_chat($request) {
        ...
        if (!empty($frontend_ivr_guidance)) {
            ...
            // No frontend match — try server-side IVR matching
...
function flosc_get_app_icon_url($size = '') {
    // v1.9.7: Kept for backward compat. Prefers favicon, falls back to logo, then default.
    $url = flosc_get_favicon_url($size);
    if ($url) return $url;
    $url = flosc_get_logo_url();
    if ($url) return $url;
    $suffix = $size ? "-{$size}" : '';
...
/**
 * v1.9.7: Get the flow's favicon URL (browser tab icon).
 * Uses product.app_icon_url. Falls back to bundled FLOSC default icon.
 */
...
/**
 * v1.9.7: Get the flow's logo URL (landing state header image, sidebar logo).
 * Uses product.logo_url. Falls back to bundled FLOSC default icon.
 */
function flosc_get_logo_url() {
    $product = FLOSC_Framework::instance()->get_product_config();
