...
class FLOSC_Pronunciation_Analyzer {
    ...
    // Phoneme lessons mapping (for LeSAEp-style quiz)
    private $lesson_mapping = [
    ...
    public function analyze($transcript, $expected) {
        ...
        foreach ($expected_items as $index => $expected_item) {
            ...
            if (!$found) {
                ...
                if (isset($this->lesson_mapping[$expected_item])) {
                    $lesson = $this->lesson_mapping[$expected_item];
        ...
        if (!empty($results['missed_items'])) {
            foreach ($results['missed_items'] as $missed) {
                if (isset($this->lesson_mapping[$missed])) {
                    $results['free_lesson'] = array_merge(
                        $this->lesson_mapping[$missed],
                        ['item' => $missed]
                    );
                    break;
                }
