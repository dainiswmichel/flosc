...
The {product_name} Team";
    ...
    /**
     * Enqueue chat styling (v9.3.7 - Clean Architecture)
     * 
     * Loads preset CSS file (variable definitions only)
     * Then applies inline CSS for bubble style, accent, font, scale, and custom CSS
     */
    private function enqueue_chat_style() {
        ...
        $accent = get_option('flosc_chat_style_accent', '#2563eb');
        ...
        // Bubble style presets (border-radius values)
        $bubble_styles = [
        ...
        if ($preset === 'auto') {
            // Inject both variable sets using prefers-color-scheme
            $light_vars = file_get_contents(FLOSC_PLUGIN_DIR . 'assets/css/chat-style-light.css');
            $dark_vars = file_get_contents(FLOSC_PLUGIN_DIR . 'assets/css/chat-style-dark.css');
            // Remove comments and :root wrappers
            $light_vars = preg_replace('/\/\*.*?\*\//s', '', $light_vars);
            $light_vars = preg_replace('/^\s*:root\s*{/', '', $light_vars);
            $light_vars = preg_replace('/}\s*$/', '', $light_vars);
            $dark_vars = preg_replace('/\/\*.*?\*\//s', '', $dark_vars);
            $dark_vars = preg_replace('/^\s*:root\s*{/', '', $dark_vars);
            $dark_vars = preg_replace('/}\s*$/', '', $dark_vars);
            $inline_css .= ":root {\n$light_vars}\n";
        ...
        // Always add dynamic overrides (bubble, accent, font, scale, custom CSS)
        ...
        if ($font !== 'system') {
            $font_family = $font_families[$font] ?? $font_families['system'];
            $inline_css .= "    --flosc-font-family: {$font_family};\n";
        }
        $inline_css .= "}\n";
        if (!empty($custom_css)) {
            $inline_css .= "\n/* Custom CSS */\n" . $custom_css . "\n";
        }
        // Add inline styles to the correct handle
        if ($preset === 'auto') {
            wp_add_inline_style('flosc-theme', $inline_css);
        } else {
            wp_add_inline_style('flosc-preset', $inline_css);
        }
    }
