...
// Match quiz.php's default: flosc_sample_text_based_quiz is the default active quiz
...
if ( $quiz_configured && class_exists( 'FLOSC_Quiz_Type_Factory' ) ) {
    $names = [];
    foreach ( $enabled_quizzes as $qid ) {
        $qt      = FLOSC_Quiz_Type_Factory::get_quiz_type( $qid );
        $names[] = $qt ? $qt->get_name() : ucwords( str_replace( '_', ' ', $qid ) );
    }
    $quiz_label = implode( ', ', $names );
} else {
    $quiz_label = 'None configured';
}

// Pill counts = admin-panel pills + IVR-defined pills
$admin_visitor_pills = count( $flow_settings['autoprompts']['visitor'] ?? [] );
$visitor_pills       = $admin_visitor_pills + $ivr_pill_counts['freeline'];
