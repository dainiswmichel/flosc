...
class floscApp {
    ...
    /**
     * v1.1.0: Infinite carousel using transform (no clones)
     * - Uses CSS transform for positioning
     * - Index-based with modular arithmetic for infinite loop
     * - Arrow buttons and swipe supported
     */
    initCarouselOverflow(container) {
        ...
        const getVisibleCount = () => {
            ...
            return Math.floor(track.parentElement.clientWidth / itemWidth) || 1;
        };

        // Check if we need scrolling (more items than visible)
        const checkOverflow = () => {
        ...
        // Navigate to next item (infinite)
        const goNext = () => {
            ...
            // If we've gone past the last item, loop to start
            if (currentIndex >= items.length) {
                updatePosition(true);
                // After animation, jump to start instantly
                setTimeout(() => {
                    currentIndex = 0;
        ...
        // Navigate to previous item (infinite)
        const goPrev = () => {
            if (isAnimating) return;
            isAnimating = true;
            ...
            // If we've gone before first item, loop to end
            if (currentIndex < 0) {
                // Jump to end+1 instantly, then animate to end
        ...
        // Setup: disable native scroll, enable transform
