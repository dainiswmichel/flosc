 ...
 * 
 * v9.4.4: Fixed flush permalinks - now uses admin-post.php to avoid nested form issue
 */

if (!defined('ABSPATH')) exit;

// Check for success message from redirect
$flush_success = isset($_GET['flosc_flushed']) && $_GET['flosc_flushed'] === '1';
...
// Show success message if just flushed
if ($flush_success): ?>
    <div class="notice notice-success is-dismissible">
        <p>✓ Permalinks flushed successfully. Your app should now be accessible.</p>
    </div>
<?php endif; ?>
...
<div style="background: <?php echo $rules_exist ? '#d1fae5' : '#fef3c7'; ?>; border: 1px solid <?php echo $rules_exist ? '#10b981' : '#f59e0b'; ?>; border-radius: 6px; padding: 16px 20px; margin-bottom: 24px;">
    ...
    <?php if (!$rules_exist): ?>
        <?php 
        // v9.4.4: Use link with nonce instead of nested form
        $flush_url = wp_nonce_url(
            admin_url('admin-post.php?action=flosc_flush_permalinks'),
            'flosc_flush_permalinks',
            '_wpnonce'
        );
        ?>
