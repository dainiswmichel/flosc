# FLOSC v9.4.4 - Flush Permalinks Fix
...
### 🐛 Flush Permalinks Fix
- **Issue**: "Link expired" error when clicking Flush Permalinks button
- **Root cause**: Nested HTML forms are invalid - browser ignored inner form and posted to outer form's `options.php`
- **Fix**: Changed from `<form>` to `<a href>` with nonce URL, using WordPress `admin-post.php` handler
- **New handler**: `handle_admin_flush_permalinks()` method added to main plugin class

### ✅ Includes v9.4.3 CSS Fix
- Modal CSS restored (Quiz, Share, Payment, Login Gate overlays)

### ✅ Includes v9.4.2 Security Hardening
- HMAC-signed cookies for quiz scores
