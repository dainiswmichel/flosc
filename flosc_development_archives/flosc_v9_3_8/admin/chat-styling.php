<?php
/**
 * FLOSC Chat Styling Tab (v9.3.8)
 * 
 * Clean architecture with:
 * - Theme preset (Auto/Day/Night)
...
if (isset($_POST['flosc_reset_chat_style']) && check_admin_referer('flosc_reset_chat_style_nonce')) {
    ...
    delete_option('flosc_chat_style_bubble');
    delete_option('flosc_chat_style_accent');
...
// Get current values
$current_preset = get_option('flosc_chat_style_preset', 'day');
$current_bubble = get_option('flosc_chat_style_bubble', 'subtle-notch');
$current_accent = get_option('flosc_chat_style_accent', '#2563eb');
...
<table class="form-table">
    ...
    <!-- Theme Preset -->
    <tr>
        <th scope="row"><label for="flosc_chat_style_preset">Theme</label></th>
        <td>
            <select name="flosc_chat_style_preset" id="flosc_chat_style_preset" class="regular-text">
                ...
                <option value="day" <?php selected($current_preset, 'day'); ?>>☀️ Day (Light)</option>
                <option value="night" <?php selected($current_preset, 'night'); ?>>🌙 Night (Dark)</option>
            </select>
            <p class="description"><strong>Auto</strong> follows user's system preference. Day = light background. Night = dark background.</p>
        </td>
    </tr>

    <!-- Bubble Style -->
    <tr>
        <th scope="row"><label for="flosc_chat_style_bubble">Bubble Style</label></th>
        <td>
            <select name="flosc_chat_style_bubble" id="flosc_chat_style_bubble" class="regular-text">
            ...
            <div style="margin-top: 16px; padding: 16px; background: #f9fafb; border-radius: 8px; max-width: 400px;">
                <div style="display: flex; gap: 12px; margin-bottom: 12px;">
                    <div style="width: 24px; height: 24px; background: <?php echo esc_attr($current_accent); ?>; border-radius: 50%;"></div>
                    <div id="bubble-preview-user" style="background: <?php echo esc_attr($current_accent); ?>; color: white; padding: 8px 14px; border-radius: <?php echo esc_attr($bubble_styles[$current_bubble]['user']); ?>; font-size: 14px;">User message</div>
                </div>
                <div style="display: flex; gap: 12px;">
                    <div style="width: 24px; height: 24px; background: #e5e7eb; border-radius: 50%;"></div>
                    <div id="bubble-preview-assistant" style="background: transparent; border: 1px solid rgba(0,0,0,0.1); padding: 8px 14px; border-radius: <?php echo esc_attr($bubble_styles[$current_bubble]['assistant']); ?>; font-size: 14px;">Assistant message</div>
                </div>
            </div>
        </td>
    </tr>
    ...
    <!-- Accent Color -->
    <tr>
        <th scope="row"><label for="flosc_chat_style_accent">Accent Color</label></th>
        <td>
            <input type="color" name="flosc_chat_style_accent" id="flosc_chat_style_accent" value="<?php echo esc_attr($current_accent); ?>" style="width: 60px; height: 36px; padding: 0; border: 1px solid #ccc; border-radius: 4px; cursor: pointer;">
            <input type="text" id="flosc_chat_style_accent_hex" value="<?php echo esc_attr($current_accent); ?>" style="width: 100px; margin-left: 8px;" readonly>
    ...
    <!-- Font Family -->
    <tr>
        <th scope="row"><label for="flosc_chat_style_font">Font</label></th>
        <td>
    ...
    <!-- Text Scaling -->
    <tr>
        <th scope="row"><label for="flosc_chat_style_scale">Text Size</label></th>
        <td>
            <input type="range" name="flosc_chat_style_scale" id="flosc_chat_style_scale" 
    ...
    <tr>
        ...
        <td>
            <textarea name="flosc_chat_style_custom_css" id="flosc_chat_style_custom_css" 
...
<div style="background: #f0f6ff; border: 1px solid #c3dafe; border-radius: 8px; padding: 20px; margin: 30px 0; max-width: 700px;">
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; font-family: monospace; font-size: 13px;">
        <div>
            ...
            <code>--flosc-user-bg</code><br>
            <code>--flosc-user-text</code><br>
            <code>--flosc-user-message-radius</code><br>
            <code>--flosc-assistant-bg</code><br>
            <code>--flosc-assistant-text</code>
        </div>
        <div>
            <strong style="color: #1e40af;">Global</strong><br>
            <code>--flosc-bg</code><br>
            <code>--flosc-text-primary</code><br>
            <code>--flosc-accent</code><br>
            <code>--flosc-border-color</code>
        </div>
    </div>
</div>
...
jQuery(document).ready(function($) {
    ...
    function updateBubblePreview() {
        ...
        // Update the avatar color too
        $('#bubble-preview-user').prev().css('background', accent);
    }
