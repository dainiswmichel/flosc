...
class FLOSC_Framework {
    ...
    public function capture_external_quiz_score($data) {
        ...
        if (!$user_id) {
            // Visitor: store in cookie for login gate to pick up
            setcookie(
...
The {product_name} Team";
    ...
    public function register_settings() {
        ...
        register_setting('flosc_settings', 'flosc_chat_style_bubble');
        register_setting('flosc_settings', 'flosc_chat_style_accent');
    ...
    /**
     * Enqueue chat styling (v9.3.8)
     * 
     * Supports: auto (system preference), day, night presets
     * Plus: bubble style, accent color, font, scale, custom CSS
     */
    private function enqueue_chat_style() {
        // Get settings
        $preset = get_option('flosc_chat_style_preset', 'day');
        ...
        $inline_css = "";
        $style_handle = 'flosc-app'; // Default handle for inline styles

        // Handle preset loading
        if ($preset === 'auto') {
            // Load day CSS as base, then add dark mode media query
            $day_path = FLOSC_PLUGIN_DIR . 'assets/css/chat-style-day.css';
            $night_path = FLOSC_PLUGIN_DIR . 'assets/css/chat-style-night.css';
            
            if (file_exists($day_path)) {
                wp_enqueue_style('flosc-chat-style', FLOSC_PLUGIN_URL . 'assets/css/chat-style-day.css', ['flosc-app'], filemtime($day_path));
                $style_handle = 'flosc-chat-style';
            }
            
            // Add dark mode override via inline CSS
            if (file_exists($night_path)) {
                $night_css = @file_get_contents($night_path);
                if ($night_css) {
                    // Extract just the variable definitions from the file
                    if (preg_match('/:root\s*\{([^}]+)\}/s', $night_css, $matches)) {
                        $dark_vars = $matches[1];
                        $inline_css .= "@media (prefers-color-scheme: dark) {\n:root {\n{$dark_vars}\n}\n}\n";
                    }
                }
            }
        } else {
            ...
            if (file_exists($preset_path)) {
                wp_enqueue_style('flosc-chat-style', FLOSC_PLUGIN_URL . 'assets/css/' . $preset_file, ['flosc-app'], filemtime($preset_path));
                $style_handle = 'flosc-chat-style';
            }
        }
        ...
        $inline_css .= ":root {\n";
        $inline_css .= "    --flosc-user-message-radius: {$bubble_config['user']};\n";
        $inline_css .= "    --flosc-assistant-message-radius: {$bubble_config['assistant']};\n";
        
        if (!empty($accent) && $accent !== '#2563eb') {
            $inline_css .= "    --flosc-accent: {$accent};\n";
            $inline_css .= "    --flosc-user-bg: {$accent};\n";
            $inline_css .= "    --flosc-user-message-bg: {$accent};\n";
        }
        
        if ($scale != 100) {
            $scale_factor = $scale / 100;
            $inline_css .= "    --flosc-scale: {$scale_factor};\n";
        }
        
        if ($font !== 'system' && !empty($font_families[$font])) {
            $inline_css .= "    --flosc-font-family: {$font_families[$font]};\n";
        }
        
        $inline_css .= "}\n";

        // Apply font family to messages if set
        if ($font !== 'system' && !empty($font_families[$font])) {
            $inline_css .= ".flosc-app .messages, .flosc-app .message-text {\n";
...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7: [link]. It has the complete IPA guide and video walkthrough. Ready to dive in?\"",
    ...
    public function get_ivr_messages($request) {
        ...
        // Initialize condition evaluator once (v9.2.6: Performance fix)
        require_once FLOSC_PLUGIN_DIR . 'includes/class-condition-evaluator.php';
