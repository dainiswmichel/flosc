# FLOSC v1.2.3 - Multi-Flow System (Stable)
...
### 🔧 Bug Fixes from v1.2.2

| Issue | Fix |
|-------|-----|
| `handle_custom_domain()` only checked legacy global setting | Now iterates ALL flows to find matching custom domain |
| Orphaned docblock comments | Cleaned up duplicate documentation |
| `delete_flow()` SQL broke serialized user meta | Now properly uses `revoke_flow_access()` for each user |
| `get_flow_users()` fragile serialized search | Now deserializes and checks arrays properly |
| `ivr_file` sanitization stripped directory separators | Now allows subdirectory paths like `lesaep/ivr.md` |
| IVR save could write to wrong file in admin | Added optional `$target_flow` parameter |

### ✅ Inherited from v1.2.2

### 🔀 FLOSC Flows - Multiple Independent Chatbots

Run multiple independent chatbots from a single WordPress installation. Each flow can have:

| Feature | Description |
|---------|-------------|
| **Unique Identity** | Name, emoji, tagline, colors |
| **Own URL** | Either a slug (`/myapp`) or custom domain (`myapp.com`) |
| **Own IVR Messages** | Different conversation flows per app |
| **Own Content** | WordPress category for lessons |
| **Own Team** | Assign Editors/Authors to specific flows |

### 📁 New Files

- `includes/class-flow-manager.php` - CRUD operations for flows
- `admin/flows.php` - Flows list admin page
- `admin/flow-edit.php` - Flow create/edit form with tabs

### 🔧 Core Changes

| Function | Change |
|----------|--------|
| `get_current_flow()` | **NEW** - Returns current flow based on request |
