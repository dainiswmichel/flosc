        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.nonce) {
                    ...
                    console.log('[FLOSC] Nonce refreshed');
                    return true;
                }
            }
        } catch (e) {
            console.warn('[FLOSC] Could not refresh nonce:', e);
        }
        return false;
    }
        ...
        if (this.state !== 'visitor' && !this.currentSession) {
            try {
                ...
                if (sessionData.success && sessionData.session) {
                    ...
                    console.log('[FLOSC] Auto-created session:', this.currentSession.id, this.currentSession.title);
                    // Refresh sidebar session list
                    this.loadSessions();
                }
            } catch (e) {
                console.warn('[FLOSC] Could not auto-create session:', e);
            }
        }
    ...
    async restoreLastSession() {
        try {
            ...
            if (latestSession && latestSession.messages && latestSession.messages.length > 0) {
                console.log('[FLOSC] Restoring last session:', latestSession.id, latestSession.title);
                await this.loadSession(latestSession.id);
            }
        } catch (e) {
            console.warn('[FLOSC] Could not restore last session:', e);
        }
    }
            ...
            paypal.Buttons({
                ...
                createOrder: async () => {
                    ...
                    try {
                        ...
                        console.error('[FLOSC-CHECKOUT] PayPal create order error:', err);
                ...
                onApprove: async (data, actions) => {
                    try {
                        ...
                        // v1.7.5: Handle INSTRUMENT_DECLINED (per PayPal official sample)
                        const errorDetail = result?.data?.details?.[0] || result?.details?.[0];
                        ...
                        console.error('[FLOSC-CHECKOUT] PayPal capture error:', err);
                ...
                onError: (err) => {
                    console.error('[FLOSC-CHECKOUT] PayPal error:', err);
                ...
                onCancel: () => {
                    console.log('[FLOSC-CHECKOUT] PayPal cancelled by user');
