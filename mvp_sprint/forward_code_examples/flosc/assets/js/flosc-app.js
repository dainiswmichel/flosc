...
class floscApp {
    ...
    async init() {
        ...
        try {
            ...
            const magicStatus = url.searchParams.get('flosc_magic_status');
            const remainingComplimentary = this.config.complimentary_logins_remaining;
            if (magicStatus === 'success' && remainingComplimentary !== null && remainingComplimentary !== undefined) {
                const template = this.config.magicLinkComplimentaryTemplate || 'You have {n} more complimentary logins.';
                const loginMessage = template
                    .replace('{n}', remainingComplimentary)
                    .replace('{email}', this.user?.email || '')
                    .replace('{link_name}', this.config.magicLinkName || 'Complimentary LeSAEp Learners Guest Access Link')
                    .replace('{upgrade_url}', this.config.magicLinkUpgradeUrl || '#');
                this.addMessage('assistant', loginMessage, true);
            } else if (magicStatus === 'invalid' || magicStatus === 'create_failed') {
                this.addMessage('assistant', this.config.magicLinkExpiredMessage || "That login link is no longer valid. Enter your email again and I'll send you a fresh one.", false);
            } else if (magicStatus === 'limit_reached') {
                this.addMessage('assistant', this.config.magicLinkLimitReachedMessage || 'Your complimentary logins have been used up. Choose a plan to continue.', false);
            }
            if (magicStatus) {
                url.searchParams.delete('flosc_magic_status');
                window.history.replaceState({}, '', url.toString());
            }
            if (this.config.needsProfileCompletion && !sessionStorage.getItem('flosc_nickname_prompt_dismissed')) {
    ...
    async processEmailAuth(email) {
        ...
        const modal = document.getElementById('flosc-auth-modal');
        const configKey = modal?.dataset?.configKey || 'authModal';
        const loadingText = this.config[configKey + 'LoadingText'] || 'Sending login link...';
        const buttonText = this.config[configKey + 'ButtonText'] || 'Continue with Email';
        const checkEmailMessage = (this.config.magicLinkCheckEmailMessage || "Check your email for a login link. Once you click it, you'll come right back here.")
            .replace('{link_name}', this.config.magicLinkName || 'Complimentary LeSAEp Learners Guest Access Link');
        ...
        try {
            ...
            if (result.success) {
                this.hideAuthModal();
                this.addMessage('assistant', result.message || checkEmailMessage, false);
            } else {
    ...
    showGuestProfileCard() {
        if (document.getElementById('flosc-guest-profile-card')) return;

        const cardHtml = `
            <div class="flosc-guest-profile-card" id="flosc-guest-profile-card" style="background:var(--flosc-surface,#fff);border:1px solid var(--flosc-border,#e5e7eb);border-radius:14px;padding:16px;margin-top:12px;">
                <div style="font-weight:700;font-size:16px;margin-bottom:8px;">Complete your profile</div>
                <div style="font-size:14px;color:var(--flosc-text-secondary,#666);margin-bottom:12px;">Choose a name or nickname and create a password for future logins.</div>
                <div style="display:flex;flex-direction:column;gap:10px;">
                    <input type="text" class="flosc-profile-name" placeholder="Your name or nickname" style="width:100%;padding:12px 14px;border:1px solid var(--flosc-border,#ddd);border-radius:8px;box-sizing:border-box;">
                    <input type="password" class="flosc-profile-password" placeholder="Choose a password (min 6 characters)" style="width:100%;padding:12px 14px;border:1px solid var(--flosc-border,#ddd);border-radius:8px;box-sizing:border-box;">
                    <div class="flosc-profile-error" style="display:none;color:#b91c1c;font-size:13px;"></div>
                    <div style="display:flex;gap:10px;flex-wrap:wrap;">
                        <button type="button" class="flosc-profile-save" style="padding:10px 16px;border:none;border-radius:8px;background:var(--flosc-primary,#4f46e5);color:#fff;cursor:pointer;">Save</button>
                        <button type="button" class="flosc-profile-skip" style="padding:10px 16px;border:1px solid var(--flosc-border,#ddd);border-radius:8px;background:transparent;cursor:pointer;">Not right now</button>
                    </div>
                </div>
            </div>
        `;
        ...
        const card = document.getElementById('flosc-guest-profile-card');
        if (!card) return;
        ...
        saveBtn?.addEventListener('click', async (e) => {
            ...
            if (!name) {
                if (errEl) {
                    errEl.textContent = 'Please enter your name or nickname.';
            ...
            try {
                ...
                sessionStorage.removeItem('flosc_nickname_prompt_dismissed');
                this.config.needsProfileCompletion = false;
                if (this.user) {
                    this.user.name = result.display_name || name;
                }
                card.closest('.message')?.remove();
                const confirm = (this.config.magicLinkProfileConfirmMessage || 'Perfect, {name}! You can always log in directly at {login_url}, update your profile, and upgrade to full access.')
                    .replace('{name}', result.display_name || name)
                    .replace('{login_url}', this.config.loginUrl || window.location.origin);
                this.addMessage('assistant', confirm, true);
            } catch (error) {
                if (errEl) {
                    errEl.textContent = error.message || 'Could not save profile.';
                    errEl.style.display = 'block';
                }
        ...
        skipBtn?.addEventListener('click', (e) => {
            e.preventDefault();
            sessionStorage.setItem('flosc_nickname_prompt_dismissed', '1');
            card.closest('.message')?.remove();
        });
    }
