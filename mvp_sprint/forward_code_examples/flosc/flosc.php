...
The {product_name} Team";
    ...
    public function handle_login_token() {
        // Case 0: Email magic-link login
        if (!empty($_GET['flosc_magic'])) {
            $token = sanitize_text_field(wp_unslash($_GET['flosc_magic']));
            $transient_key = 'flosc_magic_link_' . $token;
            $payload = get_transient($transient_key);

            if (!$payload || empty($payload['email'])) {
                $clean_url = add_query_arg('flosc_magic_status', 'invalid', remove_query_arg('flosc_magic'));
            ...
            if ($existing_user) {
                ...
                if (is_wp_error($user_id)) {
                    $clean_url = add_query_arg('flosc_magic_status', 'create_failed', remove_query_arg('flosc_magic'));
                    wp_redirect($clean_url);
                    exit;
                }
                ...
                update_user_meta($user_id, '_flosc_complimentary_logins_remaining', 10);
                update_user_meta($user_id, '_flosc_profile_completed', false);
                do_action('flosc_user_registered', $user_id, 'email');
                error_log("FLOSC v10.0.0: New user created via magic link: {$email} (User ID: {$user_id})");
            }

            $remaining_before_login = $this->get_complimentary_logins_remaining($user_id);
            if ($remaining_before_login !== null && $remaining_before_login <= 0) {
                $clean_url = add_query_arg('flosc_magic_status', 'limit_reached', remove_query_arg('flosc_magic'));
                wp_redirect($clean_url);
                exit;
            }
            $remaining_after_login = $this->consume_complimentary_login($user_id);
            ...
            if ($remaining_after_login !== null) {
                set_transient('flosc_just_magic_login_' . $user_id, (int) $remaining_after_login, MINUTE_IN_SECONDS * 10);
            }

            $clean_url = add_query_arg('flosc_magic_status', 'success', remove_query_arg('flosc_magic'));
            wp_redirect($clean_url);
            exit;
        }
    ...
    private function render_flosc_app() {
        ...
        if (is_user_logged_in()) {
            ...
            $user_data = [
                ...
                'complimentaryLoginsRemaining' => (function() use ($user) {
                    $remaining = get_transient('flosc_just_magic_login_' . $user->ID);
                    if ($remaining !== false) {
                        delete_transient('flosc_just_magic_login_' . $user->ID);
                        return (int) $remaining;
                    }
                    $stored = get_user_meta($user->ID, '_flosc_complimentary_logins_remaining', true);
                    return ($stored === '' || $stored === null) ? null : (int) $stored;
                })(),
    ...
    public function register_rest_routes() {
        ...
        // v10.0.0: Explicit magic-link sender endpoint (same behavior as register-email).
        register_rest_route('flosc/v1', '/send-magic-link', [
            'methods' => 'POST',
            'callback' => [$this, 'handle_send_magic_link'],
            'permission_callback' => [$this, 'check_public_endpoint_permission'],
        ]);
...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7. Ready to dive in?\"",
    ...
    public function handle_purchase($request) {
        ...
        if ($method === 'free' || empty($provider_id)) {
            ...
            if ($price > 0 && $method === 'free') {
                return new WP_Error('not_free', 'This offer requires payment', ['status' => 400]);
            }
            // Grant access
     ...
     *
     * v10.0.0: Email entry no longer creates/logs in a user immediately.
     * We now send a one-time magic link and create/login the user only when
     * that link is redeemed.
     */
    public function handle_email_registration($request) {
        return $this->handle_send_magic_link($request);
    }

    /**
     * v10.0.0: Send a single-use magic link for email authentication.
     * Stores pending quiz/session payload in a 24-hour transient so it can
     * be attached to the user only after the link is redeemed.
     */
    public function handle_send_magic_link($request) {
        ...
        $sent = $this->send_magic_link_email($email, $token);
        if (!$sent) {
            delete_transient($transient_key);
            return new WP_REST_Response([
                'success' => false,
    ...
    /**
     * v10.0.0: Send the actual magic-link email.
     */
    private function send_magic_link_email($email, $token) {
        $magic_url = add_query_arg('flosc_magic', rawurlencode($token), $this->get_app_url());
        $product_name = get_option('flosc_product_name', 'LeSAEp');
        $subject = sprintf('Your %s Login Link', $product_name ?: 'LeSAEp');

        $safe_product = esc_html($product_name ?: 'LeSAEp');
        ...
        $body = '<!doctype html><html><body style="margin:0;padding:0;background:#f6f7fb;font-family:Arial,sans-serif;color:#1f2937;">'
            ...
            . '<h1 style="margin:0 0 16px 0;font-size:28px;line-height:1.2;">Your login link is ready</h1>'
            . '<p style="margin:0 0 16px 0;font-size:16px;line-height:1.6;">Use the button below to log in to ' . $safe_product . '.</p>'
            . '<p style="margin:0 0 24px 0;font-size:16px;line-height:1.6;">This link is valid for 24 hours and can be used once.</p>'
            . '<p style="margin:0 0 24px 0;"><a href="' . $safe_url . '" style="display:inline-block;background:#2563eb;color:#ffffff;text-decoration:none;padding:14px 22px;border-radius:10px;font-weight:700;">Log In to ' . $safe_product . '</a></p>'
    ...
    /**
     * v10.0.0: Get remaining complimentary magic-link logins for an email user.
     * Returns null for users who should not be limited (purchased users, SSO users, etc.).
     */
    private function get_complimentary_logins_remaining($user_id) {
        $user_id = (int) $user_id;
        if ($user_id <= 0) return null;

        if ((bool) get_user_meta($user_id, '_flosc_purchased', true)) {
            return null;
        }

        $registration_method = (string) get_user_meta($user_id, '_flosc_registration_method', true);
        $is_email_user = ($registration_method === 'email' || $registration_method === '');
        if (!$is_email_user) {
            return null;
        }

        $remaining = get_user_meta($user_id, '_flosc_complimentary_logins_remaining', true);
        if ($remaining === '' || $remaining === null) {
            $remaining = 10;
            update_user_meta($user_id, '_flosc_complimentary_logins_remaining', $remaining);
        }
    ...
    /**
     * v10.0.0: Consume one complimentary magic-link login.
     * Returns remaining balance after the current login is counted.
     */
    private function consume_complimentary_login($user_id) {
        $remaining = $this->get_complimentary_logins_remaining($user_id);
    ...
    /**
     * v10.0.1: Save guest nickname + required password after magic-link login.
     */
    ...
    public function paypal_create_order($request) {
        ...
        if (FLOSC_DEBUG) {
            error_log('[FLOSC-PAYPAL] === create_order START === offer=' . $offer_id . ', flow=' . ($flow_id ?: 'none') . ', user=' . get_current_user_id());
        }

        $offer = $this->sale_manager->offers()->get_offer($offer_id, $flow_id ?: null);

        if (!$offer) {
            if (FLOSC_DEBUG) error_log('[FLOSC-PAYPAL] create_order FAIL: offer "' . $offer_id . '" not found');
        ...
        if (!$paypal || !$paypal->is_configured()) {
            ...
            if (FLOSC_DEBUG) error_log('[FLOSC-PAYPAL] create_order FAIL: not configured (client_id: ' . $has_id . ')');
        ...
        if ($amount <= 0) {
            if (FLOSC_DEBUG) error_log('[FLOSC-PAYPAL] create_order FAIL: no price for offer "' . $offer_id . '" — pricing: ' . wp_json_encode($offer['pricing'] ?? 'NONE') . ', price: ' . ($offer['price'] ?? 'NONE'));
        ...
        if (FLOSC_DEBUG) {
            error_log('[FLOSC-PAYPAL] create_order calling API: amount=' . $amount . ', currency=' . $currency . ', user_id=' . $user->ID);
        }
        ...
        if (FLOSC_DEBUG) {
            error_log('[FLOSC-PAYPAL] === create_order SUCCESS === order_id=' . ($result['order_id'] ?? 'NONE'));
        }
    ...
    public function paypal_capture_order($request) {
        ...
        if (FLOSC_DEBUG) {
            error_log('[FLOSC-PAYPAL] === capture_order START === order=' . $order_id . ', offer=' . $offer_id . ', flow=' . ($flow_id ?: 'none') . ', user=' . get_current_user_id());
        }
        ...
        $offer = $this->sale_manager->offers()->get_offer($offer_id, $flow_id ?: null);
        if (!$offer) {
            if (FLOSC_DEBUG) error_log('[FLOSC-PAYPAL] capture_order FAIL: offer not found');
            return new WP_Error('invalid_offer', 'Offer not found', ['status' => 404]);
        }
        ...
        $paypal = $this->sale_manager->get_provider('paypal');
        if (!$paypal || !$paypal->is_configured()) {
            if (FLOSC_DEBUG) error_log('[FLOSC-PAYPAL] capture_order FAIL: PayPal not configured');
            return new WP_Error('paypal_not_configured', 'PayPal is not configured', ['status' => 500]);
        }
        ...
        if (isset($capture_result['success']) && $capture_result['success'] === false) {
            if (FLOSC_DEBUG) error_log('[FLOSC-PAYPAL] capture_order: PayPal error forwarded — ' . ($capture_result['issue'] ?? $capture_result['message'] ?? 'unknown'));
        ...
        if ($user_id > 0 && $captured_user_id && intval($captured_user_id) > 0 && intval($captured_user_id) !== $user_id) {
            if (FLOSC_DEBUG) error_log('[FLOSC-PAYPAL] capture_order FAIL: user mismatch (captured=' . $captured_user_id . ', current=' . $user_id . ')');
            return new WP_Error('user_mismatch', 'Payment does not belong to this user', ['status' => 403]);
        }
        ...
        if (FLOSC_DEBUG) {
            error_log('[FLOSC-PAYPAL] capture_order: granting access for user=' . $user_id . ', txn=' . $capture_result['transaction_id']);
        }
        ...
        if (FLOSC_DEBUG) {
            error_log('[FLOSC-PAYPAL] === capture_order SUCCESS === txn=' . $capture_result['transaction_id'] . ', user=' . $user_id);
        }
