...
<body class="flosc-app<?php echo is_admin_bar_showing() ? ' admin-bar' : ''; ?>"
    ...
    <script>
        ...
        window.FLOSC_CONFIG = <?php 
            ...
            'magicLinkCheckEmailMessage' => wp_unslash(flosc_get_setting('guest_link_check_email_message', "We've sent you a {link_name} to your email — click it to access this chat as a guest and view your quiz score, free lessons, and a special upgrade offer.")),
            'magicLinkComplimentaryTemplate' => wp_unslash(flosc_get_setting('guest_link_welcome_message', 'Hi, welcome back! Just to confirm: your email address is {email} and you can use your {link_name} {n} more times to access this chat. <a href="{upgrade_url}">Upgrade</a> for complete access to all lessons, quiz audios, and our LeSAEp Learners network...')),
            'magicLinkName' => wp_unslash(flosc_get_setting('guest_link_name', 'Complimentary LeSAEp Learners Guest Access Link')),
            'magicLinkUpgradeUrl' => flosc_get_setting('guest_link_upgrade_url', ''),
            'magicLinkProfileConfirmMessage' => wp_unslash(flosc_get_setting('guest_link_profile_confirm_message', 'Perfect, {name}! You can always log in directly at {login_url}, update your profile, and upgrade to full access.')),
            'magicLinkExpiredMessage' => wp_unslash(flosc_get_setting('magic_link_expired_message', "That login link is no longer valid. Enter your email again and I'll send you a fresh one.")),
            'magicLinkLimitReachedMessage' => wp_unslash(flosc_get_setting('magic_link_limit_reached_message', 'Your complimentary logins have been used up. Choose a plan to continue.')),
            ...
            'complimentary_logins_remaining' => (function() {
                if (!is_user_logged_in()) return null;
                $uid = get_current_user_id();
                $remaining = get_transient('flosc_just_magic_login_' . $uid);
                if ($remaining !== false) {
                    return (int) $remaining;
                }
                $stored = get_user_meta($uid, '_flosc_complimentary_logins_remaining', true);
                return ($stored === '' || $stored === null) ? null : (int) $stored;
            })(),
            'needsProfileCompletion' => (function() {
                if (!is_user_logged_in()) return false;
                $uid = get_current_user_id();
                if (get_user_meta($uid, '_flosc_profile_completed', true)) {
                    return false;
                }
                $user = get_userdata($uid);
                if (!$user || !in_array('lesaep_learners', (array) $user->roles, true)) {
                    return false;
                }
                $registration_method = (string) get_user_meta($uid, '_flosc_registration_method', true);
                return ($registration_method === 'email' || $registration_method === '');
            })(),
