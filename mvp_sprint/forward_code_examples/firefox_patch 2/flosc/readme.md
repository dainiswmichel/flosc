...
**Freeline-Login-Offer-Sale-Content** - A WordPress framework for quiz-based learning and conversational sales flow.

**Version:** 5.0.7
**Status:** PayPal sandbox payments FIXED — definitive v5.0.7
...
## Architecture Rule: IVR + AI

**IVR = brain (WHAT). AI = voice (HOW). When AI is configured, IVR never speaks directly.**

- IVR messages define WHAT to communicate (content, actions, conditions, phase logic)
- AI decides HOW to say it (tone, phrasing, conversational flow)
- When AI provider is set to anything other than `ivr`, all IVR matches route through AI
- IVR only speaks directly when admin explicitly sets provider to `ivr` (no-AI mode)
- If AI fails, raw IVR content is the fallback — never silence

---

## 🆕 What's New in v1.9.3

### IVR Leak Prevention (3 layers)
- **Frontend pipeline** — `sendMessage()` checks `aiProvider` config. IVR matches route through `callAPI()` with guidance payload when AI is active.
- **Backend acceptance** — `handle_chat()` reads `ivr_guidance` from request, passes to Chatpack for AI rewriting.
- **Dispatch null returns** — All provider error paths return `null` instead of falling back to `ivr_response()`. Only explicit `case 'ivr'` calls IVR directly.

### Chatpack Revert
- Reverted 4 fabricated style/brevity rules from v1.9.2 chatpack.
...
### 🔒 Stripe Disabled
- **Stripe temporarily disabled** — Stripe account verification pending. All Stripe JS SDK loading and config key suppressed. PayPal sandbox is the sole payment method.

### 🐛 Bug Fixes
- **Gate button invisible** — "Sign Up to See Results" button now bright blue (#2563eb) with white text and glow shadow, overriding any theme CSS variable interference.
- **Markdown not rendering** — Assistant messages now render **bold**, *italic*, `code`, and [links] instead of showing raw asterisks. New `formatMarkdown()` method.
- **Cookie check retry** — If sandbox payment gets "Cookie check failed", fetches fresh nonce from `/nonce` endpoint and retries automatically.
- **Offer shows raw ID** — Offer cards now show "Full Access" instead of raw `full_access_001`. Humanizes IDs by removing underscores and trailing numbers.
- **Nonce refresh endpoint** — New REST route `GET /flosc/v1/nonce` returns fresh `wp_rest` nonce for stale-cookie recovery.

### 💰 Payment
- **PayPal-only mode** — "Get Access Now" button goes straight to PayPal sandbox. No more "Payment temporarily unavailable" from missing Stripe.
- **Sandbox fallback** — If neither PayPal nor Stripe is available at runtime, falls back to sandbox purchase flow.
...
### 🐛 Bug Fixes
- **Cookie check failed** — Added `credentials: 'same-origin'` to sandbox-purchase, PayPal create-order, and PayPal capture-order fetch calls. Without it, WordPress auth cookies weren't sent, causing nonce validation failure.
- **Carousel pills fly off screen** — Added `min-width: 0` to `.flosc-carousel-track` so the flex item can shrink below content width. Changed `overflow: visible` → `overflow: hidden` to clip pills during translateX carousel animation.
- **Profile card clipped below viewport** — Added `.admin-bar .flosc-app { height: calc(100vh - 32px) }` to account for WordPress admin bar height (32px desktop, 46px mobile).
- **Missing CSS selector** — Restored `.flosc-login-gate-body` selector that was lost during a previous edit.

### 🎨 UI Improvements
- **Full-width rich cards** — Quiz results, sandbox payment, and quiz gate cards now expand to full chat width using `:has()` selector instead of being constrained by the 75% message bubble max-width.
- **Collapsible sidebar** — Desktop sidebar can be collapsed/expanded via hamburger menu. Uses `.collapsed { width: 0 }` transition.
- **Header visibility** — Header with hamburger menu button now always visible on desktop (was `display: none`).
- **Quiz gate styling** — "Sign Up to See Results" card fully styled with icon, title, text, and CTA button.

### 💰 Payment (from v1.6.9)
- PayPal Orders API v2 sandbox integration
- PayPal provider class, REST routes, JS SDK, payment modal

---

## Previous: What's New in v1.2.6
...
| `get_floscflow_identity()` | Reads identity config (name, chatlogo, favicon, color, pricing) from current flow |
