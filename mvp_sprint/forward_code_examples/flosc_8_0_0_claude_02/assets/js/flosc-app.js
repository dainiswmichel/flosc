...
class floscApp {
    ...
    async init() {
        ...
        try {
            ...
            // v8.0.0-claude-01 Bug 2 fix: Profile card trigger is now independent of
            // guestLinkRemaining. Previously nested inside that block, meaning both
            // transients had to survive the redirect for the card to appear — if either
            // expired (race condition, OPcache, slow redirect), the card never showed.
            // needsProfileCompletion is a persistent user-meta gate (set in flosc-app.php)
            // that remains true until handle_update_guest_profile() marks it complete.
            // sessionStorage prevents re-prompting within the same browser session (Skip).
    ...
    showGuestProfileCard() {
        ...
        skipBtn?.addEventListener('click', (e) => {
            e.preventDefault();
            // v8.0.0-claude-01 Bug 2 fix: Mark dismissed for this session only.
            // The server-side needsProfileCompletion flag remains true, so the
            // card reappears on next login until the user completes their profile.
        ...
        saveBtn?.addEventListener('click', async () => {
            ...
            // v8.0.0-claude-02: Clear any previous error before re-validating so stale
            // messages don't linger if the user corrects a field and hits Save again.
            if (errEl) { errEl.style.display = 'none'; errEl.textContent = ''; }

            // Both name and password are required — no partial saves.
            // Password is required so the user can log in directly on future visits
            // without needing a new magic link. The server also enforces this.
            if (!name) {
                if (errEl) { errEl.textContent = 'Please enter your name or nickname.'; errEl.style.display = 'block'; }
                nameEl?.focus();
                return;
            }
            ...
            try {
                // Always send both fields — server requires both and saves atomically.
                // Removed the old `if (password) body.password = password` conditional.
                const body = { display_name: name, password: password };
