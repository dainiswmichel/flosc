...
class FLOSC_Framework {
    ...
    private function init_hooks() {
        ...
        add_action('init', function() {
            ...
            if (!get_role('guest_lesaep_learner')) {
    ...
    public function handle_login_token() {
        ...
        if (!empty($_GET['flosc_magic'])) {
            ...
            if ($existing_user) {
                ...
                if (!in_array('lesaep_learners', (array) $existing_user->roles, true) &&
                    !in_array('guest_lesaep_learner', (array) $existing_user->roles, true)) {
                    $existing_user->set_role('guest_lesaep_learner');
                }
            } else {
                ...
                if ($new_user) {
                    $new_user->set_role('guest_lesaep_learner');
                }
...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7. Ready to dive in?\"",
     ...
     */
    /**
     * v8.0.0-claude-02 Bug 2 fix (updated): Strict validation — both display_name and password
     * are now required. Previously the handler was permissive (password optional, no
     * auth check, no completion flag). Changes from original:
     *   - 401 if not logged in (was missing entirely)
     *   - 400 if display_name empty
     *   - 400 if password empty or < 6 chars
     *   - is_wp_error() check on wp_update_user() — return 500 on failure so
     *     _flosc_profile_completed is never set on a failed DB write (prevents
     *     permanently silencing the profile card for a user who never actually completed)
     *   - Saves both atomically — no partial saves
     *   - Sets _flosc_profile_completed meta on success only
     */
    public function handle_update_guest_profile($request) {
        ...
        if (empty($password) || strlen($password) < 6) {
            return new WP_REST_Response(['success' => false, 'message' => 'Password must be at least 6 characters.'], 400);
        }

        // Save display name — check for DB errors before touching password or meta
        ...
        if (is_wp_error($updated)) {
            return new WP_REST_Response(['success' => false, 'message' => 'Could not update profile.'], 500);
        }
        ...
        // Mark profile as completed — gates needsProfileCompletion in FLOSC_CONFIG to false.
        // Only reached if wp_update_user() succeeded above. _flosc_profile_completed is
