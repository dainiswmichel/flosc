...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7. Ready to dive in?\"",
     ...
     */
    /**
     * v8.0.0-claude-01 Bug 2 fix: Strict validation — both display_name and password
     * are now required. Previously the handler was permissive (password optional, no
     * auth check, no completion flag). Changes:
     ...
     *   - Sets _flosc_profile_completed meta on success, which causes needsProfileCompletion
     *     in FLOSC_CONFIG to return false, permanently suppressing the profile card prompt
     */
    public function handle_update_guest_profile($request) {
        ...
        // Save both atomically — no partial saves
        wp_update_user([
            'ID'           => $user_id,
        ...
        // from this point on. _flosc_profile_completed is never unset once true.
