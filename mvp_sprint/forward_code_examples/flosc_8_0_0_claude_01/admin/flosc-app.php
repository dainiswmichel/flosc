...
<body class="flosc-app<?php echo is_admin_bar_showing() ? ' admin-bar' : ''; ?>"
    ...
    <script>
        ...
        window.FLOSC_CONFIG = <?php 
            ...
            // v1.9.3: AI provider — frontend needs to know if AI is active
            // When AI is active, frontend routes IVR matches through AI instead of displaying directly
            'aiProvider' => flosc_get_setting('ai_provider', 'ivr'),
            ...
            // Guest link config
            // v8.0.0-claude-01 Bug 1 fix: wp_unslash() strips accumulated escape layers from
            // settings containing apostrophes (e.g. "We've"). The autoprompts loop already does
            // this — these four guest link settings were the only ones missing the same treatment.
            // guestLinkUpgradeUrl is a URL (no apostrophes) — left as-is intentionally.
            'guestLinkName'              => wp_unslash(flosc_get_setting('guest_link_name', 'Complimentary LeSAEp Learners Guest Access Link')),
            'guestLinkCheckEmailMessage' => wp_unslash(flosc_get_setting('guest_link_check_email_message', "We've sent you a {link_name} to your email — click it to access this chat as a guest and view your quiz score, free lessons, and a special upgrade offer.")),
            'guestLinkWelcomeMessage'    => wp_unslash(flosc_get_setting('guest_link_welcome_message', 'Hi, welcome back! Just to confirm: your email address is {email} and you can use your {link_name} {n} more times to access this chat. <a href="{upgrade_url}">Upgrade</a> for complete access to all lessons, quiz audios, and our LeSAEp Learners network...')),
            ...
            // v8.0.0-claude-01 Bug 2 fix: Replace transient-based isFirstGuestLogin with
            // persistent user-meta gate. The transient approach was race-prone — if the redirect
            // took longer than the transient window, or OPcache served stale PHP, the flag was
            // false and the profile card never appeared. This meta-based check persists across
            // sessions until the user completes their profile (sets _flosc_profile_completed).
            // JS now checks needsProfileCompletion independently of guestLinkRemaining.
            // IMPORTANT: magic link users receive role 'guest_lesaep_learner' (flosc.php ~line 2006).
            // Do NOT use 'lesaep_learners' here — that is the paid role and would never match.
            'needsProfileCompletion' => (function() {
                if (!is_user_logged_in()) return false;
                $uid = get_current_user_id();
                if (get_user_meta($uid, '_flosc_profile_completed', true)) return false;
                $user = get_userdata($uid);
                if (!$user || !in_array('guest_lesaep_learner', (array) $user->roles, true)) return false;
                return true;
            })(),
            'guestLinkProfileConfirmMessage' => wp_unslash(flosc_get_setting('guest_link_profile_confirm_message', 'Perfect, {name}! You can always log in directly at {login_url}, update your profile, and upgrade to full access.')),
        ]); ?>;
