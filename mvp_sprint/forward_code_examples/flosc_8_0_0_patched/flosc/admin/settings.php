...
if (isset($_POST['flosc_save']) && wp_verify_nonce($_POST['_wpnonce'], 'flosc_save_settings')) {
    ...
    if ($active_tab === 'sso') {
        ...
        foreach ($sso_providers as $provider) {
            if (!isset($_POST["flow_sso_{$provider}_enabled"])) {
    ...
    if ($active_tab === 'quiz') {
        ...
        if (!isset($_POST['flow_enabled_quizzes'])) {
            $new_settings['enabled_quizzes'] = [];
        }
        foreach (['A', 'B', 'C', 'D', 'E'] as $letter) {
            if (!isset($_POST["flow_quiz_variant_{$letter}_enabled"])) {
    ...
    if ($active_tab === 'member-levels') {
        ...
        $level_slugs        = $_POST['level_slug']        ?? [];
        $level_names        = $_POST['level_name']        ?? [];
        $level_descriptions = $_POST['level_description'] ?? [];
        $member_levels = [];
        ...
        $prot_types  = $_POST['protection_type']  ?? [];
        $prot_values = $_POST['protection_value'] ?? [];
        $prot_levels = $_POST['protection_level'] ?? [];
        $protected_content = [];
    ...
    if ($active_tab === 'lessons') {
        $group_quizzes    = $_POST['lesson_group_quiz']     ?? [];
        $group_categories = $_POST['lesson_group_category'] ?? [];
        $lesson_groups = [];
    ...
    if ($active_tab === 'ui') {
        ...
        $menu_labels  = $_POST['visitor_menu_label']  ?? [];
        $menu_actions = $_POST['visitor_menu_action'] ?? [];
        $new_menu = [];
        ...
        $guest_labels  = $_POST['guest_menu_label']  ?? [];
        $guest_actions = $_POST['guest_menu_action'] ?? [];
        $new_guest_menu = [];
        ...
        $member_labels  = $_POST['member_menu_label']  ?? [];
        $member_actions = $_POST['member_menu_action'] ?? [];
        $new_member_menu = [];
        ...
        $login_dest = trim($_POST['flosc_login_destination'] ?? '');
        ...
        $profile_bar = [
            'visitor' => [
                ...
                'badge' => sanitize_text_field($_POST['profile_bar_visitor_badge'] ?? 'Hope you enjoy our chat :-)'),
        ...
        <?php elseif ($active_tab === 'identity'): ?>
            ...
            if (isset($_POST['flosc_save_flow']) && wp_verify_nonce($_POST['_wpnonce'], 'flosc_save_settings')) {
                ...
                if (isset($all_flows[$save_ivr])) {
                    ...
                    $id_keys = ['name', 'title', 'tagline', 'primary_color', 'chatlogo_url', 'favicon_url', 'badgeUrl', 'share_text'];
            ...
            <div style="max-width: 1000px;">
                ...
                <?php foreach ($all_flows as $ivr_file => $flow_data): 
                    ...
                    foreach (['name', 'title', 'tagline', 'primary_color', 'chatlogo_url', 'favicon_url', 'badgeUrl', 'share_text'] as $_ik) {
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        ...
                        <div>
                            ...
                            <div style="margin-bottom: 15px;">
                                ...
                                <input type="text" name="<?php echo $prefix; ?>title" 
