...
<p>An individual message in the chat interface, rendered as a styled HTML element. User messages appear right-aligned; assistant messages appear left-aligned. Chat style presets control the bubble shape (rounded, sharp, pill), colors, and font.</p>
...
<p>A named CSS theme for the chat interface. Presets include variants like "Classic", "Modern", "Minimal", etc. Each preset is a separate CSS file loaded alongside the base layout. Selectable from the Chat Styling admin page without editing code.</p>
...
<h3 id="term-corrections">Corrections (AI Feedback)</h3>
<p>Admin-written examples of AI responses that were wrong, with the correct response. Stored in WordPress options and injected into the AI system prompt as negative examples. The AI is instructed to avoid repeating the corrected mistakes. Configured from the AI Corrections admin page.</p>
...
<p>Admin-written examples of AI responses that were correct and should be reinforced. The counterpart to Corrections. Injected into the AI system prompt as positive examples. Configured from the AI Corrections/Praise admin page.</p>
