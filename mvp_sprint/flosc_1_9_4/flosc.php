...
The {product_name} Team";
    ...
    public function handle_chat($request) {
        ...
        // v1.9.2: Chatpack — compute session tracking metadata (backend-authoritative)
        $chatpack_user_id = $eval_context['user_id'] ?? 0;
        $chatpack_session_hash = FLOSC_Chatpack::generate_session_hash($chatpack_user_id, $session_id);
        ...
        if ($response_message && $ai_available) {
            ...
            $chatpack_prompt = $chatpack_is_first
                ? FLOSC_Chatpack::build_full_chatpack($phase, $eval_context, $flow_id, $chatpack_session_hash, $chatpack_pair_number, $response_message['content'])
        ...
        if (!$response_message) {
            ...
            if ($ai_available) {
                ...
                if ($flosc_use_rag) {
                    ...
                    $chatpack_prompt = $chatpack_is_first
                        ? FLOSC_Chatpack::build_full_chatpack($phase, $eval_context, $flow_id, $chatpack_session_hash, $chatpack_pair_number)
