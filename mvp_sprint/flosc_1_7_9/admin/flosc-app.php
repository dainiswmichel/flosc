...
<body class="flosc-app"
    ...
    <aside class="flosc-sidebar" id="flosc_app_sidebar">
        ...
        <?php
        $visitor_bar_text = get_option('flosc_visitor_bar_text', 'Sign in to get started');
        $visitor_bar_icon = get_option('flosc_visitor_bar_icon', '👋');
        ?>
        <div class="visitor-profile-card" id="flosc_visitor_profile_card">
            <button class="profile-button" id="flosc_visitor_profile_button">
                <div class="profile-avatar visitor-avatar">
                    <?php echo esc_html($visitor_bar_icon); ?>
                </div>
                <div class="profile-info">
                    <div class="profile-name">Visitor</div>
                    <div class="profile-badge"><?php echo esc_html($visitor_bar_text); ?></div>
                </div>
            ...
            <div class="profile-dropdown" id="flosc_visitor_profile_dropdown">
                <?php
                // Get configurable visitor menu items (associative: key = action)
                ...
                foreach ($visitor_menu as $action => $item):
                    if (!isset($item['enabled']) || $item['enabled']):
                ?>
                <a href="#" class="profile-dropdown-item" data-action="<?php echo esc_attr($action); ?>">
        ...
        <!-- User profile card (for logged-in users) -->
        <?php if (is_user_logged_in()): ?>
    ...
    <main class="flosc-main">
        ...
        <div class="chat-container" id="flosc_chat_container">
            ...
            <!-- v1.7.9: Visitor Bar - DISABLED by default. Enable via flosc_visitor_bar_enabled option.
                 Removed from default build per floscAdmin feedback 2026-02m-14d. -->
