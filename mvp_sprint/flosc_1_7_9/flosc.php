...
The {product_name} Team";
    ...
    public function register_settings() {
        ...
        // Visitor Profile Bar (v1.7.8)
        register_setting('flosc_settings', 'flosc_visitor_bar_text');
        register_setting('flosc_settings', 'flosc_visitor_bar_icon');
    ...
    /**
     * v1.8.0: Render UI & Navigation Settings Page
     */
    public function render_ui_navigation_page() {
        include FLOSC_PLUGIN_DIR . 'admin/ui-navigation.php';
    }
    ...
    public function register_rest_routes() {
        ...
        // v1.7.1: Nonce refresh endpoint for stale cookie recovery
        // v1.7.7: Requires login — only authenticated users need nonce refresh
    ...
    private function build_rag_system_prompt($user_context) {
        ...
        $personality_desc = get_option('flosc_personality_description', 'friendly and knowledgeable learning guide');
...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7. Ready to dive in?\"",
    ...
    public function paypal_create_order($request) {
        ...
        // v1.7.5: Resolve price — check offer pricing, then product config fallback
        $amount = 0;
        if (!empty($offer['pricing']['price'])) {
            $amount = floatval($offer['pricing']['price']);
        }
        if ($amount <= 0) {
            // Fallback to product-level price (set in admin Identity tab)
            $product = $this->get_product_config();
            $amount = floatval($product['price'] ?? 0);
        }
        if ($amount <= 0) {
        ...
        if (empty($lessons)) {
            ...
            if (!empty($category)) {
                if (is_numeric($category)) {
                    ...
                    $diagnostic['category_exists'] = ($term && !is_wp_error($term));
                    if ($term && !is_wp_error($term)) {
                        $diagnostic['category_name'] = $term->name;
                        $diagnostic['category_post_count'] = $term->count;
                    }
                } else {
                    $term = get_term_by('slug', sanitize_title($category), 'category');
                    $diagnostic['category_exists'] = !empty($term);
                    if ($term) {
                        $diagnostic['category_name'] = $term->name;
                        $diagnostic['category_post_count'] = $term->count;
                    }
