...
### 🔐 User Status Check Fix (Critical)

**Root Cause Identified:**
The JavaScript `fetch()` call to the `/chat` API endpoint was missing `credentials: 'same-origin'`. Without this, the browser does NOT send cookies with the request. WordPress REST API needs cookies + nonce to authenticate users.

**The Fix (in `flosc-app.js` line ~2660):**
```javascript
...
**Result:**
- Admin asking "What's my user status?" now correctly returns "You are the **FLOSC Admin**"
- Previously returned "You are a **Visitor**" because server couldn't identify logged-in user

### ⚠️ After Update: Re-Import IVR Messages
If the database has stale IVR messages, go to FLOSC Settings > IVR Messages and click "Import from ivr.md" to ensure `{user_status_response}` placeholder is used.
...
### v1.1.2
- Attempted fixes (didn't work - wrong method names)

### v1.0.9
- Simplified IVR message structure
| **Guest Messages** | Guest | Login, Offer | PromptPanel (pills) |
| **Member Messages** | Member | Sale, Content | MemberPromptPanel (pills) |

**Key insight:** Offer/Sale/Content phases are handled by **conditions**, not separate sections:
- Offer messages = Guest Messages with `offer_shown_*` conditions
