...
class FLOSC_Quiz_Manager {
    ...
    public static function submit_score($user_id, $quiz_id, $score_data) {
        if (!$user_id || !$quiz_id || !is_array($score_data)) {
            if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) error_log("FLOSC Quiz Manager: Invalid parameters - user_id: {$user_id}, quiz_id: {$quiz_id}");
            return false;
        }
        
        // Ensure score is set
        if (!isset($score_data['score'])) {
            if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) error_log("FLOSC Quiz Manager: Missing score in score_data");
            return false;
        }
        ...
        if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) error_log("FLOSC Quiz Manager: Submitted score for user {$user_id}, quiz {$quiz_id}, score {$score_data['score']}%");
