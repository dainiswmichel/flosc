...
class FLOSC_Framework {
    ...
    private function init_hooks() {
        ...
        // v5.0.9: Allow PayPal iframe on FLOSC app pages.
        // Security plugins (Wordfence, iThemes) add CSP headers that block PayPal's
        // frame-src, causing "Load failed" in the payment modal. We add PayPal domains
        // to the CSP only on FLOSC pages where the payment modal is rendered.
        add_action('send_headers', [$this, 'flosc_add_paypal_csp_headers']);
...
The {product_name} Team";
    ...
    /**
     * v5.0.9: Append PayPal domains to Content-Security-Policy on FLOSC pages.
     * Security plugins add CSP headers blocking third-party frames — PayPal's
     * button iframe shows "Load failed" when frame-src doesn't include PayPal.
     * We hook send_headers and only act on FLOSC app pages.
     */
    public function flosc_add_paypal_csp_headers() {
        if ( ! $this->is_flosc_request() ) return;

        // Check if a CSP header has already been set by another plugin
        ...
        $paypal_frame_src  = 'https://*.paypal.com https://*.paypalobjects.com';
        $paypal_script_src = 'https://*.paypal.com https://*.paypalobjects.com';
        $paypal_img_src    = 'https://*.paypal.com https://*.paypalobjects.com';

        if ( $csp_header ) {
            // Existing CSP — append PayPal to frame-src and script-src
            $updated = $csp_header;
            foreach ( [ 'frame-src', 'frame-ancestors', 'script-src', 'img-src' ] as $directive ) {
                if ( stripos( $updated, $directive ) !== false ) {
            ...
            // No existing CSP — add a permissive one that allows PayPal
            header( "Content-Security-Policy: frame-src 'self' {$paypal_frame_src}; script-src 'self' 'unsafe-inline' 'unsafe-eval' {$paypal_script_src}; img-src 'self' data: {$paypal_img_src};" );
        }
    }
    ...
    public function register_rest_routes() {
        ...
        register_rest_route('flosc/v1', '/sandbox-purchase', [
            ...
            'permission_callback' => '__return_true', // v5.0.9: Open to visitors — handler creates temp user if needed
        ]);
...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7. Ready to dive in?\"",
    ...
    /**
     * v5.0.7: PayPal - Create Order
     * Creates a PayPal order for the given offer, returns order_id for JS SDK approval.
     *
     * v5.0.7 fixes:
     * - Currency uses provider's centralized resolution (matches SDK)
     * - Logs full diagnostic info on every failure path
     * - Ensures sandbox credentials are available even without activation hook
     */
    ...
    public function paypal_capture_order($request) {
        ...
        $access_manager = $this->sale_manager->access();
        if ($user_id && $access_manager->has_offer($user_id, $offer_id)) {
            if (FLOSC_DEBUG) error_log('[FLOSC-PAYPAL] capture_order: user already has access');
            set_transient('flosc_just_purchased_' . $user_id, true, 300);
            return new WP_REST_Response([
                'success' => true,
                'message' => 'Access already granted',
