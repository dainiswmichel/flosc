...
<h3>🅿️ PayPal Configuration <span style="background: #28a745; color: #fff; padding: 2px 8px; border-radius: 3px; font-size: 11px; font-weight: bold;">v1.7.2</span></h3>
<p class="description">Accept payments via PayPal. Smart Payment Buttons with sandbox and live support.</p>
...
function flosc_process_stripe_payment($flow_settings, $offer_id, $user_email, $user_name) {
    ...
    try {
        ...
        if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) error_log('Stripe payment error: ' . $e->getMessage());
