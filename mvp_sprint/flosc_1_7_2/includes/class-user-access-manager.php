...
class FLOSC_User_Access_Manager {
    ...
    public function grant_member_access($user_id, $reason = 'quiz_completion') {
        ...
        // Log the grant
        if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) error_log("FLOSC: Granted member access to user {$user_id} - Reason: {$reason}");
    ...
    public function revoke_member_access($user_id) {
        ...
        if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) error_log("FLOSC: Revoked member access from user {$user_id}");
            ...
            if (function_exists('wlmapi_get_member_levels')) {
                try {
                    ...
                    if (defined('FLOSC_DEBUG') && FLOSC_DEBUG) {
                        error_log('FLOSC WishList Member API error: ' . $e->getMessage());
                    }
                }
            }
