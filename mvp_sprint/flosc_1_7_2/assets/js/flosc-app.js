...
class floscApp {
    ...
    openCheckout(offerId) {
        ...
        // v1.7.2: Show modal if Stripe OR PayPal is available
        const hasStripe = !!this.config.stripeKey;
        const hasPayPal = !!this.config.paypalClientId && !!window.paypal;
...
Purchased: ${ctx.purchased}
    ...
    showPaymentModal(offerId) {
        ...
        const hasStripe = !!this.stripe;
        const hasPayPal = !!window.paypal && !!this.config.paypalClientId;

        // Show/hide sections based on available providers
        const paypalContainer = document.getElementById('paypal-button-container');
        const divider = document.getElementById('payment-divider');
        const stripeForm = document.querySelector('.flosc-payment-form');
        const payBtn = document.getElementById('payBtn');

        if (paypalContainer) paypalContainer.style.display = hasPayPal ? 'block' : 'none';
        if (divider) divider.style.display = (hasPayPal && hasStripe) ? 'block' : 'none';
        if (stripeForm) stripeForm.style.display = hasStripe ? 'block' : 'none';
        if (payBtn) payBtn.style.display = hasStripe ? 'inline-flex' : 'none';

        // v1.7.2: Render PayPal Smart Payment Buttons
        if (hasPayPal && paypalContainer) {
            this.renderPayPalButton(offerId, paypalContainer);
        }

        // Mount Stripe Elements if available
        if (hasStripe) {
            this.mountStripeCard(offerId);
        }
    ...
    // v1.7.2: Render PayPal Smart Payment Buttons inside the modal
    renderPayPalButton(offerId, container) {
        ...
        window.paypal.Buttons({
            ...
            onApprove: async (data) => {
                // Capture the payment on our server
                ...
                if (result.success) {
                    ...
                    this.addMessage('assistant', '🎉 **Payment successful via PayPal!** Welcome to full membership! Refreshing your access...');
                    setTimeout(() => window.location.reload(), 2000);
                } else {
                    this.addMessage('assistant', '❌ PayPal payment failed: ' + (result.message || 'Unknown error'));
                }
            },
            ...
            onError: (err) => {
                ...
                if (errorEl) errorEl.textContent = 'PayPal payment failed. Please try again.';
            },

            onCancel: () => {
                console.log('[FLOSC-CHECKOUT] PayPal payment cancelled by user');
            },
        }).render(container);
    }

    // v1.7.2: Extracted Stripe card mounting from showPaymentModal
    mountStripeCard(offerId) {
