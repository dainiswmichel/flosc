...
The {product_name} Team";
    ...
    public function register_rest_routes() {
        ...
        // Free Lesson (v9.1.9, v1.4.4: accept both GET and POST — JS calls POST)
...
\"Great! As a member, you have full access. Based on your quiz, I recommend starting with Lesson 7: [link]. It has the complete IPA guide and video walkthrough. Ready to dive in?\"",
    ...
    /**
     * v1.4.0: Product-Aware Sandbox Purchase
    ...
    public function handle_sandbox_purchase($request) {
        ...
        // Grant full access via access manager (sale system)
        ...
        // v1.4.4: Also grant via content protection system (FLOSC_Member_Access)
        // This sets _flosc_member_access='true' so is_member() returns true
        // and _flosc_memberlevel_{level}='yes' so has_level() returns true
        // Both are required for content protection to unlock posts
        require_once FLOSC_PLUGIN_DIR . 'includes/class-member-access.php';
        $member_access = FLOSC_Member_Access::instance();
        $member_access->grant_member_access($user_id, [
            'offer_id' => $offer_id ?: 'flosc_sandbox',
            'grants_level' => $member_level,
            'transaction_id' => $transaction_id,
            'timestamp' => time(),
        ]);
        // Grant the specific level so category-level protection unlocks
        $member_access->grant_level($user_id, $member_level);
