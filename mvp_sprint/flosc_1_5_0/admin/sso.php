...
/* v1.5.0: Connection test result styles */
.flosc-test-check {
    display: flex;
...
.flosc-test-check:last-child {
    border-bottom: none;
}
.flosc-test-check-icon {
    flex-shrink: 0;
    width: 20px;
    text-align: center;
    font-size: 14px;
}
.flosc-test-check-label {
    flex-shrink: 0;
    font-weight: 600;
    min-width: 160px;
    color: #1d2327;
}
.flosc-test-check-detail {
    color: #50575e;
}
.flosc-test-check.pass .flosc-test-check-detail { color: #00a32a; }
.flosc-test-check.fail .flosc-test-check-detail { color: #d63638; }
.flosc-test-summary {
    margin-top: 10px;
    padding: 8px 14px;
    border-radius: 4px;
...
.flosc-test-summary.all-pass {
    background: #edfaef;
    border: 1px solid #00a32a;
    color: #00731e;
}
.flosc-test-summary.has-fail {
    background: #fef1f1;
...
<script>
(function() {
    var nonce = '<?php echo wp_create_nonce('flosc_test_sso'); ?>';
        ...
        btn.addEventListener('click', function() {
            ...
            fetch(ajaxUrl, { method: 'POST', body: formData, credentials: 'same-origin' })
                ...
                .then(function(resp) {
                    ...
                    if (!resp.success) {
                        statusEl.textContent = '❌ ' + (resp.data || 'Test failed');
                    ...
                    if (data.all_pass) {
                        html += '<div class="flosc-test-summary all-pass">✅ All checks passed — ready for live testing</div>';
                        statusEl.textContent = '✅ All checks passed';
                        statusEl.style.color = '#00a32a';
                    } else {
                        html += '<div class="flosc-test-summary has-fail">⚠️ Some checks failed — review the items above</div>';
                        statusEl.textContent = '⚠️ Issues found — see details below';
                        statusEl.style.color = '#d63638';
                    }
