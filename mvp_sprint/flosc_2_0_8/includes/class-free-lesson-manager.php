...
class FLOSC_Free_Lesson_Manager {
    ...
    private function find_lesson_post($lesson_num) {
        ...
        // v1.8.0: Try configured category with proper ID vs slug handling
        if (!empty($configured_cat)) {
            $args = [
    ...
    public function get_free_lessons($user_id) {
        ...
        foreach ($lesson_nums as $lesson_num) {
            ...
            if ($post) {
                ...
                // renders shortcodes, wpautop, embeds, etc. — the JS receives
                // ready-to-display HTML instead of raw post_content
