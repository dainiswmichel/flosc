...
$lessons_docs_url = add_query_arg([
    'page' => 'flosc-settings',
    'ivr'  => $current_ivr,
    'tab'  => 'documentation',
    'doc'  => 'ref-admin',
...
echo '<div style="margin:-8px 0 14px; text-align:right;">'
   . '<a href="' . esc_url($lessons_docs_url) . '" style="font-size:12px; text-decoration:none; color:#2271b1;">Docs</a>'
   . '</div>';
...
<?php if (!empty($protected_categories)): ?>
    ...
    <?php foreach ($protected_categories as $cat_slug => $cat_info):
        ...
        $req_level_label = isset($available_levels[$req_level])
            ? '"' . ($available_levels[$req_level]['offer_name'] ?? '') . '"'
            : $req_level;
    ?>
    <div class="flosc-protection-block" style="background: #f9f9f9; border: 1px solid #ddd; padding: 16px; border-radius: 6px; margin-bottom: 16px;">
        <p style="margin-top: 0;">
            ...
            <?php if ($is_protected): ?>
                ...
                <?php if ($req_level): ?>
                    <span style="color: #666;"> &mdash; required level: <code><?php echo esc_html($req_level); ?></code> (<?php echo esc_html($req_level_label); ?>)</span>
                <?php endif; ?>
            <?php else: ?>
        ...
        <table class="widefat" style="max-width: 700px;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <?php if ( $matching_level ) : ?>
                            <?php echo esc_html($matching_level['offer_name']); ?>
                        <?php else : ?>
                            <em style="color:#999;">—</em>
                        <?php endif; ?>
                    </td>
                    <td><?php echo $matching_level ? esc_html($matching_level['price']) : esc_html('—'); ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
