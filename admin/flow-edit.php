...
$flow_id = sanitize_key(wp_unslash($_GET['flow_id'] ?? ''));
$is_new = ($flow_id === 'new');
...
// Get current tab
$current_tab = sanitize_key(wp_unslash($_GET['tab'] ?? 'identity'));
$tabs = [
    'identity' => 'Identity',
    'ivr' => 'IVR',
    'content' => 'Content',
...
if (isset($_POST['flosc_save_flow']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['_wpnonce'] ?? '')), 'flosc_save_flow')) {
    ...
    if (isset($_POST['visitor_bar_text']) || isset($_POST['visitor_bar_icon'])) {
        ...
        if (isset($_POST['visitor_bar_text'])) {
            $profile_bar['visitor']['badge'] = sanitize_text_field(wp_unslash($_POST['visitor_bar_text']));
        }
        if (isset($_POST['visitor_bar_icon'])) {
            $profile_bar['visitor']['icon'] = sanitize_text_field(wp_unslash($_POST['visitor_bar_icon']));
        }
...
<div class="wrap">
    <h1>
        ...
        &nbsp;/&nbsp;
        <?php echo $is_new ? 'New Flow' : esc_html(($flow['identity']['name'] ?? $flow_id)); ?>
    </h1>
    ...
    <?php if (!$is_new): ?>
        ...
        <nav class="nav-tab-wrapper">
            <?php foreach ($tabs as $tab_id => $tab_label): ?>
                     <a href="<?php echo esc_url( admin_url('admin.php?page=flosc-flow-edit&flow_id=' . urlencode($flow_id) . '&tab=' . $tab_id) ); ?>" 
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" id="floscflow_tagline" name="floscflow_tagline" 
                                   value="<?php echo esc_attr($identity['tagline'] ?? ''); ?>" 
                                   class="large-text" 
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="color" id="floscflow_color" name="floscflow_color" 
                                   value="<?php echo esc_attr($identity['primary_color'] ?? '#4f46e5'); ?>" 
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="url" id="floscflow_chatlogo" name="floscflow_chatlogo" 
                                   value="<?php echo esc_attr($identity['chatlogo_url'] ?? ''); ?>" 
                                   class="large-text" 
                                   placeholder="https://...">
                        ...
                        <td>
                            <div style="display: flex; align-items: center; gap: 12px;">
                                <input type="url" id="floscflow_favicon" name="floscflow_favicon" 
                                       value="<?php echo esc_attr($identity['favicon_url'] ?? ''); ?>" 
                                       class="regular-text" 
                                       placeholder="https://...">
                                <button type="button" class="button" id="flosc_upload_app_icon">Upload</button>
                                <?php $favicon_url = $identity['favicon_url'] ?? ''; ?>
                                <img src="<?php echo esc_url($favicon_url); ?>" 
                                     alt="" id="flosc_app_icon_preview"
                                     style="width: 32px; height: 32px; border-radius: 6px;<?php echo empty($favicon_url) ? ' display: none;' : ''; ?>">
                            </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="url" id="floscflow_badge" name="floscflow_badge" 
                                   value="<?php echo esc_attr($identity['badgeUrl'] ?? ''); ?>" 
                                   class="large-text" 
                                   placeholder="https://...">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" id="floscflow_share_text" name="floscflow_share_text"
                                   value="<?php echo esc_attr($identity['share_text'] ?? ''); ?>"
                                   class="large-text"
                    ...
                    <table class="widefat" style="max-width: 600px;">
                        ...
                        <tbody>
                            <?php foreach ($team_users as $user): ?>
                                <tr>
                                    <td>
                                        <input type="checkbox" name="team_users[]" value="<?php echo esc_attr( $user->ID ); ?>"
